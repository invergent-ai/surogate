...
- [x] **CHECKED, no drift** (`f0b4e37`) `wrapper/attn_input_proj.cpp` agrees with its plan: the gated path derives the split from parent rows and lands on all three gated families; the ungated table matches the plan's three predicates entry for entry; the workspace gate lists only gated shapes, which is consistent since no ungated target asks it. The split is restated in **four** places though, and nothing checked the agreement — so the two ungated parents this engine serves (qwen3-0.6b, tinyllama) now run through the public op, which is both their first numbers and the check. Removing tinyllama from the wrapper's table fails them.
- [ ] **TODO** The qwen3.5-4b gated shape `{10240, 2560}` is unexercised in `test_attn_input_proj.cpp` (covered by that target's own tests) — a gap in the file, not the engine.
...
- [~] **IN PROGRESS** Converter/registry/package boilerplate duplicated per target. Hoist into `family/`.
  - [x] **DONE** (`f1a869eb`) Debug probe, `graph_profiles_through`, `validate_token_interval`, unrunnable-leaf stubs. 608 lines out of the targets, 265 into `family/`. The three dense targets roughly halve.
  - [x] **DONE** (`3673030c`) `LoadedX`/`XInstance` are `LoadedTarget<Package>`/`TargetInstance<Package>` with an alias per target. `registry.h` 334 → 131, `registry.cpp` 631 → 490. The text-only frontend-resource trio went to `family::bind_text_only_frontend_resources` at the same time.
  - [x] **DONE** (`a0d40f05`) `LoadPlan`/`LoadedModel` pimpl: four macros in `api/family/target_package.h`. 630 lines out, 130 in. `plan_load` and `make_sequence_planner` stay per-target — the two that differ are doing real work.
  - [x] **DONE** (`89d96c64`) `convert/qwen3_6/common/` → `convert/common/`. That package held no converter and no internal user, while llama, gemma3, qwen3, and qwen4exp all imported through it. Caught a latent `Path(__file__).parents[3]` that the move silently redirected away from an 11.9 MB draft-head fixture.
  - [x] **DONE** (`bda92164`) `engine.cpp`'s executor ladder. All twelve arms built `ConcurrentExecutor<Instance>` from the instance they matched, so the ladder carried no decision; both it and the executor variant now derive from `targets::ActiveTarget`. Adding a target needs no edit in `engine.cpp`. Also closes a trap: the ladder's final `else` caught `Qwen38FlashNextInstance` rather than erroring, so a thirteenth target would have been handed the Flash-Next executor silently.
  - [ ] **PARKED** (owner: leave the converter for now) The `convert()` driver. Not one duplicated loop but clusters: llama/qwen3 byte-identical (72 lines), gemma3 95% of them, the two 27Bs ~76%, the three Qwen3.5 converters identical to each other (96 lines); qwen4exp, gemma_embedding and 35B-A3B are genuinely different and should stay. **Blocked on verification, not on effort:** no test anywhere calls `convert()`, and no source checkpoint is on this disk, so there is currently no way to show a refactor of the artifact-producing code changed nothing. The prerequisite is a synthetic-checkpoint round-trip — gemma3 is the cheapest at ~536 MB of tensors (236 sources, all fifteen HF name patterns derivable from the recipes) and its config fixture already exists in `tests/serve/test_gemma3_convert.py`, which states outright that it never opens a shard.
