...
- [?] **Shared expert under NVFP4.** The export quantizes `shared_expert.{gate,up,down}_proj`; `sparse_moe` computes the shared expert *inside* its routed kernel bodies (`dot_two_rows<W8Codec, …>` in the decode body) and admits W8 only (`sparse_moe.cpp:154`). Three routes: (a) swap the shared codec to the NVFP4 one the routed path already has, and route the shared expert through generic NVFP4 ops at prefill; (b) compute the shared expert in the leaf with generic `linear` and give the op a no-shared mode (it already produces the per-token `shared_scale`); (c) requantize those 120 tensors to W8 as a documented lossy exception (~0.8 % on 3.1 M params per layer). (c) needs no kernel work but overrides the export for those tensors; (a) is the honest one. Decide after the route defect above is settled.
