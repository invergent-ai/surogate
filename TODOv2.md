...
- [~] Kernels: **NVFP4 activation quantizer at K = 2048 and K = 512.** The file's NVFP4 Linears at this geometry are `[9216×2048]` (attention input), `[2048×4096]` (`o_proj`), `[1024×2048]` (shared `gate_up`), `[2048×512]` (shared `down`, K = `intermediate`). `Nvfp4ActivationGeometry<K>` asserts only `K % 64 == 0`, and `launch_quantize_exact` is one thread per 16-group with no row tiling, so both sizes are an alias, a `case` in the one K-switch (`nvfp4_w4a4.cu`), and a gate entry. Edited; not yet built.
- [ ] **M1 scope decisions**, recorded so they are not relitigated mid-implementation:
  ...
  4. *Structure stays per-target until M4.* The 35B's `inventory.py`/`recipe.py` remain the structure source for M1; only the per-object *format* comes from the resolver, and the converter's two source roles (`--model` BF16 base + `--routed-nvfp4-dir`) collapse to one directory.
