...
- Descriptor + heuristic caching is straightforward (csrc/src/kernels/matmul.cpp:84), but at 8B it’s likely only low single‑digit % unless CPU launch is actually on the critical
  path; it’s still useful to make CUDA-graph replay “pure” (no per-iter host setup).
- “Persistent cuDNN graphs” is mostly already implemented: SDPA graphs are cached (csrc/src/kernels/cudnn_att.cpp:86). The remaining overhead called out in the doc is mainly per-call
  unordered_map variant-pack construction (csrc/src/kernels/cudnn_att.cpp:288).
- Fusion items (RMSNorm+GEMM, proj+res+norm, SwiGLU+GEMM) are feasible but high-effort; cuBLASLt epilogues won’t cover RMSNorm/SwiGLU, so this realistically means CUTLASS/
  TransformerEngine/TensorRT‑LLM style kernels.

## Corrections / Gaps in the Doc

- “No operation fusion across layers” isn’t accurate anymore: residual+RMSNorm is already fused in the main forward path (csrc/src/modules/modular_model.h:873, csrc/src/modules/
  modular_model.h:995).
- “No overlap” is also partly outdated: NCCL has a dedicated comm stream (csrc/src/utilities/comm.h:68) and reduce-scatter can be triggered per-layer (csrc/src/modules/
  gradient_manager.h:545).
- Phase 3.2’s cudaMemPrefetchAsync doesn’t prefetch device weights into L2 (it’s managed-memory prefetch); you already prefetch next block weights via gather_block on a side stream
  (csrc/src/modules/modular_model.h:858).
- The validation commands reference non-existent configs (bf16_scaling_analysis.md:390); the working harness is benchmarks/benchmark.sh (benchmarks/benchmark.sh:1).

## High-ROI Enhancements to Add

- cuBLASLt autotune > “cache heuristic”: request top‑N heuristics, time them at init, cache best algo per (dtype/epilogue/MNK/ld*/workspace/alignment), and consider making workspace
size configurable (32MB can be limiting) (csrc/src/kernels/matmul.cpp:84).
- Fused MLP kernel (GEMM → SwiGLU → GEMM with minimal intermediate traffic) is likely the biggest single‑GPU lever at 8B; CUTLASS EVT is one route, but reusing a proven TE/TRT‑LLM
fused MLP tends to land faster.
- If the goal is ≥1.5× at 8B BF16, plan for at least one “compute-path” win (autotuned GEMMs and/or fused MLP / FA3); descriptor/graph bookkeeping alone is unlikely to close that
gap.