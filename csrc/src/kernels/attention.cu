...
// ============================================================================
// KV-cache extraction (for GRPO inference)
// ============================================================================

/// Extract K or V from interleaved QKV and write into the KV-cache.
///
/// qkv   : (B, T, H, HS)  where H = Hq + 2*Hkv
/// cache : (B, max_seq_len, Hkv, HS)
/// head_start : Hq for K,  Hq+Hkv for V
/// cache_pos  : first position in the time axis to write to
///
/// Grid  : (Hkv, T, B)   Block : min(HS, 256)
__global__ void extract_kv_to_cache_kernel(
        nv_bfloat16* __restrict__ cache,
        const nv_bfloat16* __restrict__ qkv,
        int B, int T, int H, int Hkv, int HS,
        int max_seq_len, int cache_pos, int head_start)
{
    const int h  = blockIdx.x;
    ...
    if (h >= Hkv || t >= T || b >= B || hs >= HS) return;

    const nv_bfloat16 val =
        qkv[b * T * H * HS + t * H * HS + (head_start + h) * HS + hs];

    cache[b * max_seq_len * Hkv * HS + (cache_pos + t) * Hkv * HS + h * HS + hs] = val;
}
...
void extract_kv_to_cache(
        ...
        cudaStream_t stream)
{
    const int block = std::min(HS, 256);
    const dim3 grid(Hkv, T, B);
    extract_kv_to_cache_kernel<<<grid, block, 0, stream>>>(
        cache, qkv, B, T, H, Hkv, HS, max_seq_len, cache_pos, head_start);
}
