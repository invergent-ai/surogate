...
// ---------------------------------------------------------------------------
// FP8 E4M3 KV variant: Q is BF16, KV pages are FP8, output is BF16.
// FlashInfer handles the dequant on-the-fly in registers.
// ---------------------------------------------------------------------------
template <int HeadDim>
void dispatch_flashinfer_paged_decode_fp8(
        nv_bfloat16* out,
        float* lse,
        const nv_bfloat16* q,
        const __nv_fp8_e4m3* k_pages,
        const __nv_fp8_e4m3* v_pages,
        const int32_t* page_indptr,
        const int32_t* page_indices,
        const int32_t* last_page_len,
        int32_t* request_indices,
    ...
    flashinfer::paged_kv_t<__nv_fp8_e4m3, int32_t> paged_kv(
        ...
        static_cast<uint32_t>(batch_size),
        flashinfer::QKVLayout::kNHD,
        const_cast<__nv_fp8_e4m3*>(k_pages),
        const_cast<__nv_fp8_e4m3*>(v_pages),
        const_cast<int32_t*>(page_indices),
        const_cast<int32_t*>(page_indptr),
        const_cast<int32_t*>(last_page_len));
    ...
    if (err != cudaSuccess) {
        throw std::runtime_error(
            std::string("attention_decode_flashinfer_paged_fp8 failed: ") + cudaGetErrorString(err));
    }
}
...
// ============================================================================
// FP8 E4M3 KV-cache variant: native FlashInfer FP8 decode (no BF16 dequant).
// ============================================================================
void attention_decode_flashinfer_paged_fp8(
    ...
    build_flashinfer_decode_metadata_kernel<<<1, 1, 0, stream>>>(
        seqused_k, batch_size, page_block_size,
        scratch_page_counts, scratch_indptr, scratch_last_page_len,
        scratch_request_indices, scratch_kv_tile_indices, scratch_kv_chunk_size);
    CUDA_CHECK(cudaGetLastError());
    ...
    gather_flashinfer_page_indices_kernel<<<grid, kThreads, 0, stream>>>(
        ...
        scratch_page_counts, scratch_indptr, scratch_indices, batch_size);
    CUDA_CHECK(cudaGetLastError());

    if (Hs == 64) {
        dispatch_flashinfer_paged_decode_fp8<64>(
            out, lse, q, k_pages, v_pages,
            scratch_indptr, scratch_indices, scratch_last_page_len,
            scratch_request_indices, scratch_kv_tile_indices, scratch_kv_chunk_size,
            batch_size, Hq, Hkv, page_block_size, q_stride_n, enable_pdl, stream);
