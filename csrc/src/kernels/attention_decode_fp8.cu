...
// ============================================================================
// Paged FP8 append/store in FlashInfer-compatible unit-scale format.
// ============================================================================

__global__ void kv_cache_store_paged_fp8_unit_scale_kernel(
    ...
    const float val = __bfloat162float(qkv_rope[src_base + src_head * Hs + dim_idx]);
    ...
    if (dim_idx == 0) {
        ...
        if (scales) {
            ...
            scales[scale_dst] = 1.0f;
        }
    }
}

__global__ void kv_cache_append_paged_fp8_unit_scale_kernel(
        __nv_fp8_e4m3* __restrict__ k_pages,
        __nv_fp8_e4m3* __restrict__ v_pages,
        float* __restrict__ k_scales,
        float* __restrict__ v_scales,
...
void kv_cache_append_paged_fp8(
    ...
    dim3 block(Hs);
    kv_cache_append_paged_fp8_unit_scale_kernel<<<grid, block, 0, stream>>>(
...
void kv_cache_store_paged_fp8(
    ...
    dim3 block(Hs);
    kv_cache_store_paged_fp8_unit_scale_kernel<<<grid, block, 0, stream>>>(
        k_pages_fp8, v_pages_fp8, k_scales, v_scales,
