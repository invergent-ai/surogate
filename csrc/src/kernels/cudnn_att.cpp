...
// ============================================================================
// Decode attention (KV-cache inference)
// ============================================================================

/// Cache type for decode attention graphs.
/// Key: (B, Hq, Hkv, T_kv, HS, max_seq_len)
/// max_seq_len is needed because it determines the batch stride of the K/V
/// tensors (layout is (B, max_seq_len, Hkv, HS) time-major in GPU memory).
using cache_type_decode = std::map<std::tuple<int,int,int,int,int,int>,
                                   std::shared_ptr<fe::graph::Graph>>;

/**
 * @brief Retrieves (or builds) a cuDNN SDPA graph for single-token decode.
 *
 * The decode graph takes Q (B, Hq, 1, HS), K (B, Hkv, T_kv, HS) and
 * V (B, Hkv, T_kv, HS) as *separate* tensors (not interleaved).  K and V
 * are slices of the KV-cache whose actual allocation has a larger time
 * dimension (max_seq_len), so the batch stride is max_seq_len * Hkv * HS.
 *
 * No causal mask: Q is a single new token that should attend to ALL T_kv
 * cached tokens (past + current).
 */
static auto lookup_cache_or_build_graph_decode(
        int B, int Hq, int Hkv, int T_kv, int HS, int max_seq_len,
        cudnnHandle_t handle)
{
    thread_local cache_type_decode cache;
    auto key = std::make_tuple(B, Hq, Hkv, T_kv, HS, max_seq_len);
    ...
    // Q: (B, Hq, 1, HS) — new single-token query (contiguous)
    auto Q = graph->tensor(fe::graph::Tensor_attributes()
                               .set_name("Q")
                               .set_dim({B, Hq, 1, HS})
                               .set_uid(Q_UID)
                               .set_stride({Hq * HS, HS, Hq * HS, 1}));

    // K: (B, Hkv, T_kv, HS) — slice of KV-cache with batch stride = max_seq_len * Hkv * HS
    // This correctly indexes the time-major layout (B, max_seq_len, Hkv, HS):
    //   K[b, h, t, hs] = base + b*(max_seq_len*Hkv*HS) + h*HS + t*(Hkv*HS) + hs
    auto K = graph->tensor(fe::graph::Tensor_attributes()
                               .set_name("K")
                               .set_dim({B, Hkv, T_kv, HS})
                               .set_uid(K_UID)
                               .set_stride({max_seq_len * Hkv * HS, HS, Hkv * HS, 1}));
    ...
    // No causal mask: the single query token must attend to all T_kv cached keys.
    auto sdpa_opts = fe::graph::SDPA_attributes()
                        .set_name("flash_decode")
...
/**
 * @brief Single-token decode attention using cuDNN SDPA (KV-cache inference).
 *
 * @param[out] out      Output (B, Hq, HS) in BF16 — one token per batch.
 * @param[in]  q        Query  (B, Hq, HS) in BF16 — the new token's projection.
 * @param[in]  k_cache  K-cache base (B, max_seq_len, Hkv, HS). Only the first
 *                      T_kv time-steps contain valid data; the rest are zero.
 * @param[in]  v_cache  V-cache base — same layout as k_cache.
 * @param      workspace  Pre-allocated GPU workspace.
 * @param      handle     cuDNN handle (will be bound to stream).
 * @param      B, T_kv, Hq, Hkv, HS  Dimensions.
 * @param      max_seq_len  Allocated time dimension of k_cache / v_cache.
...
{
    cuDNNCheck(cudnnSetStream(handle, stream));
    auto graph = lookup_cache_or_build_graph_decode(B, Hq, Hkv, T_kv, HS, max_seq_len, handle);

    float attn_scale = 1.0f / sqrtf(static_cast<float>(HS));
    std::unordered_map<int64_t, void*> variant_pack = {
        {Q_UID, const_cast<nv_bfloat16*>(q)},
        {K_UID, const_cast<nv_bfloat16*>(k_cache)},
        {V_UID, const_cast<nv_bfloat16*>(v_cache)},
        {Attn_scale_UID, &attn_scale},
        {O_UID, out},
    };
    checkCudnnFE(graph->execute(handle, variant_pack, workspace));
    CUDA_CHECK(cudaGetLastError());
