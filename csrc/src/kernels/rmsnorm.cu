...
__device__ void rmsnorm_forward_kernel(floatX* __restrict__ out,
...
__device__ void fused_residual_rmsnorm_forward_kernel(floatX* residual,
                                                      floatX* normed,
                                                      float* rrms,
                                                      const floatX* inp1,
                                                      const floatX* inp2,
 ...
 */
template <typename floatX>
__global__ void rmsnorm_forward_unified_kernel(floatX* residual,
                                               floatX* normed,
                                               float* rrms,
                                               const floatX* inp1,
                                               const floatX* inp2,
 ...
 */
template <typename floatX>
__device__ void rmsnorm_forward_quant_kernel(__nv_fp8_e4m3* __restrict__ out,
                                             float* __restrict__ scale_ptr,
                                             float* __restrict__ rms,
                                             const floatX* __restrict__ inp,
                                             const floatX* __restrict__ weight,
                                             const float* __restrict__ abs_max_ptr,
                                             float epsilon,
                                             int N,
                                             int C) {
    using x128 = GenericVector<floatX, 16 / sizeof(floatX)>;
    ...
    for (int c = threadIdx.x * x128::size; c < C; c += WARP_SIZE * x128::size) {
        ...
        for (int k = 0; k < x128::size; ++k) {
            ...
            __nv_fp8_e4m3 quant;
            quant.__x = __nv_cvt_float_to_fp8(scale * scaled,
 ...
 */
template <typename floatX>
__global__ void rmsnorm_forward_quant_unified_kernel(__nv_fp8_e4m3* out,
                                                     float* scale_ptr,
                                                     float* rms,
                                                     const floatX* inp,
                                                     const floatX* weight,
 ...
 */
template <typename floatX>
void rmsnorm_forward_unified_imp(floatX* residual,
                                 floatX* normed,
                                 float* rrms,
                                 const floatX* inp1,
                                 const floatX* inp2,
    ...
    auto status =
        cudaFuncSetAttribute(rmsnorm_forward_unified_kernel<floatX>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem);
    CUDA_CHECK(cudaGetLastError());
    if (status == cudaSuccess) {
        rmsnorm_forward_unified_kernel<<<grid_size, dim3(WARP_SIZE, block_y), smem, stream>>>(residual,
                                                                                              normed,
                                                                                              rrms,
                                                                                              inp1,
                                                                                              inp2,
...
void rmsnorm_forward(nv_bfloat16* out,
                     ...
                     int C,
                     cudaStream_t stream) {
    rmsnorm_forward_unified_imp<
        nv_bfloat16>(nullptr, out, rms, inp, nullptr, weight, abs_max_ptr, epsilon, B * T, C, stream);
}
...
template <typename floatX>
void rmsnorm_apply_saved_imp(floatX* out,
                             const floatX* inp,
                             const floatX* weight,
                             const float* rstd,
                             int N,
...
template <typename floatX>
void fused_residual_rmsnorm_apply_saved_imp(floatX* residual,
                                            floatX* normed,
                                            const floatX* inp1,
                                            const floatX* inp2,
                                            const floatX* weight,
    ...
    const int block_size = 256;
    const int grid_size = N;
    fused_residual_rmsnorm_apply_saved_kernel<<<grid_size, block_size, 0, stream>>>(residual,
                                                                                    normed,
                                                                                    inp1,
                                                                                    inp2,
                                                                                    weight,
 ...
 */
template <class floatX>
__global__ void __launch_bounds__(512, 2) rmsnorm_backward_kernel10(floatX* dinp,
                                                                    floatX* dweight,
                                                                    std::byte* scratch,
                                                                    const floatX* dout,
                                                                    const floatX* inp,
 ...
 */
template <class floatX>
void rmsnorm_backward_imp(floatX* dinp,
                          floatX* dweight,
                          std::byte* scratch,
                          const floatX* dresidual,
                          const floatX* dout,
                          const floatX* inp,
                          const floatX* weight,
                          const float* rstd,
    ...
    CUDA_CHECK(cudaFuncSetAttribute(rmsnorm_backward_kernel10<floatX>,
    ...
    rmsnorm_backward_kernel10<<<grid_size, block_size, shared_mem_size, stream>>>(dinp,
                                                                                  dweight,
                                                                                  scratch,
                                                                                  dout,
                                                                                  inp,
...
template <typename T>
__global__ void
rmsnorm_backward_dweight_fp32_kernel(const T* dout, const T* inp, const float* rstd, float* dweight, int BT, int C) {
...
void rmsnorm_backward_dweight_fp32(Tensor& dweight_fp32,
    ...
    if (inp.DType == ETensorDType::BF16) {
        rmsnorm_backward_dweight_fp32_kernel<<<blocks, threads, 0, stream>>>(dout.get<nv_bfloat16>(),
                                                                             inp.get<nv_bfloat16>(),
                                                                             rstd.get<float>(),
        ...
        rmsnorm_backward_dweight_fp32_kernel<<<blocks, threads, 0, stream>>>(dout.get<half>(),
                                                                             inp.get<half>(),
                                                                             rstd.get<float>(),
                                                                             dweight_fp32.get<float>(),
                                                                             BT,
...
void rmsnorm_forward_quant(__nv_fp8_e4m3* out,
    ...
    rmsnorm_forward_quant_unified_kernel<<<grid_size, dim3(WARP_SIZE, warps_per_block), smem, stream>>>(out,
                                                                                                        scale_ptr,
                                                                                                        rms,
                                                                                                        inp,
                                                                                                        weight,
...
void rmsnorm_forward_quant(Tensor& out,
    ...
    rmsnorm_forward_quant(out.get<__nv_fp8_e4m3>(),
                          scale_ptr,
                          rms.get<float>(),
                          inp.get<nv_bfloat16>(),
