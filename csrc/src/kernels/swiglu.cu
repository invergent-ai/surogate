...
template <typename floatX>
__global__ void
swiglu_forward_quant_kernel(__nv_fp8_e4m3* out, float* scale_ptr, const floatX* inp, const float* abs_max_ptr, int C) {
    using x128 = GenericVector<floatX, 16 / sizeof(floatX)>;
    ...
    for (int k = 0; k < up_inp.size; ++k) {
        ...
        __nv_fp8_e4m3 quant;
        quant.__x = __nv_cvt_float_to_fp8(scale * (float)qr,
...
__global__ __launch_bounds__(
    128) void swiglu_forward_persistent_kernel(floatX* out, const floatX* inp, float* abs_max_ptr, int BT, int C) {
    using x128 = GenericVector<floatX, 16 / sizeof(floatX)>;
 ...
 */
template <typename floatX>
__global__ void swiglu_forward_quant_persistent_kernel(__nv_fp8_e4m3* out,
                                                       float* scale_ptr,
                                                       const floatX* inp,
...
template <typename floatX>
__global__ void
swiglu_backward_kernel1(floatX* dinp, const floatX* dout, const floatX* inp, float* abs_max_ptr, int B, int T, int C) {
    using x128 = GenericVector<floatX, 16 / sizeof(floatX)>;
...
void swiglu_forward_impl(floatX* out, const floatX* inp, float* abs_max_ptr, int B, int T, int C, cudaStream_t stream) {
    ...
    int bpsm;
    if (abs_max_ptr) {
        CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&bpsm,
                                                                 swiglu_forward_persistent_kernel<true, floatX>,
                                                                 block_size,
                                                                 0));
    } else {
        CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&bpsm,
                                                                 swiglu_forward_persistent_kernel<false, floatX>,
                                                                 block_size,
                                                                 0));
    }
    int sms;
    ...
    if (num_blocks < bpsm * sms) {
        ...
        if (abs_max_ptr) {
            swiglu_forward_persistent_kernel<true>
                <<<bpsm * sms, block_size, 0, stream>>>(out, inp, abs_max_ptr, B * T, C);
        } else {
...
void swiglu_forward_quant(__nv_fp8_e4m3* out,
    ...
    int bpsm;
    CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&bpsm,
                                                             swiglu_forward_quant_persistent_kernel<nv_bfloat16>,
                                                             block_size,
                                                             0));
    int sms;
    CUDA_CHECK(cudaDeviceGetAttribute(&sms, cudaDevAttrMultiProcessorCount, 0));
    ...
    if (num_blocks < bpsm * sms) {
        ...
        swiglu_forward_quant_persistent_kernel<<<bpsm * sms, block_size, 0, stream>>>(out,
                                                                                      scale_ptr,
                                                                                      inp,
                                                                                      abs_max_ptr,
                                                                                      B * T,
 ...
 */
template <typename floatX>
void swiglu_backward_impl(floatX* dinp,
                          const floatX* dout,
                          const floatX* inp,
                          float* abs_max,
                          int B,
    ...
    // Zero-element case: MoE+EP can produce 0 routed tokens for some experts.
    if (B * T * C == 0) {
        if (abs_max) CUDA_CHECK(cudaMemsetAsync(abs_max, 0, sizeof(float), stream));
        return;
    }
