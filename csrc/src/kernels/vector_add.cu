 ...
 */
template <typename T>
__global__ void vector_reduce_sr_kernel(T* dest,
                                        const T* src,
                                        float scale,
                                        int n_shards,
                                        int skip,
...
void vector_add_sr_imp(T* dest,
    ...
    if (grid_size <= 0) {
        ...
        snprintf(buf,
                 sizeof(buf),
                 "vector_add_sr_imp: grid_size=%ld is zero or negative (nelem=%ld, vec_size=%ld, block_size=%ld)",
                 grid_size,
                 nelem,
                 vec_size,
                 block_size);
 ...
 */
template <typename T>
void vector_reduce_sr_imp(T* dest,
                          const T* src,
                          float scale,
                          int n_shards,
                          int skip,
    ...
    vector_reduce_sr_kernel<T><<<(unsigned int)grid_size, (unsigned int)block_size, 0, stream>>>(dest,
                                                                                                 src,
                                                                                                 scale,
                                                                                                 n_shards,
                                                                                                 skip,
 ...
 */
__global__ void
fused_bf16_accum_to_fp32_strided_kernel(float* out, const nv_bfloat16* src, int rows, int src_cols, int out_stride) {
    int row = blockIdx.y;
