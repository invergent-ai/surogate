...
void BnBWeightsManager::load_embeddings(SafeTensorsReader& reader, cudaStream_t stream) {
    ...
    // Debug allocator usage before embedding allocation to pinpoint OOM sources.
    static int emb_alloc_debug = 0;
    auto log_allocator_summary = [&]() {
        if (!mAllocator) {
            return;
        }
        const auto to_mib = [](std::size_t bytes) { return static_cast<long>(bytes / (1024 * 1024)); };
        std::cerr << "[BnB EMBED ALLOC] total=" << to_mib(mAllocator->total_allocation())
                  << " MiB device=" << to_mib(mAllocator->total_allocation(EAllocationType::ON_DEVICE))
                  << " MiB managed=" << to_mib(mAllocator->total_allocation(EAllocationType::MANAGED))
                  << " MiB pinned=" << to_mib(mAllocator->total_allocation(EAllocationType::PINNED))
                  << " MiB host=" << to_mib(mAllocator->total_allocation(EAllocationType::ON_HOST))
                  << "\n";

        auto segments = mAllocator->get_allocation_segments();
        std::sort(segments.begin(), segments.end(),
                  [](const auto& a, const auto& b) {
                      return a.second.OnDevice > b.second.OnDevice;
                  });
        for (const auto& [name, seg] : segments) {
            if (seg.OnDevice == 0 && seg.Managed == 0 && seg.PinnedHost == 0 && seg.PageableHost == 0) {
                continue;
            }
            std::cerr << "[BnB EMBED ALLOC] segment=" << name
                      << " device=" << to_mib(seg.OnDevice)
                      << " MiB managed=" << to_mib(seg.Managed)
                      << " MiB pinned=" << to_mib(seg.PinnedHost)
                      << " MiB host=" << to_mib(seg.PageableHost)
                      << " MiB\n";
        }
        mAllocator->print_stats();
    };

    if (emb_alloc_debug < 1) {
        emb_alloc_debug++;
    ...
    try {
        ...
        std::cerr << "[BnB EMBED ALLOC] allocation failed for embedding [" << vocab << ", " << hidden << "]\n";
        log_allocator_summary();
        throw;
    }
...
void BnBWeightsManager::load_and_quantize_moe_block(int layer_idx, SafeTensorsReader& reader,
    ...
    const int shared_inter = (mConfig.qlora_config.moe_shared_expert_intermediate_size > 0)
        ...
        : moe_inter;
    const int shared_M = mConfig.mlp_up_factor * shared_inter;
    static int moe_row_debug_enabled = -1;
    if (moe_row_debug_enabled < 0) {
        moe_row_debug_enabled = (std::getenv("SUROGATE_BNB_MOE_ROW_DEBUG") != nullptr) ? 1 : 0;
    }
    static bool moe_row_debug_logged = false;
    const int debug_layer = 1;
    const int debug_expert = 68;
    const int debug_row = 711;
    ...
    for (int e = 0; e < n_experts; ++e) {
        ...
        // DEBUG: Log whether expert weights exist for a few experts in layer 0/top.
        if ((layer_idx == 0 || layer_idx == mConfig.num_layers - 1) && e < 4) {
            const bool has_gate_up = find_entry_opt(reader, exp_prefix + ".gate_up_proj.weight");
            const bool has_up = find_entry_opt(reader, exp_prefix + ".up_proj.weight");
            const bool has_gate = find_entry_opt(reader, exp_prefix + ".gate_proj.weight");
            const bool has_down = find_entry_opt(reader, exp_prefix + ".down_proj.weight");
            std::cerr << "[BnB EXPERT WEIGHTS] layer=" << layer_idx << " expert=" << e
                      << " prefix=" << exp_prefix
                      << " gate_up=" << has_gate_up
                      << " up=" << has_up
                      << " gate=" << has_gate
        ...
        const bool debug_target = (moe_row_debug_enabled == 1 && !moe_row_debug_logged &&
                                   layer_idx == debug_layer && e == debug_expert);
        ...
        if (find_entry_opt(reader, gate_up_name)) {
            ...
            if (const auto* entry = find_entry_opt(reader, gate_up_name)) {
                ...
                if (debug_target) {
                    CUDA_CHECK(cudaStreamSynchronize(stream));
                    const long row_width = mLoadBuffer.Sizes[1];
                    if (debug_row >= 0 && debug_row < mLoadBuffer.Sizes[0]) {
                        std::vector<nv_bfloat16> row_vals;
                        if (copy_tensor_range_host<nv_bfloat16>(mLoadBuffer,
                                                                static_cast<long>(debug_row) * row_width,
                                                                row_width, row_vals)) {
                            float max_abs = 0.0f;
                            float max_val = 0.0f;
                            std::size_t max_idx = 0;
                            const int block_size = mScaleConfig.block_size;
                            const int num_blocks = static_cast<int>((row_width + block_size - 1) / block_size);
                            float blk_min = std::numeric_limits<float>::infinity();
                            float blk_max = -std::numeric_limits<float>::infinity();
                            float blk_mean = 0.0f;
                            for (std::size_t i = 0; i < row_vals.size(); ++i) {
                                const float v = static_cast<float>(row_vals[i]);
                            ...
                            if (num_blocks > 0) {
                                for (int b = 0; b < num_blocks; ++b) {
                                    ...
                                    const int end = std::min<int>(start + block_size, static_cast<int>(row_vals.size()));
                                    float bmax = 0.0f;
                                    for (int i = start; i < end; ++i) {
                                        bmax = std::max(bmax, std::fabs(static_cast<float>(row_vals[static_cast<std::size_t>(i)])));
                                    }
                                    blk_min = std::min(blk_min, bmax);
                                    blk_max = std::max(blk_max, bmax);
                                    blk_mean += bmax;
                                }
                                blk_mean /= static_cast<float>(num_blocks);
                            }
                            fprintf(stderr,
                                    "[BNB_MOE_PREQ_ROW] layer=%d expert=%d row=%d max_abs=%.6f max_idx=%zu max_val=%.6f\n",
                                    layer_idx, e, debug_row, max_abs, max_idx, max_val);
                            fprintf(stderr,
                                    "[BNB_MOE_PREQ_ABSMAX] layer=%d expert=%d row=%d blocks=%d min=%.6f max=%.6f mean=%.6f\n",
                                    layer_idx, e, debug_row, num_blocks, blk_min, blk_max, blk_mean);
                            const long row_start = static_cast<long>(debug_row) * row_width;
                            const long start_block = row_start / block_size;
                            const long flat_offset = start_block * block_size;
                            const long flat_count = static_cast<long>(num_blocks) * block_size;
                            if (flat_offset + flat_count <= mLoadBuffer.Sizes[0] * mLoadBuffer.Sizes[1]) {
                                std::vector<nv_bfloat16> flat_vals;
                                if (copy_tensor_range_host<nv_bfloat16>(mLoadBuffer, flat_offset, flat_count, flat_vals)) {
                                    float flat_min = std::numeric_limits<float>::infinity();
                                    float flat_max = -std::numeric_limits<float>::infinity();
                                    float flat_mean = 0.0f;
                                    for (int b = 0; b < num_blocks; ++b) {
                                        const int start = b * block_size;
                                        const int end = std::min<int>(start + block_size, static_cast<int>(flat_vals.size()));
                                        float bmax = 0.0f;
                                        for (int i = start; i < end; ++i) {
                                            bmax = std::max(bmax, std::fabs(static_cast<float>(flat_vals[static_cast<std::size_t>(i)])));
                                        }
                                        flat_min = std::min(flat_min, bmax);
                                        flat_max = std::max(flat_max, bmax);
                                        flat_mean += bmax;
                                    }
                                    flat_mean /= static_cast<float>(num_blocks);
                                    fprintf(stderr,
                                            "[BNB_MOE_PREQ_ABSMAX_FLAT] layer=%d expert=%d row=%d blocks=%d min=%.6f max=%.6f mean=%.6f\n",
                                            layer_idx, e, debug_row, num_blocks, flat_min, flat_max, flat_mean);
                                }
                            }
                        }
                    }
                ...
                if (debug_target) {
                    CUDA_CHECK(cudaStreamSynchronize(stream));
                    const auto& weight = expert.gate_up_proj;
                    const long K = weight.K;
                    const long start_elem = static_cast<long>(debug_row) * K;
                    ...
                    if (num_blocks > 0) {
                        if (weight.double_quant) {
                            ...
                            const long group_start = start_block / group_size;
                            const long group_offset = group_start * group_size;
                            const long group_count = std::min<long>(group_size,
                                                                    (weight.num_blocks() - group_offset));
                            std::vector<float> group_vals;
                            if (copy_tensor_range_host<float>(mAbsmaxBuffer, group_offset, group_count, group_vals)) {
                                float gmin = std::numeric_limits<float>::infinity();
                                float gmax = -std::numeric_limits<float>::infinity();
                                float gmean = 0.0f;
                                for (float v : group_vals) {
                                    gmin = std::min(gmin, v);
                                    gmax = std::max(gmax, v);
                                    gmean += v;
                                }
                                gmean /= static_cast<float>(group_vals.size());
                                fprintf(stderr,
                                        "[BNB_MOE_PREQ_GROUP_ABSMAX] layer=%d expert=%d row=%d group=%ld count=%ld min=%.6f max=%.6f mean=%.6f\n",
                                        layer_idx, e, debug_row, group_start, group_count, gmin, gmax, gmean);
                            }
                        }
                        ...
                        if (weight.double_quant) {
                            if (copy_tensor_range_host<unsigned char>(weight.absmax, start_block, num_blocks, absmax_q)) {
                                ...
                                if (copy_tensor_range_host<float>(weight.absmax_scale, group_start, group_count, scales) &&
                                    copy_tensor_range_host<float>(weight.absmax_offset, group_start, group_count, offsets)) {
                                    if (!scales.empty() && !offsets.empty()) {
                                        fprintf(stderr,
                                                "[BNB_MOE_POSTQ_GROUP_SCALE] layer=%d expert=%d row=%d group=%ld scale=%.6e offset=%.6e\n",
                                                layer_idx, e, debug_row, group_start, scales[0], offsets[0]);
                                    }
                                    if (!absmax_q.empty()) {
                                        unsigned char qmin = 255;
                                        unsigned char qmax = 0;
                                        for (unsigned char qv : absmax_q) {
                                            qmin = std::min(qmin, qv);
                                            qmax = std::max(qmax, qv);
                                        }
                                        fprintf(stderr,
                                                "[BNB_MOE_POSTQ_QRANGE] layer=%d expert=%d row=%d qmin=%u qmax=%u\n",
                                                layer_idx, e, debug_row,
                                                static_cast<unsigned int>(qmin),
                                                static_cast<unsigned int>(qmax));
                                    }
                                    ...
                                    for (long i = 0; i < num_blocks; ++i) {
                                        ...
                                        absmax_vals[static_cast<std::size_t>(i)] =
                                            (static_cast<float>(absmax_q[static_cast<std::size_t>(i)]) - 128.0f) *
                                            scales[static_cast<std::size_t>(group_idx)] +
                                            offsets[static_cast<std::size_t>(group_idx)];
                                    }
                                }
                            }
                        } else {
                            copy_tensor_range_host<float>(weight.absmax, start_block, num_blocks, absmax_vals);
                        }
                        if (!absmax_vals.empty()) {
                            float min_v = std::numeric_limits<float>::infinity();
                            float max_v = -std::numeric_limits<float>::infinity();
                            float mean = 0.0f;
                            for (float v : absmax_vals) {
                                if (std::isnan(v) || std::isinf(v)) {
                                    continue;
                                }
                            ...
                            mean /= static_cast<float>(absmax_vals.size());
                            fprintf(stderr,
                                    "[BNB_MOE_POSTQ_ABSMAX] layer=%d expert=%d row=%d blocks=[%ld..%ld] min=%.6f max=%.6f mean=%.6f absmax_dev=%d\n",
                                    layer_idx, e, debug_row, start_block, end_block, min_v, max_v, mean, weight.absmax.Device);
                        }
                    }
                    moe_row_debug_logged = true;
                }
            }
        } else if (mConfig.mlp_up_factor == 1) {
