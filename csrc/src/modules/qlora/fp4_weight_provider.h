        ...
        if (qkv_trace_enabled &&
            ...
            if (cudaStreamIsCapturing(stream, &capture_status) != cudaSuccess ||
                ...
                if (qkv_trace_count == 0) {
                    std::cerr << "[QLORA_QKV_DEQUANT_FP4] stream capture active; skipping trace\n";
                }
            } else {
                CUDA_CHECK(cudaStreamSynchronize(stream));
                ...
                if (sample > 0 && copy_tensor_range_host<nv_bfloat16>(mDequantQKV, 0, sample, vals)) {
                    ...
                    std::cerr << "[QLORA_QKV_DEQUANT_FP4] layer=" << layer_idx
                              ...
                              << " max=" << max_v
                              << " mean_abs=" << mean_abs
                              << " scale=" << fp4_block.qkv_proj.global_decode_scale_rowwise()
                              << "\n";
                }
            }
            qkv_trace_count++;
