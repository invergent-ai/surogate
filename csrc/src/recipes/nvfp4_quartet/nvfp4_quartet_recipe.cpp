...
void NVFP4QuartetRecipe::forward_matmul(modules::MatmulContext& ctx) const {
    ...
    compute_fp4_alpha(alpha.get<float>(), inp_global_amax.get<float>(), weight_amax_ptr, ctx.stream);
...
void NVFP4QuartetRecipe::backward_matmul(modules::MatmulContext& ctx) const {
    ...
    Tensor hadamard = rs.temp_alloc(ETensorDType::BF16, {quartet::HADAMARD_DIM, quartet::HADAMARD_DIM}, "hadamard");
    ...
    // =========================================================================
    {
        ...
        Tensor dout_scales_linear =
            rs.temp_alloc(ETensorDType::BYTE, {static_cast<long>(dout_scale_elems)}, "dout_scales_linear");
        Tensor dout_scales_cutlass =
            rs.temp_alloc(ETensorDType::BYTE, {static_cast<long>(dout_scale_size)}, "dout_scales_cutlass");
        Tensor dout_amax = rs.temp_alloc(ETensorDType::FP32, {1}, "dout_amax");

        // EDEN scratch buffers
        Tensor dout_scratch_scales =
            rs.temp_alloc(ETensorDType::BF16, {static_cast<long>(dout_scale_elems)}, "dout_scratch_scales");
        ...
        quartet::group_transform_128_eden(reinterpret_cast<__nv_fp4x2_storage_t*>(dout_fp4.get<uint8_t>()),
        ...
        nvfp4_scales_linear_to_cutlass(dout_scales_cutlass.get<uint8_t>(),
        ...
        if (use_cached_wt) {
            ...
            w_scratch_scales =
                rs.temp_alloc(ETensorDType::BF16, {static_cast<long>(w_scale_elems)}, "w_scratch_scales");
            ...
            Tensor cached_w_scales_linear =
                rs.temp_alloc(ETensorDType::BYTE, {static_cast<long>(cached_w_scale_elems)}, "cached_w_scales_linear");
            ...
            compute_fp4_tensor_scale(cached_w_tensor_scale.get<float>(),
                                     ctx.cached_fp4_amax,
                                     quartet::FP4_MAX,
                                     /*fp8_max=*/448.0f,
                                     ctx.stream);
            ...
            nvfp4_scales_cutlass_to_linear(cached_w_scales_linear.get<uint8_t>(),
            ...
            quartet::dequant_tp_had_quant(
                ...
                fp8_max_eden,
                OC,
                C,  // Original W is (OC, C), transposed is (C, OC)
                ctx.stream);

            // Convert requantized scales to CUTLASS layout for GEMM.
            nvfp4_scales_linear_to_cutlass(w_scales.get<uint8_t>(), w_scales_linear.get<uint8_t>(), C, OC, ctx.stream);
            ...
            quartet::group_transform_128_eden(reinterpret_cast<__nv_fp4x2_storage_t*>(w_fp4.get<uint8_t>()),
                                              reinterpret_cast<__nv_fp8_e4m3*>(w_scales_linear.get<uint8_t>()),
                                              w_amax.get<float>(),
                                              w_scratch_scales.get<nv_bfloat16>(),
                                              reinterpret_cast<unsigned*>(w_max_scale.Data),
        ...
        Tensor alpha = rs.temp_alloc(ETensorDType::FP32, {1}, "alpha");
        compute_fp4_alpha_from_tensor_scale(alpha.get<float>(), dout_amax.get<float>(), w_amax_ptr, ctx.stream);
    ...
    if (!ctx.skip_weight_grad && ctx.dweight) {
        ...
        Tensor a_scales_linear =
            rs.temp_alloc(ETensorDType::BYTE, {static_cast<long>(a_scale_elems)}, "a_scales_linear");
        Tensor a_scales_cutlass =
            rs.temp_alloc(ETensorDType::BYTE, {static_cast<long>(a_scale_size)}, "a_scales_cutlass");
        ...
        quartet::group_transform_128_eden(reinterpret_cast<__nv_fp4x2_storage_t*>(a_fp4.get<uint8_t>()),
        ...
        nvfp4_scales_linear_to_cutlass(a_scales_cutlass.get<uint8_t>(),
        ...
        Tensor b_scales_linear =
            rs.temp_alloc(ETensorDType::BYTE, {static_cast<long>(b_scale_elems)}, "b_scales_linear");
        Tensor b_scales_cutlass =
            rs.temp_alloc(ETensorDType::BYTE, {static_cast<long>(b_scale_size)}, "b_scales_cutlass");
        ...
        quartet::group_transform_128_eden(reinterpret_cast<__nv_fp4x2_storage_t*>(b_fp4.get<uint8_t>()),
        ...
        nvfp4_scales_linear_to_cutlass(b_scales_cutlass.get<uint8_t>(),
        ...
        // EDEN kernels produce tensor_scale (amax / (fp4_max * fp8_max)), so alpha is just the product.
        Tensor alpha = rs.temp_alloc(ETensorDType::FP32, {1}, "alpha");
        compute_fp4_alpha_from_tensor_scale(alpha.get<float>(), a_amax.get<float>(), b_amax.get<float>(), ctx.stream);
