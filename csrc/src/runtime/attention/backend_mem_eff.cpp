...
//
// Forward/backward support MHA, stride-zero MQA and explicitly broadcast GQA.
...
// Materialize true-GQA K/V into [B*T,Hq,D] scratch. No device allocation
// occurs here: the caller supplies its bounded, capture-safe arena.
std::pair<const void*, const void*> broadcast_grouped_kv(
    AttentionParams& p, const MemEffScratchAllocator& allocate) {
    const long tokens = static_cast<long>(p.B) * p.T;
    if (p.Hq <= 0 || p.Hkv <= 0 || p.Hq % p.Hkv != 0 ||
        p.total_doc_tokens < 0 || p.total_doc_tokens > tokens)
        throw std::invalid_argument("mem_eff: invalid GQA geometry or token count");
    const int group_size = p.Hq / p.Hkv;
    const long source_stride = (p.Hq + 2 * p.Hkv) * p.Hs;
    const long expanded_stride = p.Hq * p.Hs;
    const std::size_t bytes = tokens * expanded_stride * sizeof(nv_bfloat16);
    auto* k = reinterpret_cast<nv_bfloat16*>(allocate(bytes));
    auto* v = reinterpret_cast<nv_bfloat16*>(allocate(bytes));
    CUDA_CHECK(cudaMemsetAsync(k, 0, bytes, p.stream));
    CUDA_CHECK(cudaMemsetAsync(v, 0, bytes, p.stream));
    auto* input = p.qkv.get<nv_bfloat16>();
    for (int q = 0; q < p.Hq; ++q) {
        const int kv = q / group_size;
        for (int value = 0; value < 2; ++value) {
            auto* dest = (value ? v : k) + q * p.Hs;
            const auto* src = input + (p.Hq + value * p.Hkv + kv) * p.Hs;
            CUDA_CHECK(cudaMemcpy2DAsync(dest, expanded_stride * sizeof(nv_bfloat16),
                src, source_stride * sizeof(nv_bfloat16), p.Hs * sizeof(nv_bfloat16),
                p.total_doc_tokens, cudaMemcpyDeviceToDevice, p.stream));
        }
    }
    return {k, v};
}
...
void mem_eff_forward_with_scratch(AttentionParams& p, const MemEffScratchAllocator& allocate) {
    ...
    const bool is_gqa = Hkv > 1 && Hkv < Hq;
    if (is_gqa) {
        auto broadcast = broadcast_grouped_kv(p, allocate);
        k_ptr = broadcast.first;
        v_ptr = broadcast.second;
    }
    // Document tails are not written by the packed kernel. Keep them deterministic.
    CUDA_CHECK(cudaMemsetAsync(p.out.Data, 0, p.out.bytes(), p.stream));
    if (p.lse.Data) CUDA_CHECK(cudaMemsetAsync(p.lse.Data, 0, p.lse.bytes(), p.stream));
    ...
    float* accum_ptr = reinterpret_cast<float*>(allocate(accum_elems * sizeof(float)));
    ...
    float* lse_kernel_ptr = reinterpret_cast<float*>(allocate(lse_kernel_elems * sizeof(float)));
    ...
    args.k_strideM = is_gqa ? Hq * Hs : q_strideM;
    ...
    args.k_strideB = is_gqa ? static_cast<int64_t>(T) * Hq * Hs : q_strideB;
    args.v_strideM = is_gqa ? Hq * Hs : q_strideM;
    args.v_strideH = (Hkv == 1 && Hq > 1) ? 0 : q_strideH;
    args.v_strideB = is_gqa ? static_cast<int64_t>(T) * Hq * Hs : q_strideB;
...
void mem_eff_backward_with_scratch(AttentionParams& p, const MemEffScratchAllocator& allocate) {
    ...
    const int lse_dim = (max_seqlen + kMemEffLseAlignment - 1) / kMemEffLseAlignment * kMemEffLseAlignment;
    const long lse_kernel_elems = static_cast<long>(num_batches) * Hq * lse_dim;
    float* lse_ptr = reinterpret_cast<float*>(allocate(lse_kernel_elems * sizeof(float)));
    ...
    float* delta_ptr = reinterpret_cast<float*>(allocate(delta_flat_elems * sizeof(float)));
    ...
    float* delta_dense_ptr =
        reinterpret_cast<float*>(allocate(delta_dense_elems * sizeof(float)));
    ...
    const bool is_gqa = Hkv > 1 && Hkv < Hq;
    const bool grouped = Hkv < Hq;
    CUDA_CHECK(cudaMemsetAsync(p.d_qkv.Data, 0, p.d_qkv.bytes(), p.stream));
    ...
    if (grouped) {
        d_k_target =
            reinterpret_cast<nv_bfloat16*>(allocate(partial_elems * sizeof(nv_bfloat16)));
        d_v_target =
            reinterpret_cast<nv_bfloat16*>(allocate(partial_elems * sizeof(nv_bfloat16)));
    ...
    if (is_gqa) {
        auto broadcast = broadcast_grouped_kv(p, allocate);
        args.k_ptr = broadcast.first;
        args.v_ptr = broadcast.second;
    }
    args.out_ptr = out_ptr;
    ...
    if (grouped) {
        ...
        args.d_q_ptr = allocate(partial_elems * sizeof(nv_bfloat16));
        CUDA_CHECK(cudaMemsetAsync(args.d_q_ptr, 0, partial_elems * sizeof(nv_bfloat16), p.stream));
    ...
    if (grouped) {
        ...
        // Sum within each KV group, never across unrelated KV heads. The
        // existing MQA reduction accepts explicit token/head strides, so each
        // group is an independent reduction over Hq/Hkv contiguous query heads.
        const int group_size = Hq / Hkv;
        for (int kv = 0; kv < Hkv; ++kv) {
            const long partial_offset = static_cast<long>(kv) * group_size * Hs;
            surogate::mem_eff::mqa_reduce_kv_bf16(
                dk_partial + partial_offset, dv_partial + partial_offset,
                d_qkv_ptr + (Hq + kv) * Hs,
                d_qkv_ptr + (Hq + Hkv + kv) * Hs,
                static_cast<int>(total_tokens), group_size, Hs,
                /*partial_strideM=*/Hq * Hs, /*partial_strideH=*/Hs,
                /*out_strideM=*/HtotQKV * Hs, p.stream);
        }
    }
}
...
public:
    ...
    bool supports(const AttentionParams& p) const override {
        ...
        if (p.Hq <= 0 || p.Hkv <= 0 || p.Hq % p.Hkv != 0)
            return reject("invalid grouped-query head geometry");
    ...
    void forward(AttentionParams& p) override {
        ...
        if (!exec) throw std::runtime_error("mem_eff forward: no active executor");
        synthesize_dense_cu_seqlens_if_needed(p, rs);
        exec->mem_eff_scratch_reset();
        mem_eff_forward_with_scratch(p, [exec](std::size_t bytes) {
            return exec->mem_eff_scratch_alloc(bytes);
        });
    }
    ...
    void backward(AttentionParams& p) override {
        DslRunState& rs = *p.run_state;
        CompiledExecutor* exec = rs.active_executor();
        if (!exec) throw std::runtime_error("mem_eff backward: no active executor");
        synthesize_dense_cu_seqlens_if_needed(p, rs);
        exec->mem_eff_scratch_reset();
        mem_eff_backward_with_scratch(p, [exec](std::size_t bytes) {
            return exec->mem_eff_scratch_alloc(bytes);
        });
    }
};
