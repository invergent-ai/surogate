...
#include "runtime/attention/mem_eff/aten_compat.h"
...
#include <runtime/attention/mem_eff/debug_utils.h>
#include <runtime/attention/mem_eff/gemm_kernel_utils.h>

#include <runtime/attention/mem_eff/gemm/custom_mma.h>
#include <runtime/attention/mem_eff/gemm/find_default_mma.h>
#include <runtime/attention/mem_eff/gemm/mma_accum_lambda_iterator.h>
#include <runtime/attention/mem_eff/gemm/mma_from_smem.h>

#include <runtime/attention/mem_eff/epilogue/epilogue_pipelined.h>
#include <runtime/attention/mem_eff/iterators/epilogue_predicated_tile_iterator.h>
#include <runtime/attention/mem_eff/transform/tile_smem_loader.h>

#include <cinttypes>
// #include <c10/util/Exception.h> - provided by aten_compat.h
  ...
  */
    ...
    CUTLASS_DEVICE void load(FragmentType& fragment, int thread_id) {
        ...
        for (int i = 0; i < kNumIters; ++i) {
            AccessType* __restrict__ gmem_ptr =
                reinterpret_cast<AccessType*>(ptr + thread_id * AccessType::kElements + i * kStride);
    ...
    CUTLASS_DEVICE void storeAtomicAdd(FragmentType const& fragment, int thread_id) {
        CUTLASS_PRAGMA_UNROLL
        for (int i = 0; i < kNumIters; ++i) {
...
struct AtomicLock {
    CUTLASS_DEVICE static void acquire(int32_t* lock, int set_val, int thread_id) {
        if (thread_id == 0) {
...
struct AttentionBackwardKernel {
    ...
    static constexpr bool kKeysQueriesAlignedToBlockSize = kKeysQueriesAlignedToBlockSize_;
    ...
    static constexpr bool kKernelComputesDelta = kIsHalf && (kOutputInRF || ArchTag::kMinComputeCapability != 70);

    // Launch bounds
    static constexpr int64_t kNumThreads = kWarpSize * kNumWarpsPerBlock;
    static constexpr int64_t kMinBlocksPerSm = getWarpsPerSmBw<scalar_t, ArchTag>() / kNumWarpsPerBlock;

    using GemmType = DefaultGemmType<ArchTag, scalar_t>;
    using DefaultConfig = typename cutlass::gemm::device::DefaultGemmConfiguration<typename GemmType::OpClass,
                                                                                   ArchTag,
                                                                                   scalar_t,
                                                                                   scalar_t,
                                                                                   scalar_t,  // ElementC
    ...
    */
        using ThreadblockShape = cutlass::gemm::GemmShape<kBlockSizeJ, kBlockSizeI, GemmType::ThreadK>;
        ...
        // the second matmul
        using B2bGemm = typename cutlass::gemm::threadblock::
            B2bGemm<typename Mma::Operator::IteratorC, typename Mma::Operator, scalar_t, WarpShape, ThreadblockShape>;
        using AccumLambdaIterator =
            typename DefaultMmaAccumLambdaIterator<typename Mma::Operator::IteratorC, accum_t, kWarpSize>::Iterator;
    ...
    */
        ...
        using OutputTileIterator = typename cutlass::epilogue::threadblock::MakePrefetchableIterator<
    ...
    */
        using ThreadblockShape = cutlass::gemm::GemmShape<kBlockSizeI, kBlockSizeJ, GemmType::ThreadK>;
        using WarpShape = cutlass::gemm::GemmShape<32, 32, GemmType::WarpK>;
        ...
        using AccumLambdaIterator = typename DefaultMmaAccumLambdaIterator<typename Mma::Operator::IteratorC,
                                                                           ElementAccum,
                                                                           kWarpSize>::Iterator;
        ...
        // Epilogue to store to shared-memory in a format that we can use later for
        // the second matmul
        using B2bGemm = typename cutlass::gemm::threadblock::B2bGemm<typename DefaultGemm::Mma::Operator::IteratorC,
    ...
    struct MatmulGradK {
        ...
        using DefaultMmaFromSmem = typename cutlass::platform::
            conditional<DefaultMmaFromSmemT::kIsTransposedA, DefaultMmaFromSmemT, DefaultMmaFromSmemN>::type;
        using Mma = typename DefaultMmaFromSmem::Mma;
        using IteratorB = typename Mma::IteratorB;
        using WarpCount = typename Mma::WarpCount;
    ...
    static constexpr bool kNeedsAccumGradQ =
        kEnableSplitKeys || !cutlass::platform::is_same<output_accum_t, output_t>::value;
    ...
    struct Params {
        ...
        output_accum_t* workspace_gv = nullptr;    // (will be calculated by the kernel)
        GradQTempStorage* workspace_gq = nullptr;  // (will be calculated by the kernel)
        ...
        CUTLASS_DEVICE bool advance_to_block() {
            ...
            if (kNeedsAccumGradQ || kNeedsAccumGradK || kNeedsAccumGradV) {
                ...
                workspace_gq = (GradQTempStorage*)(workspace_gv + workspace_elements_gv());
                if (kEnableSplitKeys) {
                    workspace_gv += workspace_elements_gv() * split_key_device() / num_splits_key_device();
                    workspace += workspace_elements_gk() * split_key_device() / num_splits_key_device();
                }
            } else {
                workspace = nullptr;
            }
            ...
            dropout_batch_head_rng_offset =
                batch_id * (num_heads * num_queries * num_keys) + head_id * (num_queries * num_keys);
      ...
      PRINT_T0("[b:%d h:%d] dp[0]:%f Q:%f K:%f V:%f LSE:%f",
        ...
        CUTLASS_HOST_DEVICE int64_t workspace_elements_gk() const {
            ...
            return num_splits_key * kBlockSizeJ * align_up(head_dim, kBlockSizeI);
        }
        CUTLASS_HOST_DEVICE int64_t workspace_elements_gv() const {
            if (!kNeedsAccumGradV) {
                return 0;
            }
            return num_splits_key * kBlockSizeJ * align_up(head_dim_value, kBlockSizeI);
        }
        CUTLASS_HOST_DEVICE int64_t workspace_elements_gq() const {
            ...
            return num_blocks * num_cols * sizeof(GradQTempStorage) / sizeof(output_accum_t);
        }
        CUTLASS_HOST_DEVICE int64_t workspace_strideBH() const {
            // Aligned on 128bits
            return align_up(workspace_elements_gk() + workspace_elements_gv() + workspace_elements_gq(), int64_t(4));
        }
    ...
    using ZijSharedStorage =
        typename cutlass::platform::conditional<kApplyDropout,
    ...
    struct SharedStoragePrologue {
        ...
        union {
            ...
            struct {
                // part2 - dQ
                union {
                    typename MatmulQK::AccumulatorSharedStorage tmpT_shared_storage;  // (from part1)
...
#define FIELD(INSIDE_STRUCT, FIELDNAME) \
    ...
    using SharedStorage =
        typename cutlass::platform::conditional<kPreload, SharedStoragePrologue, SharedStorageNoPrologue>::type;
    ...
    static bool __host__ check_supported(Params const& p) {
        ...
        TORCH_CHECK(p.num_heads <= 1 || p.lse_strideH % 8 == 0, "LSE is not correctly aligned (strideH)");
        TORCH_CHECK(p.num_batches <= 1 || p.lse_strideB % 8 == 0, "LSE is not correctly aligned (strideB)");
        TORCH_CHECK(p.num_heads <= 1 || p.q_strideH % kMinimumAlignment == 0,
                    "query is not correctly aligned (strideH)");
        TORCH_CHECK(p.num_heads <= 1 || p.k_strideH % kMinimumAlignment == 0, "key is not correctly aligned (strideH)");
        TORCH_CHECK(p.num_heads <= 1 || p.v_strideH % kMinimumAlignment == 0,
                    "value is not correctly aligned (strideH)");
        TORCH_CHECK(p.num_batches <= 1 || p.q_strideB % kMinimumAlignment == 0,
                    "query is not correctly aligned (strideB)");
        TORCH_CHECK(p.num_batches <= 1 || p.k_strideB % kMinimumAlignment == 0,
                    "key is not correctly aligned (strideB)");
        TORCH_CHECK(p.num_batches <= 1 || p.v_strideB % kMinimumAlignment == 0,
                    "value is not correctly aligned (strideB)");
        TORCH_CHECK(p.q_strideM % kMinimumAlignment == 0, "query is not correctly aligned (strideM)");
        TORCH_CHECK(p.k_strideM % kMinimumAlignment == 0, "key is not correctly aligned (strideM)");
        TORCH_CHECK(p.v_strideM % kMinimumAlignment == 0, "value is not correctly aligned (strideM)");
        if (p.bias_ptr) {
            TORCH_CHECK(p.num_batches <= 1 || p.bias_strideB % kMinimumAlignment == 0,
                        ...
                        kMinimumAlignment,
                        ".");
            TORCH_CHECK(p.num_heads <= 1 || p.bias_strideH % kMinimumAlignment == 0,
                        ...
                        kMinimumAlignment,
                        ".");
            TORCH_CHECK(p.num_queries <= 1 || p.bias_strideM % kMinimumAlignment == 0,
        ...
        if (p.grad_bias_ptr) {
            TORCH_CHECK(p.num_batches <= 1 || p.gB_strideB % kMinimumAlignment == 0,
                        "attn_bias.grad is not correctly aligned (strideB)");
            TORCH_CHECK(p.num_heads <= 1 || p.gB_strideH % kMinimumAlignment == 0,
                        "attn_bias.grad is not correctly aligned (strideH)");
            TORCH_CHECK(p.gB_strideM % kMinimumAlignment == 0, "attn_bias.grad is not correctly aligned (strideM)");
        }
        TORCH_CHECK(!(p.cu_seqlens_q_ptr && p.bias_ptr), "CuSeqlen + bias not implemented yet");
        TORCH_CHECK(p.custom_mask_type < NumCustomMaskTypes, "Invalid value for `custom_mask_type`");
        TORCH_CHECK(p.dropout_prob <= 1.0f && p.dropout_prob >= 0.0f, "Invalid value for `dropout_prob`");
        TORCH_CHECK(kApplyDropout || p.dropout_prob == 0.0f, "Set `kApplyDropout`=True to support `dropout_prob > 0`");
        ...
        TORCH_CHECK(p.head_dim_value <= kMaxK, "kMaxK: Expected `head_dim_value < kMaxK`");
        if (kKeysQueriesAlignedToBlockSize) {
            TORCH_CHECK(p.cu_seqlens_k_ptr == nullptr, "This kernel does not support cu_seqlen");
            TORCH_CHECK(p.cu_seqlens_q_ptr == nullptr, "This kernel does not support cu_seqlen");
            TORCH_CHECK(p.num_queries % kBlockSizeI == 0, "kKeysQueriesAlignedToBlockSize condition not respected");
            TORCH_CHECK(p.num_keys % kBlockSizeJ == 0, "kKeysQueriesAlignedToBlockSize condition not respected");
        }
        TORCH_CHECK(kEnableSplitKeys || p.num_splits_key == 1, "SplitKeys is disabled");
        TORCH_CHECK(p.num_splits_key > 0, "Invalid `num_splits_key` (expected >0)");
        TORCH_CHECK(p.num_splits_key <= cutlass::ceil_div(p.num_keys, kBlockSizeJ),
                    "Invalid `num_splits_key` (",
                    p.num_splits_key,
                    ") - too large for `num_keys` = ",
                    p.num_keys);
        if (p.window_size != 0) {
            TORCH_CHECK(p.custom_mask_type != NoCustomMask, "LocalAttention only supported in causal mode");
        }
        return true;
    }
    ...
    static CUTLASS_DEVICE void attention_kernel(Params p) {
        ...
        if (kPrologueQK) {
            ...
            prologueQkNextIteration<true>(shared_storage, p, query_start, key_start, warp_id, lane_id);
        }
        ...
        if (kApplyDropout) {
            ...
            curand_init(seed, 0, offset + p.dropout_batch_head_rng_offset, &rng_state_init);
        }

        CUTLASS_PRAGMA_UNROLL
        for (; key_start < p.num_keys; key_start += p.num_splits_key_device() * kBlockSizeJ) {
            output_frags.clear();
            ...
            if (kOutputInRF) {
                writeFragsToGmem<kKeysQueriesAlignedToBlockSize>(shared_storage,
                                                                 output_frags,
                                                                 p,
                                                                 key_start,
                                                                 warp_id,
                ...
                zfillGradKV<kKeysQueriesAlignedToBlockSize>(p, key_start, warp_id, lane_id);
            }
            __syncthreads();
        }
    }
    ...
    static CUTLASS_DEVICE void zfillGradKV(Params const& p, int32_t key_start, uint8_t warp_id, uint8_t lane_id) {
    ...
    static CUTLASS_DEVICE void processBlockIJ(SharedStorage& shared_storage,
                                              ...
                                              uint8_t warp_id,
                                              uint8_t lane_id) {
        cutlass::Array<cutlass::uint1b_t, MatmulDOIVJ::Mma::FragmentC::kElements> dropout_keep_mask_doivj;
        dropout_keep_mask_doivj.fill(cutlass::uint1b_t{1});
        const float dropout_scale = kApplyDropout ? 1.0 / (1.0 - p.dropout_prob) : 1.0f;
        ...
        int32_t num_queries_in_block =
            skipBoundsChecks
                ...
                : warp_uniform(cutlass::fast_min(MatmulQK::Mma::Shape::kN, (int32_t)(p.num_queries - query_start)));
        int32_t num_keys_in_block =
            skipBoundsChecks
                ? MatmulQK::Mma::Shape::kM
                : warp_uniform(cutlass::fast_min(MatmulQK::Mma::Shape::kM, (int32_t)(p.num_keys - key_start)));
        ...
        auto prologueGradV = [&](int64_t col) {
            typename MatmulGradV::Mma::IteratorB iterator_dO(
                ...
                thread_id,
                no_offset);
            MatmulGradV::Mma::prologue(shared_storage.mm_gradV(), iterator_dO, thread_id, num_queries_in_block);
        };
        auto prologueGradQ = [&](int col) {
            typename MatmulGradQ::Mma::IteratorB iterator_K(
                ...
                thread_id,
                no_offset);
            MatmulGradQ::Mma::prologue(shared_storage.mm_gradQ(), iterator_K, thread_id, num_keys_in_block);
        };
        auto prologueGradK = [&](int col) {
            typename MatmulGradK::Mma::IteratorB iterator_Q(
                ...
                thread_id,
                no_offset);
            MatmulGradK::Mma::prologue(shared_storage.mm_gradK(), iterator_Q, thread_id, num_queries_in_block);
        };
        auto prologueDOV = [&]() {
            ...
            typename MatmulDOIVJ::Mma::IteratorB iterator_B(
                ...
                thread_id,
                no_offset);
            MatmulDOIVJ::Mma::prologue(shared_storage.mm_doivj(), iterator_A, iterator_B, thread_id, p.head_dim_value);
        };
        ...
        /////////////////////////////////////////////////////////////////////////////////////////////////
        {
            ...
            Mma mma(shared_storage.mm_qk_k(), shared_storage.mm_qk_q(), thread_id, warp_id, lane_id);
            ...
            auto output_tile_coords = cutlass::MatrixCoord{warp_idx_mn_0 % Mma::Base::WarpCount::kM,
            ...
            if (p.bias_ptr != nullptr) {
                ...
                typename MatmulQK::BiasLoader::SmemTileIterator smem_tile_iter(bias_tensor_ref, thread_id);
                MatmulQK::BiasLoader::load(bias_iter, smem_tile_iter);

                // Pij += Bij, where Pij is in register fragment and Bij is in shmem
                auto lane_offset = MatmulQK::AccumLambdaIterator::get_lane_offset(lane_id, warp_id, output_tile_coords);
            ...
            // Apply mask
            if (p.custom_mask_type == CausalFromTopLeft || p.custom_mask_type == CausalFromBottomRight) {
                auto lane_offset = MatmulQK::AccumLambdaIterator::get_lane_offset(lane_id, warp_id, output_tile_coords);
      ...
      PRINT_TENSOR4x4_T0_L0("attn_T", accum_ref_attnT);
            ...
            if (kApplyDropout) {
                ...
                const int threads_per_row =
                    cutlass::fast_min(kNumThreads / kQueriesPerBlock, (int64_t)num_keys_in_block);
                const int elts_per_thread =
                    cutlass::round_nearest(cutlass::ceil_div(num_keys_in_block, threads_per_row), 4);

                const int thread_i = thread_id / threads_per_row;
                const int thread_start_j = (thread_id % threads_per_row) * elts_per_thread;

                if (thread_i < kQueriesPerBlock && thread_start_j < num_keys_in_block) {
                    curandStatePhilox4_32_10_t curand_state = curand_state_init;
                    skipahead((query_start + thread_i) * p.num_keys + (key_start + thread_start_j), &curand_state);

                    // generate elements of Zij, 4 elements at a time
                    for (int zij_start_col_idx = thread_start_j;
                         zij_start_col_idx <
                         cutlass::fast_min<int32_t>(thread_start_j + elts_per_thread, num_keys_in_block);
                         zij_start_col_idx += 4) {
                        ...
                        for (int quad_idx = 0; quad_idx < 4; ++quad_idx) {
                            ...
                            zij.at({zij_start_col_idx + quad_idx /*k*/, thread_i /*q*/}) =
                                (&rand_uniform_quad.x)[quad_idx] > p.dropout_prob ? scalar_t(dropout_scale)
                                                                                  : scalar_t(0);
                        }
                    }
                }
        ...
        PRINT_TENSOR4x4_T0_L0_START("zij", zij, kBlockSizeJ - 4, kBlockSizeI - 4);
                ...
                int warp_idx_mn_0 =
                    warp_id % (MatmulDOIVJ::Mma::Base::WarpCount::kM * MatmulDOIVJ::Mma::Base::WarpCount::kN);
                auto output_tile_coords_doivj =
                    cutlass::MatrixCoord{warp_idx_mn_0 % MatmulDOIVJ::Mma::Base::WarpCount::kM,
                                         warp_idx_mn_0 / MatmulDOIVJ::Mma::Base::WarpCount::kM};
                auto lane_offset =
                    MatmulDOIVJ::AccumLambdaIterator::get_lane_offset(lane_id, warp_id, output_tile_coords_doivj);
        ...
        for (int32_t col = 0; col < (kSingleIterationGradV ? 1 : p.head_dim_value);
            ...
            cutlass::gemm::GemmCoord problem_size(num_keys_in_block, p.head_dim_value - col, num_queries_in_block);
            ...
            __syncthreads();
            if (kPrologueGV && !kSingleIterationGradV && col + MatmulGradV::ThreadblockShape::kN < p.head_dim_value) {
        ...
        /////////////////////////////////////////////////////////////////////////////////////////////////
        {
            ...
            // tmp_shared_storage [smem] <- tmp
            {
                ...
                auto lane_offset = LambdaIterator::get_lane_offset(lane_id, warp_id, output_tile_coords);
        ...
        PRINT_TENSOR4x4_T0_L0("attn_T", attn_T)
                ...
                LambdaIterator::iterateRows(
                    ...
                    [&](int accum_m, int accum_n, int idx) {
                        ...
                        if (skipBoundsChecks || (accum_m < num_queries_in_block && accum_n < num_keys_in_block)) {
                ...
                if (p.grad_bias_ptr != nullptr) {
                    typename MatmulDOIVJ::BiasGradEpilogue::OutputTileIterator output_iter(
                        typename MatmulDOIVJ::BiasGradEpilogue::OutputTileIterator::Params{p.gB_strideM},
                    ...
                    typename MatmulDOIVJ::BiasGradEpilogue epilogue(shared_storage.gradB_epilogue(),
                                                                    thread_id,
                                                                    warp_id,
                                                                    lane_id);
        ...
        print_warp_accum<LambdaIterator>(accum, lane_offset, 4, 4);
                ...
                if (!MatmulGradK::DefaultMmaFromSmem::kIsTransposedA) {
                    ...
                    LambdaIterator::iterateRows(
                        lane_offset,
                        [&](int accum_m) {},
                        [&](int accum_m, int accum_n, int idx) { tmpT.at({accum_n, accum_m}) = scalar_t(accum[idx]); },
                        [&](int accum_m) {});
                }
            }

            MatmulDOIVJ::B2bGemm::accumToSmem(shared_storage.tmp_shared_storage(), accum, lane_id, output_tile_coords);
            __syncthreads();
        }
        ...
        for (int col = 0; col < (kSingleIterationGradQ ? 1 : p.head_dim); col += MatmulGradQ::ThreadblockShape::kN) {
            using Mma = typename MatmulGradQ::Mma;
            using AccumTileGmem = typename MatmulGradQ::AccumTileGmem;

            cutlass::gemm::GemmCoord problem_size(num_queries_in_block,
            ...
            int num_cols = kSingleIterationGradQ ? 1 : ceil_div(p.head_dim, MatmulGradQ::ThreadblockShape::kN);
            int storage_id = (col_id + query_start / kBlockSizeI * num_cols);

            if (p.num_splits_key_device() > 1) {
                AtomicLock::acquire(&p.workspace_gq[storage_id].lock, p.split_key_device() + 1, thread_id);
                // Make sure we can see other block's output
                __threadfence();
            }

            AccumTileGmem gmem_tile{&p.workspace_gq[storage_id].buffer[0]};
            if (!kNeedsAccumGradQ || (p.num_splits_key_device() == 1 && key_start == 0)) {
            ...
            __syncthreads();
            bool isLastColumn = kSingleIterationGradQ || (col + MatmulGradQ::ThreadblockShape::kN >= p.head_dim);
            ...
            if (p.num_splits_key_device() > 1) {
                ...
                if (isLast && thread_id == 0) {
                    numAddsSoFar =
                        atomicAdd(&p.workspace_gq[storage_id].counter, 1) + 1;  // `atomicAdd` returns the old value
                }
                isLast = __syncthreads_or(numAddsSoFar == getNumParallelBlocksForQuery(p, query_start));
            ...
            if (kNeedsAccumGradQ && !isLast) {
                ...
                bool direct_store = kNeedsAccumGradQ || key_start == 0 || (p.num_splits_key_device() > 1);
                accumulateInGmem<MatmulGradQ>(isLastColumn ? shared_storage.gradQ_epilogue_lastIter()
        ...
        for (int col = 0; col < (kSingleIterationGradK ? 1 : p.head_dim); col += MatmulGradK::ThreadblockShape::kN) {
            ...
            auto createEpilogueIter = [&]() {
                return typename MatmulGradK::OutputTileIterator(
                    ...
                    {num_keys_in_block, false ? MatmulGradK::ThreadblockShape::kN : p.head_dim - col},
                    thread_id);
            };
            ...
            auto getTmp = [&](int) {
                return &shared_storage.tmp_shared_storage();
            };
            auto getTmpT = [&](int) {
                return &shared_storage.tmpT_shared_storage();
            };
            // this is basically:
            // opA = kIsTransposedA ? getTmp() : getTmpT();
            bool constexpr kIsTransposedA = MatmulGradK::DefaultMmaFromSmem::kIsTransposedA;
            auto& opA =
                *call_conditional<kIsTransposedA, decltype(getTmp), decltype(getTmpT)>::apply(getTmp, getTmpT, 0);
            Mma mma(
                // operand A: dSij.T
                opA.accum_ref(),
                // operand B: Qi
            ...
            AccumTileGmem gmem_tile{p.workspace + storage_id * AccumTileGmem::kElementsStored};
            if (!kOutputInRF) {
            ...
            __syncthreads();
            bool isLastColumn = kSingleIterationGradK || col + MatmulGradK::ThreadblockShape::kN >= p.head_dim;
            ...
            if (kPrologueQK && isLastColumn) {
                ...
                DISPATCH_BOOL(next_key != key_start, kForceReloadK, ([&]() {
                                  prologueQkNextIteration<kForceReloadK>(shared_storage,
                                                                         p,
                                                                         next_query,
                                                                         next_key,
                                                                         warp_id,
            ...
            if (!kOutputInRF) {
                if (kNeedsAccumGradK && !isLastQuery) {
                    ...
                    accumulateInGmem<MatmulGradK>(isLastColumn ? shared_storage.gradK_epilogue_final()
    ...
    static CUTLASS_HOST_DEVICE int64_t getQueryStart(Params const& p, int64_t key_start) {
    ...
    static CUTLASS_HOST_DEVICE int64_t getSmallestQueryForKey(Params const& p, int64_t key_start) {
        if (p.custom_mask_type == NoCustomMask) {
            return 0;
        }
        int64_t shift = p.custom_mask_type == CausalFromBottomRight ? p.num_keys - p.num_queries : 0;
        int64_t window_size = p.window_size == 0 ? p.num_queries + p.num_keys : p.window_size;

        auto last_key_for_block = cutlass::fast_min(key_start + kBlockSizeJ, (int64_t)p.num_keys) - 1;
    ...
    static CUTLASS_HOST_DEVICE int32_t getNumParallelBlocksForQuery(Params const& p, int32_t query_start) {
        int16_t num_key_blocks = ceil_div(p.num_keys, kBlockSizeJ);
        if (p.custom_mask_type != NoCustomMask) {
            int32_t shift = p.custom_mask_type == CausalFromBottomRight ? p.num_keys - p.num_queries : 0;
            int32_t last_query_for_block = cutlass::fast_min(query_start + kBlockSizeI, p.num_queries) - 1;
            int32_t last_key_for_block = cutlass::fast_min(last_query_for_block + shift, p.num_keys - 1);
            int32_t first_key_for_block =
                p.window_size == 0 ? 0 : cutlass::fast_max(query_start - p.window_size + 1 + shift, 0);

            if (p.window_size == 0) {
                num_key_blocks = last_key_for_block / kBlockSizeJ + 1;
            } else {
                num_key_blocks = (last_key_for_block / kBlockSizeJ) - (first_key_for_block / kBlockSizeJ) + 1;
            }
    ...
    static CUTLASS_HOST_DEVICE void
    incrIteration(Params const& p, int64_t query_start, int64_t key_start, int64_t& next_query, int64_t& next_key) {
        ...
        if (query_shift) {
            ...
            if (p.window_size > 0) {
                ...
                int last_key_for_block = cutlass::fast_min(key_start + kBlockSizeJ, (int64_t)p.num_keys) - 1;
    ...
    static CUTLASS_DEVICE void writeFragsToGmem(SharedStorage& shared_storage,
        ...
        int32_t num_keys_in_block = skipBoundsChecks
                                        ? MatmulQK::Mma::Shape::kM
                                        : cutlass::fast_min(MatmulQK::Mma::Shape::kM, p.num_keys - key_start);
    ...
    static CUTLASS_DEVICE void accumulateInGmem(typename MatmulT::DefaultEpilogue::SharedStorage& epilogue_smem,
        ...
        DISPATCH_BOOL(
            ...
            kIsFirst,
            ([&]() {
                static constexpr auto ScaleType = kIsFirst ? cutlass::epilogue::thread::ScaleType::Nothing
                                                           : cutlass::epilogue::thread::ScaleType::NoBetaScaling;
                using EpilogueOutputOp =
                    typename cutlass::epilogue::thread::LinearCombination<typename DefaultOutputOp::ElementOutput,
    ...
    static CUTLASS_DEVICE void computeDelta(Params const& p, int32_t query_start, uint8_t warp_id, uint8_t lane_id) {
        ...
        const AccessType* __restrict__ output_ptr =
            reinterpret_cast<const AccessType*>(p.output_ptr + (query_start + laneRow) * p.o_strideM() + laneFirstCol);
        ...
        for (int iter = 0; iter < kPipelineStages - 1; ++iter) {
            ...
            pred = pred && (laneFirstCol + iter * kElementsPerAccess * kNumThreadsPerLine) < p.head_dim_value;
            loadAndIncrement(ld_pos, pred);
        }
        auto columnIteration = [&](int iter) {
            // Load for next iter
            int ld_pos = (iter + kPipelineStages - 1) % kPipelineStages;
            pred = pred && (laneFirstCol + (iter + kPipelineStages - 1) * kElementsPerAccess * kNumThreadsPerLine) <
                               p.head_dim_value;
            loadAndIncrement(ld_pos, pred);
            CUTLASS_PRAGMA_UNROLL
