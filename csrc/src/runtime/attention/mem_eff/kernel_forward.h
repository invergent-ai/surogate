...
#include <runtime/attention/mem_eff/epilogue/epilogue_rescale_output.h>
...
constexpr int getWarpsPerSmFw() {
    return (Arch::kMinComputeCapability >= 80 && !cutlass::platform::is_same<scalar_t, float>::value ? 16 : 12);
}
...
struct AttentionKernel {
    ...
    struct Params {
        ...
        CUTLASS_DEVICE bool advance_to_block() {
            ...
            if (output_accum_ptr != nullptr) {
                output_accum_ptr +=
                    int64_t(q_start + query_start) * (head_dim_value * num_heads) + head_id * head_dim_value;
            } else {
            ...
            if (custom_mask_type == CausalFromTopLeft || custom_mask_type == CausalFromBottomRight) {
                ...
                num_keys =
                    cutlass::fast_min(int32_t(query_start + causal_diagonal_offset + kQueriesPerBlock), num_keys);
            }
            ...
            if (num_queries == 1 && k_strideH == 0 && v_strideH == 0 && logsumexp_ptr == nullptr && window_size == 0) {
    ...
    */
        ...
        using DefaultConfig = typename cutlass::gemm::device::DefaultGemmConfiguration<OpClass,
                                                                                       ArchTag,
                                                                                       scalar_t,
                                                                                       scalar_t,
                                                                                       scalar_t,  // ElementC
                                                                                       ...
                                                                                       >;
        static constexpr int kAlignmentA = kIsAligned ? DefaultConfig::kAlignmentA : GemmType::kMinimumAlignment;
        static constexpr int kAlignmentB = kIsAligned ? DefaultConfig::kAlignmentB : GemmType::kMinimumAlignment;
        ...
        using Mma = typename cutlass::platform::conditional<kSingleValueIteration,
    ...
    */
        ...
        using DefaultGemm =
            cutlass::gemm::kernel::DefaultGemm<scalar_t,                   // ElementA,
                                               ...
                                               ArchTag::kMinComputeCapability >= 80 && kIsHalf ? 4
        ...
        using OutputTileIterator = typename cutlass::epilogue::threadblock::
            PredicatedTileIterator<typename DefaultEpilogue::OutputTileIterator::ThreadMap, output_t>;
        using OutputTileIteratorAccum = typename cutlass::epilogue::threadblock::
            PredicatedTileIterator<typename DefaultEpilogue::OutputTileIterator::ThreadMap, output_accum_t>;
    };
    ...
    static bool __host__ check_supported(Params const& p) {
        ...
        if (kSupportsBias) {
            ...
            TORCH_CHECK(p.num_batches <= 1 || p.bias_strideB % kAlignmentQ == 0,
                        ...
                        kAlignmentQ,
                        ".");
            TORCH_CHECK(p.num_heads <= 1 || p.bias_strideH % kAlignmentQ == 0,
                        ...
                        kAlignmentQ,
                        ".");
            TORCH_CHECK(p.num_queries <= 1 || p.bias_strideM % kAlignmentQ == 0,
        ...
        TORCH_CHECK(p.num_heads <= 1 || p.q_strideH % kAlignmentQ == 0, "query is not correctly aligned (strideH)");
        TORCH_CHECK(p.num_heads <= 1 || p.k_strideH % kAlignmentK == 0, "key is not correctly aligned (strideH)");
        TORCH_CHECK(p.num_heads <= 1 || p.v_strideH % kAlignmentV == 0, "value is not correctly aligned (strideH)");
        TORCH_CHECK(p.custom_mask_type < NumCustomMaskTypes, "invalid value for `custom_mask_type`");
        if (p.window_size > 0) {
            TORCH_CHECK(p.custom_mask_type == CausalFromTopLeft || p.custom_mask_type == CausalFromBottomRight,
    ...
    static void CUTLASS_DEVICE attention_kernel(Params& p) {
        ...
        if (kSupportsDropout && p.use_dropout) {
            ...
            curand_init(seed, 0, offset + p.dropout_batch_head_rng_offset, &curand_state_init);
        }
        ...
        for (int32_t iter_key_start = 0; iter_key_start < p.num_keys; iter_key_start += kKeysPerBlock) {
            if (p.window_size > 0) {
                ...
                if (iter_key_start + kKeysPerBlock < int32_t(query_start + p.causal_diagonal_offset) - p.window_size) {
                    continue;
                }
            }
            ...
            auto prologueV = [&](int blockN) {
                ...
                MM1::Mma::prologue(shared_storage.after_mm0.mm1, iterator_V, thread_id(), problem_size_1_k);
            };
            ...
            if (kSupportsBias && p.attn_bias_ptr != nullptr) {
                ...
                typename MM0::BiasLoader::SmemTileIterator smem_tile_iter(bias_tensor_ref, thread_id());
                MM0::BiasLoader::load(bias_iter, smem_tile_iter);

                // Pij += Bij, Pij is in register fragment and Bij is in shared memory
                auto lane_offset =
                    MM0::AccumLambdaIterator::get_lane_offset(my_lane_id, my_warp_id, iteratorC_tile_offset);
            ...
            if (p.custom_mask_type && cutlass::fast_min(iter_key_start + kKeysPerBlock, p.num_keys) >=
                ...
                MM0::AccumLambdaIterator::iterateRows(
                    ...
                    [&](int accum_m) {
                        ...
                        // iter_key_start)
                        last_col = query_start + accum_m + p.causal_diagonal_offset - iter_key_start;
                    },
                    [&](int accum_m, int accum_n, int idx) {
                        if (accum_n > last_col) {
            ...
            if (p.window_size > 0 &&
                (query_start + p.causal_diagonal_offset +
                     cutlass::fast_min(int32_t(kQueriesPerBlock), int32_t(p.num_queries)) - p.window_size >=
                 iter_key_start)) {
                auto query_start = blockIdx.x * kQueriesPerBlock;
                auto lane_offset =
                    MM0::AccumLambdaIterator::get_lane_offset(my_lane_id, my_warp_id, iteratorC_tile_offset);
                int32_t first_col;
                const int32_t offset = query_start + p.causal_diagonal_offset - p.window_size - iter_key_start;
                MM0::AccumLambdaIterator::iterateRows(
                    lane_offset,
            ...
            iterative_softmax<typename MM0::Mma::Operator::IteratorC>(accum_o,
                                                                      accum,
                                                                      mi,
                                                                      m_prime,
                                                                      s_prime,
            ...
            if (kSupportsDropout && p.use_dropout) {
                ...
                const int threads_per_row = cutlass::fast_min(num_threads / problem_size_0_m, problem_size_0_n);
                const int elts_per_thread =
                    cutlass::round_nearest(cutlass::ceil_div(problem_size_0_n, threads_per_row), 4);
                ...
                if (thread_i < problem_size_0_m && thread_start_j < problem_size_0_n) {
                    ...
                    skipahead(static_cast<unsigned long long>((query_start + thread_i) * p.num_keys_absolute +
                    ...
                    for (int sij_start_col_idx = thread_start_j;
                         sij_start_col_idx < cutlass::fast_min(thread_start_j + elts_per_thread, problem_size_0_n);
                         sij_start_col_idx += 4) {
                        const float4 rand_uniform_quad = curand_uniform4(&curand_state);

                        CUTLASS_PRAGMA_UNROLL
                        for (int quad_idx = 0; quad_idx < 4; ++quad_idx) {
                            si.at({thread_i, sij_start_col_idx + quad_idx}) *= static_cast<scalar_t>(
                                dropout_scale * ((&rand_uniform_quad.x)[quad_idx] > p.dropout_prob));
                        }
                    }
                }
            ...
            for (int blockN = 0; blockN < nBlockN; ++blockN) {
                ...
                if (!kKeepOutputInRF) {
                    ...
                    if (p.window_size > 0) {
                        first_key =
                            (cutlass::fast_max(int(query_start + p.causal_diagonal_offset) - p.window_size + 1, 0) /
                             kKeysPerBlock) *
                            kKeysPerBlock;
                    }
        ...
        if (p.logsumexp_ptr && thread_id() < kQueriesPerBlock) {
            ...
            if (thread_id() < p.num_queries) {
                ...
                mi[thread_id()] =
                    (mi[thread_id()] == -cutlass::platform::numeric_limits<accum_t>::infinity()) ? 0 : mi[thread_id()];
                p.logsumexp_ptr[thread_id()] =
                    accum_t(mi[thread_id()] / kLog2e) + cutlass::fast_log(accum_t(s_prime[thread_id()]));
    ...
    iterative_softmax(typename WarpIteratorC::Fragment& frag_o,  // output so far
    ...
    */
        ...
        using LambdaIterator = typename DefaultMmaAccumLambdaIterator<WarpIteratorC, accum_t, kWarpSize>::Iterator;
        ...
        if (lane_id < kLinesPerWarp) {
            ...
            if (changed) {
                ...
                if (kSupportsBias && mi_id == -cutlass::platform::numeric_limits<accum_t>::infinity()) {
        ...
        if (kKeepOutputInRF && !is_first) {
            ...
            LambdaIterator::iterateRows(
                ...
                [&](int accum_m, int accum_n, int idx) { frag_o[idx] = frag_o[idx] * line_rescale; },
                [&](int accum_m) {});
        }
        // Update accum_m, accum_n, ...
        {
            ...
            LambdaIterator::iterateRows(
                ...
                [&](int accum_m, int accum_n, int idx) {
                    frag[idx] = (accum_n < max_col) ? exp2f(frag[idx] - mi_row) : accum_t(0.0);
                },
                [&](int accum_m) {});
            LambdaIterator::iterateRows(
                lane_offset,
                ...
                [&](int accum_m) {
                    if (LambdaIterator::reduceSameRow(lane_id, total_row, [](accum_t a, accum_t b) { return a + b; })) {
