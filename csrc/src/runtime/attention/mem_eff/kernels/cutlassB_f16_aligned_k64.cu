...
#endif
#endif
    printf("FATAL: kernel `fmha_cutlassB_f16_aligned_64x64_k64_seqaligned_sm70` is for sm70-sm74, but was built for "
           "sm%d\n",
...
__global__ void __launch_bounds__(
    ...
    AttentionBackwardKernel<cutlass::arch::Sm80, cutlass::half_t, true, false, true, 64, 64, 64, true>::
        attention_kernel(p);
    return;
#endif
#endif
    printf("FATAL: kernel `fmha_cutlassB_f16_aligned_64x64_k64_seqaligned_sm80` is for sm80-sm121, but was built for "
           "sm%d\n",
           int(__CUDA_ARCH__ + 0) / 10);
#endif
}
...
#endif
#endif
    printf("FATAL: kernel `fmha_cutlassB_f16_aligned_64x64_k64_sm50` is for sm50-sm69, but was built for sm%d\n",
           int(__CUDA_ARCH__ + 0) / 10);
#endif
}
__global__ void __launch_bounds__(
...
#endif
#endif
    printf("FATAL: kernel `fmha_cutlassB_f16_aligned_64x64_k64_sm70` is for sm70-sm74, but was built for sm%d\n",
           int(__CUDA_ARCH__ + 0) / 10);
#endif
}
__global__ void __launch_bounds__(
...
#endif
#endif
    printf("FATAL: kernel `fmha_cutlassB_f16_aligned_64x64_k64_sm75` is for sm75-sm79, but was built for sm%d\n",
           int(__CUDA_ARCH__ + 0) / 10);
#endif
}
__global__ void __launch_bounds__(
...
#endif
#endif
    printf("FATAL: kernel `fmha_cutlassB_f16_aligned_64x64_k64_sm80` is for sm80-sm121, but was built for sm%d\n",
           int(__CUDA_ARCH__ + 0) / 10);
#endif
}
