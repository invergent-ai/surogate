...
#endif
#endif
    printf("FATAL: kernel `fmha_cutlassB_f32_notaligned_64x64_k128_sm50` is for sm50-sm69, but was built for sm%d\n",
...
#endif
#endif
    printf("FATAL: kernel `fmha_cutlassB_f32_notaligned_64x64_k128_sm70` is for sm70-sm74, but was built for sm%d\n",
           int(__CUDA_ARCH__ + 0) / 10);
#endif
}
__global__ void __launch_bounds__(
...
#endif
#endif
    printf("FATAL: kernel `fmha_cutlassB_f32_notaligned_64x64_k128_sm75` is for sm75-sm79, but was built for sm%d\n",
           int(__CUDA_ARCH__ + 0) / 10);
#endif
}
