...
#endif
#endif
    printf("FATAL: kernel `fmha_cutlassF_bf16_aligned_64x64_rf_sm80` is for sm80-sm121, but was built for sm%d\n",
...
#endif
#endif
    printf("FATAL: kernel `fmha_cutlassF_bf16_aligned_64x128_rf_sm80` is for sm80-sm121, but was built for sm%d\n",
           int(__CUDA_ARCH__ + 0) / 10);
#endif
}
__global__ void __launch_bounds__(
    ...
    fmha_cutlassF_bf16_aligned_32x128_gmem_sm80(
        typename AttentionKernel<cutlass::bfloat16_t, cutlass::arch::Sm80, true, 32, 128, 65536, true, true>::Params
            p) {
#ifdef __CUDA_ARCH__
#if __CUDA_ARCH__ >= 800
#if __CUDA_ARCH__ <= 1210
...
#endif
#endif
    printf("FATAL: kernel `fmha_cutlassF_bf16_aligned_32x128_gmem_sm80` is for sm80-sm121, but was built for sm%d\n",
           int(__CUDA_ARCH__ + 0) / 10);
#endif
}
