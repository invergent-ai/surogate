 ...
 */
inline void allocate_fp4_forward_buffers(FP4ForwardQuantActivations& quants,
    ...
    quants.global_amax_buffer =
        allocator.allocate(ETensorDType::FP32, "fp4_global_amax", EAllocationType::ON_DEVICE, {4L});
    ...
    // LN1 -> QKV projection input: (B*T, C)
    {
        ...
        quants.ln1_data = allocator.allocate(fp4_dtype, "fp4_fwd_ln1_data", EAllocationType::ON_DEVICE, {M, C / 2});
        quants.ln1_scales = allocator.allocate(scale_dtype, "fp4_fwd_ln1_scales", EAllocationType::ON_DEVICE, {sr, sc});
    }

    // LN2 -> MLP up projection input: (B*T, C)
    {
        auto [sr, sc] = compute_fp4_scale_shape(M, C);
        quants.ln2_data = allocator.allocate(fp4_dtype, "fp4_fwd_ln2_data", EAllocationType::ON_DEVICE, {M, C / 2});
        quants.ln2_scales = allocator.allocate(scale_dtype, "fp4_fwd_ln2_scales", EAllocationType::ON_DEVICE, {sr, sc});
    }

    // Att -> output projection input: (B*T, AttC)
    {
        auto [sr, sc] = compute_fp4_scale_shape(M, AttC);
        quants.att_data = allocator.allocate(fp4_dtype, "fp4_fwd_att_data", EAllocationType::ON_DEVICE, {M, AttC / 2});
        quants.att_scales = allocator.allocate(scale_dtype, "fp4_fwd_att_scales", EAllocationType::ON_DEVICE, {sr, sc});
    }
    ...
    // Swiglu -> MLP down projection input: (B*T, D)
    {
        ...
        quants.swiglu_data =
            allocator.allocate(fp4_dtype, "fp4_fwd_swiglu_data", EAllocationType::ON_DEVICE, {M, D / 2});
        quants.swiglu_scales =
            allocator.allocate(scale_dtype, "fp4_fwd_swiglu_scales", EAllocationType::ON_DEVICE, {sr, sc});
    }

    // Hadamard transform workspace: largest dimension among C, AttC, D
    const long max_dim = std::max({C, AttC, D});
    quants.hadamard_workspace =
        allocator.allocate(activation_dtype, "fp4_hadamard_ws", EAllocationType::ON_DEVICE, {M, max_dim});
}
