...
std::unique_ptr<QLoRAWeightProvider> create_dsl_qlora_provider(
    ...
    if (options.OffloadExperts && qlora_cfg.is_moe()) {
        ...
        // CUDA graph decode + cross-stream offload prefetch can allocate group
        // buffers on a side stream and free them on the captured main stream,
        // which is not allocator-stable on current driver/runtime combinations.
        // Keep offload alloc/free on the main stream for graph-safe behavior.
        config.weight_manager_config.offload_config.enable_prefetch = false;
    ...
    if (options.OffloadMaster) {
        ...
        config.weight_manager_config.offload_config.max_resident_groups = 2;
        config.weight_manager_config.offload_config.device_id = config.quantizer_config.device_id;
        // See OffloadExperts note above: keep offload alloc/free on main stream
        // to avoid cross-stream allocator issues under CUDA graph capture.
        config.weight_manager_config.offload_config.enable_prefetch = false;
        ...
        if (frozen_prequant_inference) {
            ...
            if (options.UseCudaGraphs &&
                ...
                fprintf(stderr,
                        ...
                        "inference with full-step CUDA graphs (capture-safe).\n");
                dequant_cache_size = 0;
            }
        }
