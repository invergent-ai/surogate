    ...
    auto hook = [this, micro_step](int layer_idx, cudaStream_t stream, modules::ForwardHookPoint point, void* context) {
        ...
        auto get_dropout_seed = [&](int proj_type) -> unsigned int {
            ...
            return mLoRARunState->dropout_base_seed + static_cast<unsigned int>(layer_idx) * 1000000u +
                   static_cast<unsigned int>(proj_type) * 100000u + static_cast<unsigned int>(micro_step) * 10000u;
        };
        ...
        switch (point) {
            case modules::ForwardHookPoint::AfterQKVProjection: {
                ...
                if (lora_block.attention.q.has_value()) {
                    modules::detail::apply_lora_contribution(acts.qkv,
                                                             0,
                                                             acts.ln1,
                                                             lora_block.attention.q.value(),
            ...
            case modules::ForwardHookPoint::AfterAttnOutProjection: {
                ...
                if (lora_block.attention.o.has_value()) {
                    modules::detail::apply_lora_contribution(acts.att_out,
                                                             0,
                                                             acts.att,
                                                             lora_block.attention.o.value(),
                                                             mLoRARunState->intermediate,
                                                             mLoRARunState->slice,
                                                             scaling,
                                                             dropout,
            ...
            case modules::ForwardHookPoint::AfterMLPUpProjection: {
                ...
                if (lora_block.mlp.up.has_value()) {
                    modules::detail::apply_lora_contribution(acts.mlp_up,
                                                             0,
                                                             ln2_input,
                                                             lora_block.mlp.up.value(),
                                                             mLoRARunState->intermediate,
                                                             mLoRARunState->slice,
                                                             scaling,
                                                             dropout,
            ...
            case modules::ForwardHookPoint::AfterMLPDownProjection: {
                if (lora_block.mlp.down.has_value()) {
                    ...
                    if (!down_input.Data && acts.mlp_up.Data) {
                        ...
                        for (int i = 3; i < MAX_TENSOR_DIM; ++i)
                            down_input_view.Sizes[i] = 1;
                        switch (cfg.activation_type) {
                            ...
                            default: throw std::runtime_error("unsupported activation type for LoRA MLPDown replay");
                        }
                    ...
                    if (down_input.Data) {
                        try {
                            modules::detail::apply_lora_contribution(acts.mlp_down,
                                                                     0,
                                                                     down_input,
                                                                     lora_block.mlp.down.value(),
                                                                     mLoRARunState->intermediate,
                                                                     mLoRARunState->slice,
                                                                     scaling,
                                                                     dropout,
...
void DslModel::backward(Tensor inputs, Tensor targets, NCCLCommunicator& comm, int grad_accum_steps, int micro_step) {
    ...
    auto hook =
        [this,
         &comm](int layer_idx, bool accumulate, cudaStream_t stream, modules::BackwardHookPoint point, void* context) {
            (void)context;
            const auto& cfg = mModelConfig;
            auto& rs = *mRunState;
            const int B = (int)rs.B;
            ...
            switch (point) {
                case modules::BackwardHookPoint::AfterMLPDownBackward: {
                    ...
                    modules::detail::backward_lora_layer(lora_grads.mlp.down->A,
                                                         lora_grads.mlp.down->B,
                                                         da.d_swiglu,
                                                         da.d_res_ffn,
                                                         0,
                                                         a.swiglu,
                                                         lora_block.mlp.down->A,
                                                         lora_block.mlp.down->B,
                                                         mLoRAConfig->scaling(),
                                                         dropout,
                                                         get_dropout_seed(6),
                                                         is_training,
                ...
                case modules::BackwardHookPoint::AfterMLPUpBackward: {
                    ...
                    if (mOptions.recompute_enabled()) {
                        if (mLoRARunState && mLoRARunState->recompute_ln.Data) {
                            ...
                            ln2_input = recompute_lora_rmsnorm(*mLoRARunState,
                                                               ln2_residual,
                                                               ln2_weight,
                                                               mModelConfig.RmsNormEps,
                                                               B,
                                                               T,
                                                               C,
                                                               stream);
                    ...
                    if (gated_mlp) {
                        ...
                        modules::detail::backward_lora_mlp_up_gate_fused(dA_up,
                                                                         dB_up,
                                                                         dA_gate,
                                                                         dB_gate,
                                                                         d_ln2,
                        ...
                        modules::detail::backward_lora_layer(dA_up,
                                                             dB_up,
                                                             d_ln2,
                                                             da.d_mlp_up,
                                                             0,
                ...
                case modules::BackwardHookPoint::AfterAttnOutBackward: {
                    ...
                    modules::detail::backward_lora_layer(lora_grads.attention.o->A,
                                                         lora_grads.attention.o->B,
                                                         da.d_att,
                                                         da.d_att_out,
                                                         0,
                                                         a.att,
                                                         lora_block.attention.o->A,
                                                         lora_block.attention.o->B,
                                                         mLoRAConfig->scaling(),
                                                         dropout,
                                                         dropout_seed,
                                                         is_training,
                ...
                case modules::BackwardHookPoint::AfterQKVBackward: {
                    ...
                    if (mOptions.recompute_enabled()) {
                        if (mLoRARunState && mLoRARunState->recompute_ln.Data) {
                            ...
                            ln1_input = recompute_lora_rmsnorm(*mLoRARunState,
                                                               ln1_residual,
                                                               ln1_weight,
                                                               mModelConfig.RmsNormEps,
                                                               B,
                    ...
                    modules::detail::backward_lora_qkv_fused(dA_q,
                                                             dB_q,
                                                             dA_k,
                                                             dB_k,
                                                             dA_v,
...
void DslModel::backward_grpo(Tensor inputs,
    ...
    if (!lora_enabled()) {
        ...
        modules::BackwardHook hook = [this, &comm](int layer_idx,
                                                   bool accumulate,
                                                   cudaStream_t stream,
