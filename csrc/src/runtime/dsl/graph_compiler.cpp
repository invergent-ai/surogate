...
void finalize_save_for_bwd(CompiledGraph& fwd,
    ...
    // indirectly (LoRA backward hooks, per-slot simplified_acts reads).
...
void compute_layout(CompiledGraph& graph, bool is_backward) {
    ...
    // [L*peak, (L+1)*peak) slice of the arena, so simplified_acts[L][SLOT]
    // is a distinct address for every L. This is the correctness
