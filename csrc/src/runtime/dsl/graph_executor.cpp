                                     ...
                                     const modules::ForwardHook* hook)
{
    ...
    if (prefill_it == mPrefillCompiledForwardCache.end()) {
        ...
        compiled->compute_layer_segments(static_cast<std::uint8_t>(GraphCompileMode::InferencePrefill));
                                         ...
                                         bool use_cuda_graph)
{
    ...
    if (B != mDecodeCompiledB || T != mDecodeCompiledT) {
        ...
        mDecodeCompiledForward->compute_layer_segments(static_cast<std::uint8_t>(GraphCompileMode::InferenceDecode));
        mDecodeCompiledB = B;
        mDecodeCompiledT = T;
    ...
    if (use_cuda_graph) {
        const bool offload_decode = mOptions.OffloadExperts || mOptions.OffloadMaster;
        if (offload_decode) {
