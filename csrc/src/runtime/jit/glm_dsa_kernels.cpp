...
struct Workspace {
    ...
    Workspace(std::byte* data, int B, int T, int TK, int D, int P, int select) {
        ...
        scores = alloc(rows * pools, 4);
        selected = alloc(rows * select, 4);
    }
};
...
void GlmDsaKernels::indexer(const std::vector<Tensor>& x,
    ...
    launch("dsa_score",
           ...
           pcap,
           offset);
    launch("dsa_select", dim3(T, B), stream, s.scores, ends, s.selected, T, NP, pcap);
    int selected = std::min(select, TK / P);
    launch("dsa_indices", dim3(T, B), stream, s.selected, x[7].Data, indices.Data, T, offset, selected);
}
...
void GlmDsaKernels::attention(const Tensor& qkv,
    ...
    int S = std::min(select, TK / P) * P + constant("dsa_indices", "STRIDE") - select * P;
...
void GlmDsaKernels::backward(const Tensor& dout,
    ...
    int P = constant("dsa_pool", "P"), select = constant("dsa_select", "SELECT");
    int S = std::min(select, T / P) * P + constant("dsa_indices", "STRIDE") - select * P;
