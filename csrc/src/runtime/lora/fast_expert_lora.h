 ...
 */
inline void fast_expert_lora_forward(Tensor& output,              ///< (N, C)
    ...
    if (lora.gate.has_value() && lora.gate->has_value()) {
        matmul(lora_intermediate,
               ...
               stream);
        if (scaling != 1.0f)
            vector_add_sr(lora_intermediate,
                          lora_intermediate,
                          lora_intermediate,
                          0.5f * scaling,
                          lora_intermediate.nelem(),
                          0,
                          stream);
        matmul(state.e,
               lora.gate->B,
 ...
 */
inline void fast_expert_lora_backward(LoRAExpertWeights<Tensor>& lora_grads,
                                      Tensor& dx,                  ///< (N, C)
                                      const Tensor& dy,            ///< (N, C)
                                      const Tensor& gate_up_proj,  ///< (2*D, C)
                                      const Tensor& down_proj,     ///< (C, D)
    ...
    if (lora.down.has_value() && lora.down->has_value()) {
        matmul(lora_intermediate1,
               ...
               stream);
        if (scaling != 1.0f)
            vector_add_sr(lora_intermediate1,
                          lora_intermediate1,
                          lora_intermediate1,
                          0.5f * scaling,
                          lora_intermediate1.nelem(),
                          0,
                          stream);
        matmul(dh,
               lora.down->A,
...
inline void grouped_fast_expert_lora_forward(
    ...
    auto dispatch_grouped_gemm = [&](Tensor& out,
                                     const Tensor& in,
                                     const Tensor& weight,
                                     int M,
                                     int K,
        ...
        if (in.DType == ETensorDType::BF16) {
            moe_grouped_gemm(out.get<nv_bfloat16>() + out_offset,
                             in.get<nv_bfloat16>(),
            ...
            moe_grouped_gemm(out.get<float>() + out_offset,
                             in.get<float>(),
                             weight.get<float>(),
                             expert_offsets.get<int>(),
                             num_experts,
    ...
    if (permuted_input.DType == ETensorDType::BF16) {
        moe_grouped_gemm_gate_up(gate_up_buffer.get<nv_bfloat16>(),
        ...
        moe_grouped_gemm_gate_up(gate_up_buffer.get<float>(),
    ...
    if (lora.gate.has_value() && lora.gate->has_value()) {
        dispatch_grouped_gemm(lora_intermediate,
                              ...
                              EMMTranspose::TN,
                              false);
        dispatch_grouped_gemm(total_gate,
                              lora_intermediate,
                              lora.gate->B,
                              D,
                              lora_rank,
    ...
    if (lora.up.has_value() && lora.up->has_value()) {
        dispatch_grouped_gemm(lora_intermediate,
                              ...
                              EMMTranspose::TN,
                              false);
        dispatch_grouped_gemm(total_up,
                              lora_intermediate,
                              lora.up->B,
                              D,
                              lora_rank,
    ...
    // 5. Down projection: y = h @ W_down^T + lora_down(h)
    if (permuted_input.DType == ETensorDType::BF16) {
        moe_grouped_gemm_down(total_output.get<nv_bfloat16>(),
        ...
        moe_grouped_gemm_down(total_output.get<float>(),
                              h_buffer.get<float>(),
    ...
    if (lora.down.has_value() && lora.down->has_value()) {
        dispatch_grouped_gemm(lora_intermediate,
                              ...
                              EMMTranspose::TN,
                              false);
        dispatch_grouped_gemm(total_output,
                              lora_intermediate,
                              lora.down->B,
                              C,
                              lora_rank,
...
inline void grouped_fast_expert_lora_backward(
    ...
    auto dispatch_grouped_gemm = [&](Tensor& out,
        ...
        if (in.DType != weight.DType || in.DType != out.DType) {
            throw std::runtime_error(
                fmt::format("MoE LoRA backward: dtype mismatch in grouped GEMM. in={}, weight={}, out={}. "
                            "Set lora_dtype='bf16' in your config to match activation dtype.",
                            dtype_to_str(in.DType),
                            dtype_to_str(weight.DType),
    ...
    auto dispatch_weight_grad = [&](Tensor& d_weight,
                                    const Tensor& dy,
                                    const Tensor& in,
                                    int M,
                                    int N,
    ...
    if (total_dy.DType == ETensorDType::BF16) {
        moe_grouped_gemm_down_backward(total_dh.get<nv_bfloat16>(),
                                       total_dy.get<nv_bfloat16>(),
                                       batched_down_proj.get<nv_bfloat16>(),
                                       expert_offsets.get<int>(),
                                       num_experts,
                                       C,
        ...
        moe_grouped_gemm_down_backward(total_dh.get<float>(),
                                       total_dy.get<float>(),
                                       batched_down_proj.get<float>(),
                                       expert_offsets.get<int>(),
                                       num_experts,
    ...
    if (lora.down.has_value() && lora.down->has_value()) {
        dispatch_grouped_gemm(lora_intermediate1,
                              total_dy,
                              lora.down->B,
                              lora_rank,
                              C,
                              ...
                              EMMTranspose::NN,
                              false);
        dispatch_grouped_gemm(total_dh,
                              lora_intermediate1,
                              lora.down->A,
                              D,
                              lora_rank,
    ...
    if (lora.down.has_value() && lora.down->has_value() && lora_grads.down.has_value()) {
        dispatch_grouped_gemm(lora_intermediate1,
                              ...
                              EMMTranspose::TN,
                              false);
        dispatch_weight_grad(lora_grads.down->B,
                             total_dy,
                             lora_intermediate1,
                             C,
                             lora_rank,
        ...
        dispatch_grouped_gemm(lora_intermediate1,
                              ...
                              EMMTranspose::NN,
                              false);
        dispatch_weight_grad(lora_grads.down->A,
                             lora_intermediate1,
                             total_h,
                             lora_rank,
                             D,
    ...
    if (lora.gate.has_value() && lora.gate->has_value() && lora_grads.gate.has_value()) {
        dispatch_grouped_gemm(lora_intermediate1,
                              ...
                              EMMTranspose::TN,
                              false);
        dispatch_weight_grad(lora_grads.gate->B,
                             total_gate,
                             lora_intermediate1,
                             D,
                             lora_rank,
        ...
        dispatch_grouped_gemm(lora_intermediate1,
                              ...
                              EMMTranspose::NN,
                              false);
        dispatch_weight_grad(lora_grads.gate->A,
                             lora_intermediate1,
                             total_input,
                             lora_rank,
                             C,
    ...
    if (lora.up.has_value() && lora.up->has_value() && lora_grads.up.has_value()) {
        dispatch_grouped_gemm(lora_intermediate1,
                              ...
                              EMMTranspose::TN,
                              false);
        dispatch_weight_grad(lora_grads.up->B,
                             total_up,
                             lora_intermediate1,
                             D,
                             lora_rank,
        ...
        dispatch_grouped_gemm(lora_intermediate1,
                              ...
                              EMMTranspose::NN,
                              false);
        dispatch_weight_grad(lora_grads.up->A,
                             lora_intermediate1,
                             total_input,
                             lora_rank,
                             C,
    ...
    if (total_dy.DType == ETensorDType::BF16) {
        moe_grouped_gemm_gate_up_backward(total_dx.get<nv_bfloat16>(),
                                          d_gate_up_buffer.get<nv_bfloat16>(),
                                          batched_gate_up_proj.get<nv_bfloat16>(),
                                          expert_offsets.get<int>(),
                                          num_experts,
                                          C,
        ...
        moe_grouped_gemm_gate_up_backward(total_dx.get<float>(),
                                          d_gate_up_buffer.get<float>(),
                                          batched_gate_up_proj.get<float>(),
                                          expert_offsets.get<int>(),
                                          num_experts,
                                          C,
    ...
    if (lora.gate.has_value() && lora.gate->has_value()) {
        dispatch_grouped_gemm(lora_intermediate1,
                              ...
                              EMMTranspose::NN,
                              false);
        dispatch_grouped_gemm(total_dx,
                              lora_intermediate1,
                              lora.gate->A,
                              C,
                              lora_rank,
 ...
 */
inline void
grouped_fast_expert_lora_forward_nongated(Tensor& total_output,             ///< (total_tokens, C)
                                          const Tensor& permuted_input,     ///< (total_tokens, C)
    ...
    if (permuted_input.DType == ETensorDType::BF16) {
        moe_grouped_gemm(total_up.get<nv_bfloat16>(),
                         permuted_input.get<nv_bfloat16>(),
        ...
        moe_grouped_gemm(total_up.get<float>(),
                         permuted_input.get<float>(),
    ...
    switch (activation_type) {
        case ActivationType::SiLU: silu_forward(h_buffer, total_up, N, stream); break;
        case ActivationType::ReLU2: relu2_forward(h_buffer, total_up, N, stream); break;
        default: throw std::logic_error("grouped_fast_expert_lora_forward_nongated: unsupported activation");
    }
...
inline void grouped_fast_expert_lora_backward_nongated(
    ...
    switch (activation_type) {
        ...
        case ActivationType::ReLU2:
            ...
            break;
        default: throw std::logic_error("grouped_fast_expert_lora_backward_nongated: unsupported activation");
    }
    ...
    // 5. dx = d_up @ W_up
    if (total_dy.DType == ETensorDType::BF16) {
        moe_grouped_gemm_up_backward(total_dx.get<nv_bfloat16>(),
        ...
        moe_grouped_gemm_up_backward(total_dx.get<float>(),
                                     total_up.get<float>(),
                                     batched_up_proj.get<float>(),
                                     expert_offsets.get<int>(),
                                     num_experts,
