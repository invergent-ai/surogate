...
void CompiledExecutor::dispatch_ep_combine(const CompiledOp& op) {
    ...
    mComm->all_to_all_single(reinterpret_cast<const std::byte*>(unsorted_output.Data),
    ...
    const bool sync_combine_fwd_out =
        (std::getenv("SUROGATE_EP_SYNC_COMBINE_FWD_OUT") != nullptr) && mComm && (mComm->dp_size() == 1);
    ...
    if (ep_state.llep_send_reorder_gpu) {
        ...
        Tensor reordered = make_raw_tensor(ep_state_mut.llep_combined_gpu,
                                           expert_output.DType,
                                           {static_cast<long>(N), static_cast<long>(hidden_size)},
                                           expert_output.Device);

        if (expert_output.DType == ETensorDType::BF16) {
            moe_permute_tokens(reordered.get<nv_bfloat16>(),
                               combined.get<nv_bfloat16>(),
            ...
            moe_permute_tokens(reordered.get<float>(),
                               combined.get<float>(),
                               static_cast<int*>(inv_gpu_ptr),
                               N,
                               N,
...
void CompiledExecutor::dispatch_ep_combine_backward(const CompiledOp& op) {
    ...
    if (ep_state.total_send != input_total_send) {
        ...
        oss << "ep_combine_backward: EP state/input mismatch at layer " << layer_idx
            << " (selected_key=" << ep_key_selected << ", state.total_send=" << ep_state.total_send
    ...
    if (mComm && (mComm->dp_size() == 1) && (ep_size > 1)) {
        ...
        if (d_combined.DType == ETensorDType::BF16) {
            moe_scale_forward(d_combined.get<nv_bfloat16>(),
                              ...
                              inv_ep,
                              static_cast<int>(d_combined.nelem()),
                              mRunState.MainStream);
        } else {
    ...
    if (ep_state.llep_send_reorder_gpu) {
        ...
        if (d_combined.DType == ETensorDType::BF16) {
            moe_permute_tokens(d_reordered.get<nv_bfloat16>(),
                               d_combined.get<nv_bfloat16>(),
            ...
            moe_permute_tokens(d_reordered.get<float>(),
                               d_combined.get<float>(),
                               static_cast<int*>(ep_state.llep_send_reorder_gpu),
                               ep_state.total_send,
                               ep_state.total_send,
    ...
    mComm->all_to_all_single(reinterpret_cast<const std::byte*>(d_a2a_input->Data),
