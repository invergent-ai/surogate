...
void CompiledExecutor::dispatch_softplus(const CompiledOp& op) {
    Tensor& inp = resolve_tensor(op.inputs[0]);
    Tensor& out = ensure_output_tensor(op.outputs[0]);

    const long N = static_cast<long>(inp.nelem());
    softplus_forward(out, inp, N, mRunState.MainStream);

    store_tensor(op.outputs[0], out);
}

void CompiledExecutor::dispatch_softplus_backward(const CompiledOp& op) {
    ...
    Tensor& d_inp = (op.outputs[0].shape.empty() && inp.Rank > 0) ? [&]() -> Tensor& {
        ...
        Tensor t = mRunState.temp_alloc(inp.DType, shape, "softplus_backward_d_inp");
    ...
    const long N = static_cast<long>(inp.nelem());
    softplus_backward(d_inp, inp, d_out, N, mRunState.MainStream);
...
// -----------------------------------------------------------------------------
// Softplus backward rule
// Forward: y = softplus(x) = log(1 + exp(x))
// Backward: dx = dy * sigmoid(x)
// -----------------------------------------------------------------------------
std::vector<Operation> softplus_backward(const BackwardRuleContext& ctx) {
    ...
    if (ctx.needs_grad(0)) {
        ...
        ops.push_back(make_operation("softplus_backward_" + std::to_string(ctx.op_counter++),
                                     "softplus_backward",
                                     "softplus_backward",
...
REGISTER_AUTODIFF("softplus", ::dsl::softplus_backward);
