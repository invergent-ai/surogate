...
void FP4Quantizer::quantize(const Tensor& input, QuantizedTensor& output, cudaStream_t stream) {
    ...
    quantize_fp4_block_auto_scale(output.data.get<uint8_t>(),
    ...
    CUDA_CHECK(cudaMemcpyAsync(&global_amax, output.meta.Data, sizeof(float), cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));

    constexpr float FP8_MAX = 448.0f;
    constexpr float FP4_MAX = 6.0f;
    output.global_scale = (global_amax > 0.0f) ? global_amax / (FP8_MAX * FP4_MAX) : 1.0f;
}

void FP4Quantizer::dequantize(const QuantizedTensor& input, Tensor& output, cudaStream_t stream) {
    dequantize_fp4_block(output.get<nv_bfloat16>(),
...
void FP4Quantizer::allocate_storage(int M,
                                    int K,
    ...
    output.scales = allocator.allocate(ETensorDType::FP8_E4M3,
