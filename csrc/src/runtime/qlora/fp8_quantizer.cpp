...
void FP8Quantizer::quantize(const Tensor& input, QuantizedTensor& output, cudaStream_t stream) {
    quantize_per_block(output.data.get<__nv_fp8_e4m3>(),
...
void FP8Quantizer::dequantize(const QuantizedTensor& input, Tensor& output, cudaStream_t stream) {
    dequantize_per_block(output.get<nv_bfloat16>(),
...
void FP8Quantizer::allocate_storage(int M,
                                    int K,
    ...
    output.data = allocator.allocate(ETensorDType::FP8_E4M3,
    ...
    output.scales =
        allocator.allocate(ETensorDType::FP32, fmt::format("{}.block_scales", name).c_str(), alloc_type, {num_scales});
}
