...
std::unique_ptr<IQuantizer> create_quantizer(const QuantizerConfig& config) {
    switch (config.format) {
        case QuantFormat::BNB_NF4: return std::make_unique<BnBQuantizer>(config);
        ...
        case QuantFormat::FP8_PER_BLOCK: {
            ...
            if (config.sm_version > 0 && config.sm_version < 89) {
                throw std::runtime_error("FP8 per-block quantization requires SM89+ (Ada Lovelace or newer). "
                                         "Detected SM" +
                                         std::to_string(config.sm_version) + ".");
            }
        ...
        case QuantFormat::FP4_BLOCK_2D: {
            ...
            if (config.sm_version > 0 && config.sm_version < 100) {
                throw std::runtime_error("FP4 block quantization requires SM100+ (Blackwell or newer). "
                                         "Detected SM" +
                                         std::to_string(config.sm_version) + ".");
            }
            if (!device_supports_fp4()) {
                throw std::runtime_error("FP4 block quantization is not supported on this device. "
        ...
        case QuantFormat::NONE: return nullptr;

        default:
            throw std::runtime_error("Unknown quantization format: " + std::to_string(static_cast<int>(config.format)));
    }
}
