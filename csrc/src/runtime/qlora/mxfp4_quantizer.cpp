...
void MXFP4Quantizer::quantize(const Tensor& /*input*/, QuantizedTensor& /*output*/, cudaStream_t /*stream*/) {
    throw std::runtime_error("MXFP4Quantizer::quantize() is not supported. "
                             "MXFP4 is a read-only format for pre-quantized HF models.");
}

void MXFP4Quantizer::dequantize(const QuantizedTensor& input, Tensor& output, cudaStream_t stream) {
    dequantize_mxfp4(output.get<nv_bfloat16>(),
...
void MXFP4Quantizer::allocate_storage(int M,
                                      int K,
    ...
    output.scales =
        allocator.allocate(ETensorDType::BYTE, fmt::format("{}.e8m0_scales", name).c_str(), alloc_type, {num_scales});
}
