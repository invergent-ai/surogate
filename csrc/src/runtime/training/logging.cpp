...
std::string format_data_loader(const DataLoader& loader, const char* split) {
    return fmt::format(
        R"(  {{"log": "dataset", "split": "{}", "time": "{}", "step": 0, "files": {}, "tokens": {}, "file_index": {}, "chunk_index": {}, "seed": {}}})",
        split,
 ...
 */
void TrainingRunLogger::log_options(
...
void TrainingRunLogger::log_step(int step,
    ...
    log_line(fmt::format(
        ...
        sanitize_float_for_json(loss),
        sanitize_float_for_json(lr),
        sanitize_float_for_json(moe_aux_loss),
...
void TrainingRunLogger::log_eval(int step, float epoch, int eval_tokens, int duration_ms, float loss) {
    ...
    log_line(fmt::format(
        R"(  {{"log": "eval", "time": "{}", "step": {}, "epoch": {}, "eval_tokens": {}, "duration_ms": {}, "loss": {}}})",
        std::chrono::system_clock::now(),
        step,
        epoch,
        eval_tokens,
 ...
 */
void TrainingRunLogger::log_moe_stats(int step,
                                      float aux_loss,
                                      float z_loss,
...
void TrainingRunLogger::log_gpu_state(int step, int gpu_id, const GPUUtilInfo& gpu_util) {
    log_line(fmt::format(
        R"(  {{"log": "gpu", "time": "{}", "step": {}, "id": {}, "clock": {}, "max_clock": {}, "fan": {}, "power": {}, "power_limit": {}, "temperature": {}, "temp_slowdown": {}, "gpu_util": {}, "mem_util": {}, "throttle": "{}", "dram_free": {}, "pcie_rx": {}, "pcie_tx": {}}})",
        std::chrono::system_clock::now(),
        step,
        gpu_id,
        gpu_util.clock,
    ...
    if (mVerbosity >= 1) {
        ...
        printf("Mem Usage %ld MiB | PCI↓%5d MiB/s| PCI↑%5d MiB/s\n",
               ...
               static_cast<int>(gpu_util.pcie_rx / 1024 / 1024),
               static_cast<int>(gpu_util.pcie_tx / 1024 / 1024));
    }
}
    ...
    if (mRank == 0) {
        for (auto& d : all_gpus) {
            ...
            std::string line = fmt::format(
                ...
                d.prop.major,
                d.prop.minor,
                d.prop.totalGlobalMem,
                d.mem_free,
                d.mem_reserved,
                uuid,
                d.prop.ECCEnabled,
                d.prop.sharedMemPerMultiprocessor,
                d.driver_version,
                d.runtime_version);
            log_line(line);
            ...
            if (mVerbosity >= 1 || (mVerbosity >= 0 && d.rank == 0)) {
                ...
                printf("  Memory: %zu MiB / %zu MiB\n",
                       (d.mem_total - d.mem_free) / 1024 / 1024,
...
void TrainingRunLogger::log_cmd(int argc, const char** argv) {
    if (mRank != 0) return;
    std::string cmd =
        fmt::format(R"(  {{"log": "cmd", "time": "{}", "step": 0, "cmd": [)", std::chrono::system_clock::now());
...
void TrainingRunLogger::log_sol_estimate(std::vector<std::pair<ETensorDType, long>> ops, int world_size) {
    ...
    log_line(fmt::format(
        R"(  {{"log": "sol", "time": "{}", "rank": {}, "step": {}, "blocks": {}, "lm_head": {}, "attention": {}, "tps": {}, "tf32_peak": {}, "bf16_peak": {}, "fp16_peak": {}, "fp8_peak": {}}})",
        std::chrono::system_clock::now(),
        mRank,
        0,
        ops[0].second,
 ...
 */
void print_memory_breakdown(const MemoryBreakdownContext& ctx,
    ...
    auto categorize = [](const std::string& name) -> std::string {
        ...
        if (name.find("dequant") != std::string::npos || name.find("fp8_quant") != std::string::npos ||
            name.find("fp4_quant") != std::string::npos) {
            return "QLoRA Dequant Buffers";
        }
        if (name.find("qlora") != std::string::npos || name.find("fp8_weight") != std::string::npos ||
            name.find("fp4_weight") != std::string::npos || name.find("_nf4") != std::string::npos ||
            name.find("_absmax") != std::string::npos || name.find("_scales") != std::string::npos) {
        ...
        if (name == "ln1" || name == "ln2" || name == "qkv" || name == "att" || name == "att_out" ||
            name == "residual_att" || name == "mlp_up" || name == "swiglu" || name == "mlp_down" || name == "lse" ||
            name.find("_rstd") != std::string::npos || name.find("_shared") != std::string::npos) {
    ...
    printf("  Untracked CUDA overhead:      %8.1f MiB (%.1f%%)\n",
           untracked_mib,
           (untracked_mib / cuda_used_mib) * 100.0);
    printf("\n");
...
void TrainingRunLogger::log_allocator(const std::vector<std::pair<std::string, sSegmentMemory>>& stats,
    ...
    for (auto& [name, amount] : stats) {
        ...
        first = false;
        stat_str +=
            fmt::format("{{\"name\": \"{}\", \"device\": {}, \"managed\": {}, \"pinned\": {}, \"pageable\": {}}}",
                        name,
                        amount.OnDevice,
                        amount.Managed,
                        amount.PinnedHost,
    ...
    stat_str += "]";
    std::string line = fmt::format(R"(  {{"log": "allocator", "time": "{}", "step": 0, "stats": {}}})",
                                   std::chrono::system_clock::now(),
                                   stat_str);
    log_line(line);
