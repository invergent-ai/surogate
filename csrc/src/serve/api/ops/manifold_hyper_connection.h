...
/// per slice plus the `pre` gate per token -- which the reduce then finishes in slice order.
/// Bounds the adaptive split on the selected CUDA device for the entire token interval,
