...
/// The E-series pair: `google/gemma-4-E2B-it` and `google/gemma-4-E4B-it`.
///
/// It attends the way the dense sizes do -- two head geometries, a window on one of them --
/// and adds the two things that make it the E-series: a per-token, per-layer input mixed into
/// every block, and a tail of layers that project a query only and read an earlier layer's
/// keys and values.
namespace targets::gemma4_e {
...
SINFER_TARGET_LOAD_TYPES(gemma4_e::Package);
...
struct Package {
    ...
    static constexpr std::array<std::string_view, 1> model_ids{"gemma4-e"};
    static constexpr std::string_view model_id = model_ids[0];
    static constexpr std::string_view target_key = "gemma4_e";
