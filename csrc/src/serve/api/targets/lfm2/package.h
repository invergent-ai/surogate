...
enum class WeightsProfile : std::uint8_t {
    /// Group-wise integer throughout, which is what a converted LFM2 checkpoint stores and what
    /// a GGUF served natively lands on. The endpoints carry whatever the artifact declares.
    GroupwiseInt,
};
...
SINFER_TARGET_LOAD_TYPES(lfm2::Package);
