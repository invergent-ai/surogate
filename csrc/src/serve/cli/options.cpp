...
KvCacheStorage parse_kv_cache(std::string_view text) {
    if (text == "auto") { return KvCacheStorage::Auto; }
    if (text == "bf16") { return KvCacheStorage::BFloat16; }
    if (text == "int8") { return KvCacheStorage::Int8Group64; }
    if (text == "fp8" || text == "fp8_e4m3") { return KvCacheStorage::Fp8E4M3; }
...
std::string usage_text(const char* argv0) {
    return std::string("usage: ") + argv0 +
           ...
           "       [--max-context N|auto] [--kv-capacity N|auto] [--gpu-layers N|all] [--host-moe-layers N|all] [--expert-slots N] [--host-expert-bank w8|q4] [--cpu-moe-share F] [--cpu-moe-min-tokens N] [--prefill-chunk N] [--max-new N]\n"
