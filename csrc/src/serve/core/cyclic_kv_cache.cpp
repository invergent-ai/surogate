...
CyclicKVCacheLayout plan_cyclic_kv_cache(LayoutBuilder& builder, std::uint32_t layers,
    ...
    if (layers == 0 || capacity == 0 ||
        ...
        (dtype != DType::BF16 && dtype != DType::FP8_E4M3FN)) {
    ...
    for (std::uint32_t layer = 0; layer < layers; ++layer) {
        ...
        layout.k.push_back(builder.add_tensor(
            dtype, {head_dim, padded, num_kv_heads, lane_capacity}, kArenaAlign, prefix + " K"));
        layout.v.push_back(builder.add_tensor(
            dtype, {head_dim, padded, num_kv_heads, lane_capacity}, kArenaAlign, prefix + " V"));
    }
    return layout;
}
...
CyclicKVCache::CyclicKVCache(DeviceSpan backing, const CyclicKVCacheLayout& layout)
    ...
    for (std::size_t layer = 0; layer < layout.k.size(); ++layer) {
        if ((layout.k[layer].dtype != DType::BF16 && layout.k[layer].dtype != DType::FP8_E4M3FN) ||
            layout.k[layer].dtype != layout.k.front().dtype ||
            layout.v[layer].dtype != layout.k[layer].dtype ||
...
void CyclicKVCache::copy_lane_from(const CyclicKVCache& source, std::int32_t lane,
    ...
    if (source.lane_capacity_ != lane_capacity_) {
        throw std::invalid_argument("Cyclic KV copy requires identical lane capacities");
    }
...
void CyclicKVCache::copy_lane_from(const CyclicKVCache& source, std::int32_t source_lane,
                                   std::int32_t destination_lane, cudaStream_t stream) {
    ...
    if (source_lane < 0 || source_lane >= source.lane_capacity_ ||
        destination_lane < 0 || destination_lane >= lane_capacity_) {
        throw std::out_of_range("Cyclic KV lane is out of range");
    }
    for (std::size_t layer = 0; layer < k_.size(); ++layer) {
        Tensor destination_k = k_[layer].slice(3, destination_lane, 1);
        Tensor destination_v = v_[layer].slice(3, destination_lane, 1);
