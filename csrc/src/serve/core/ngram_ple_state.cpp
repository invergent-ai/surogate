...
void NgramPleStatePool::copy_slot(std::int32_t src, std::int32_t dst, cudaStream_t stream) {
    ...
    if (src == dst) { return; }
    const Tensor history_src = history.slice(1, src, 1);
    const Tensor history_dst = history.slice(1, dst, 1);
    CUDA_CHECK(cudaMemcpyAsync(history_dst.data, history_src.data, history_src.bytes(),
                               cudaMemcpyDeviceToDevice, stream));
    const Tensor conv_src = conv_state.slice(2, src, 1);
    const Tensor conv_dst = conv_state.slice(2, dst, 1);
...
void NgramPleStatePool::reset_slot(std::int32_t slot, cudaStream_t stream) {
    ...
    const Tensor history_slot = history.slice(1, slot, 1);
    ...
    const Tensor conv_slot = conv_state.slice(2, slot, 1);
