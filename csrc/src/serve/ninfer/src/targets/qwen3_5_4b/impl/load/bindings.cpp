...
load_attention_projection(const FullAttentionPlan& plan,
    ...
    if (const auto* split = std::get_if<SplitAttentionProjectionPlan>(&plan.projection)) {
        return SplitAttentionProjectionPayload{
            .query_key  = materialized_weight(materialized, split->query_key, 2560, 2560),
            .gate_value = materialized_weight(materialized, split->gate_value, 2560, 2560),
        };
    }
    ...
    return FusedAttentionProjectionPayload{
        .query_key_gate_value =
            materialized_weight(materialized, fused.query_key_gate_value, 10240, 2560),
    };
}
...
load_gdn_input_projection(const GdnPlan& plan, const artifact::MaterializedArtifact& materialized) {
    ...
    return FusedGdnInputProjectionPayload{
        .query_key_value_z =
            materialized_weight(materialized, fused.query_key_value_z, 12288, 2560),
    };
}
...
load_gdn_control_projection(const GdnPlan& plan,
    ...
    if (const auto* split = std::get_if<SplitGdnControlProjectionPlan>(&plan.control_projection)) {
        return SplitGdnControlProjectionPayload{
            .a_projection = materialized_weight(materialized, split->a_projection, 32, 2560),
            .b_projection = materialized_weight(materialized, split->b_projection, 32, 2560),
        };
    }
    const auto& fused = std::get<FusedGdnControlProjectionPlan>(plan.control_projection);
    return FusedGdnControlProjectionPayload{
        .a_b_projection = materialized_weight(materialized, fused.a_b_projection, 64, 2560),
    };
}
    ...
    for (std::size_t layer = 0; layer < kTextLayers; ++layer) {
        ...
        if (target.is_full_attention) {
            ...
            target.attention.output = bind_weight(binder, prefix + "attention/output",
                                                  NumericFormat::W8G32_F16S, {2560, 4096});
        } else {
            ...
            target.gdn.input_projection = FusedGdnInputProjectionPlan{
                .query_key_value_z =
                    bind_weight(binder, prefix + "gdn/query_key_value_z",
                                NumericFormat::W8G32_F16S, {12288, 2560}),
            };
            target.gdn.norm = artifact::bind_device_tensor(binder, prefix + "gdn/norm",
                                                           NumericFormat::BF16, {128});
            target.gdn.output =
                bind_weight(binder, prefix + "gdn/output", NumericFormat::W8G32_F16S, {2560, 4096});
        }
        target.post_attention_norm = artifact::bind_device_tensor(
            binder, prefix + "post_attention_norm", NumericFormat::BF16, {2560});
        target.mlp.gate_up =
            bind_weight(binder, prefix + "mlp/gate_up", NumericFormat::W8G32_F16S, {18432, 2560});
        target.mlp.down =
            bind_weight(binder, prefix + "mlp/down", NumericFormat::W8G32_F16S, {2560, 9216});
    }
}
...
void bind_nvfp4_text_layers(artifact::Binder& binder, BindingPlan& out) {
    for (std::size_t layer = 0; layer < kTextLayers; ++layer) {
        ...
        if (target.is_full_attention) {
            ...
            if (is_early_attention_input(layer)) {
                input = bind_weight(binder, prefix + "attention/query_key_gate_value",
                                    NumericFormat::BF16, {10240, 2560});
            } else {
                input = bind_nvfp4_weight(
                    binder, prefix + "attention/query_key_gate_value", 10240, 2560,
            ...
            if (is_bf16_attention_output(layer)) {
                target.attention.output = bind_weight(binder, prefix + "attention/output",
                                                      NumericFormat::BF16, {2560, 4096});
            } else {
                target.attention.output =
                    bind_nvfp4_weight(binder, prefix + "attention/output", 2560, 4096,
            ...
            target.gdn.input_projection = FusedGdnInputProjectionPlan{
                .query_key_value_z =
                    bind_nvfp4_weight(binder, prefix + "gdn/query_key_value_z", 12288, 2560,
            ...
            if (is_bf16_gdn_output(layer)) {
                target.gdn.output =
                    bind_weight(binder, prefix + "gdn/output", NumericFormat::BF16, {2560, 4096});
            } else {
                target.gdn.output =
                    bind_nvfp4_weight(binder, prefix + "gdn/output", 2560, 4096,
        ...
        target.post_attention_norm = artifact::bind_device_tensor(
            binder, prefix + "post_attention_norm", NumericFormat::BF16, {2560});
        target.mlp.gate_up =
            bind_nvfp4_weight(binder, prefix + "mlp/gate_up", 18432, 2560,
                              prefix + "mlp/gate_up_projection/input_scale_divisor");
        target.mlp.down = bind_nvfp4_weight(binder, prefix + "mlp/down", 2560, 9216,
        ...
        if (target.is_full_attention) {
            target.attention.projection = FusedAttentionProjectionPlan{
                .query_key_gate_value = bind_weight(
                    binder, prefix + "attention/query_key_gate_value", kFp8, {10240, 2560}),
            };
            target.attention.query_norm = artifact::bind_device_tensor(
                binder, prefix + "attention/query_norm", NumericFormat::BF16, {256});
            target.attention.key_norm = artifact::bind_device_tensor(
                binder, prefix + "attention/key_norm", NumericFormat::BF16, {256});
            target.attention.output =
                bind_weight(binder, prefix + "attention/output", kFp8, {2560, 4096});
        } else {
            target.gdn.a_log       = artifact::bind_device_tensor(binder, prefix + "gdn/a_log",
                                                                  NumericFormat::FP32, {32});
            target.gdn.dt_bias     = artifact::bind_device_tensor(binder, prefix + "gdn/dt_bias",
            ...
            target.gdn.input_projection = FusedGdnInputProjectionPlan{
                .query_key_value_z =
                    bind_weight(binder, prefix + "gdn/query_key_value_z", kFp8, {12288, 2560}),
            };
            target.gdn.norm   = artifact::bind_device_tensor(binder, prefix + "gdn/norm",
                                                             NumericFormat::BF16, {128});
            target.gdn.output = bind_weight(binder, prefix + "gdn/output", kFp8, {2560, 4096});
        }
        target.post_attention_norm = artifact::bind_device_tensor(
            binder, prefix + "post_attention_norm", NumericFormat::BF16, {2560});
        if (layer < 56) {
            ...
            target.mlp.gate_up = bind_weight(binder, prefix + "mlp/gate_up", kFp8, {18432, 2560});
            target.mlp.down    = bind_weight(binder, prefix + "mlp/down", kFp8, {2560, 9216});
        }
    }
}
...
ArtifactLoadPlan bind_artifact(artifact::Binder& binder, WeightsProfile weights_profile,
    ...
    out.token_embedding =
        bind_weight(binder, "text/token_embedding", vocabulary_format, {248320, 2560});
    switch (weights_profile) {
    case WeightsProfile::Qwen36GroupwiseInt:
    case WeightsProfile::Qwen38GroupwiseInt:
    ...
    out.final_norm =
        artifact::bind_device_tensor(binder, "text/final_norm", NumericFormat::BF16, {2560});
    out.output_head = bind_weight(binder, "text/output_head", vocabulary_format, {248320, 2560});
    ...
    if (!out.has_mtp && features.mtp()) {
        throw std::runtime_error(
            "qwen3.5-4b artifact has no MTP block (the source checkpoint was "
    ...
    // Text-only target: the qwen3_5_4b artifact carries no vision objects
    // (surogate vendor note; --vision is rejected at startup for this target).
    if (features.vision) {
        throw std::runtime_error("qwen3.5-4b target is text-only: --vision is unsupported");
    }
...
LoadedModelData::LoadedModelData(BindingPlan plan, artifact::MaterializedArtifact materialized)
    ...
    token_embedding        = materialized_weight(backing, plan.token_embedding, 248320, 2560);
    ...
    for (std::size_t layer = 0; layer < kTextLayers; ++layer) {
        ...
        if (source.is_full_attention) {
            ...
            target.key_norm   = artifact::materialized_tensor(backing, source.attention.key_norm,
                                                              NumericFormat::BF16, {256});
            target.output     = materialized_weight(backing, source.attention.output, 2560, 4096);
            target.post_attention_norm = artifact::materialized_tensor(
                backing, source.post_attention_norm, NumericFormat::BF16, {2560});
            ...
            target.output = materialized_weight(backing, source.gdn.output, 2560, 4096);
            target.post_attention_norm = artifact::materialized_tensor(
                backing, source.post_attention_norm, NumericFormat::BF16, {2560});
            target.post_mixer = load_mlp(source.mlp, backing);
        }
    ...
    final_norm =
        artifact::materialized_tensor(backing, plan.final_norm, NumericFormat::BF16, {2560});
    output_head = materialized_weight(backing, plan.output_head, 248320, 2560);
    if (plan.features.optimized_proposal()) {
        ...
        proposal.head      = artifact::materialized_weight(backing, plan.draft_head,
                                                           NumericFormat::W8G32_F16S, 131072, 2560);
    ...
    if (plan.features.mtp() && plan.has_mtp) {
        ...
        mtp.input_projection = artifact::materialized_weight(
            backing, plan.mtp.input_projection, NumericFormat::W8G32_F16S, 2560, 5120);
        ...
        mtp.attention.packed = artifact::materialized_weight(
            backing, plan.mtp.query_key_gate_value, NumericFormat::W8G32_F16S, 10240, 2560);
        // surogate vendor patch (PATCHES.md #13): qwen3.5-4b fused qkgv rows
        ...
        mtp.post_attention_norm = artifact::materialized_tensor(
            backing, plan.mtp.post_attention_norm, NumericFormat::BF16, {2560});
