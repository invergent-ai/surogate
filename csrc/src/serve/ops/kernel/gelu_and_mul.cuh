...
__device__ __forceinline__ __nv_bfloat162 gelu_mul_pair(__nv_bfloat162 g, __nv_bfloat162 u) {
    const float r0 = gelu_one<TanhApprox>(__low2float(g)) * __low2float(u);
    const float r1 = gelu_one<TanhApprox>(__high2float(g)) * __high2float(u);
...
#pragma unroll
        for (int item = 0; item < kGeluAndMulPairsPerThread; ++item) {
            ...
            if (p < n2) { out2[p] = gelu_mul_pair<TanhApprox>(gate2[p], up2[p]); }
        }
    }
    ...
    if (tid == 0 && (n & 1) != 0) {
        ...
        out[i] = __float2bfloat16(gelu_one<TanhApprox>(__bfloat162float(gate[i])) *
