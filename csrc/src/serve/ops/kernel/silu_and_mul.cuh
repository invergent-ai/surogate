...
/// `limit <= 0` is the unclamped product, and with a compile-time zero the bound compiles away.
...
__global__ void silu_and_mul_scalar_kernel(const __nv_bfloat16* gate, const __nv_bfloat16* up,
                                           __nv_bfloat16* out, std::int64_t n, float limit) {
    const std::int64_t start  = blockIdx.x * static_cast<std::int64_t>(blockDim.x) + threadIdx.x;
    const std::int64_t stride = static_cast<std::int64_t>(gridDim.x) * blockDim.x;
    for (std::int64_t i = start; i < n; i += stride) {
        out[i] = __float2bfloat16(
            swiglu_clamped(__bfloat162float(gate[i]), __bfloat162float(up[i]), limit));
    }
}
...
__global__ void silu_and_mul_strided_input_kernel(
    ...
    for (std::int64_t i = start; i < n; i += stride) {
        ...
        out[i] = __float2bfloat16(swiglu_clamped(__bfloat162float(gv), __bfloat162float(uv), limit));
    }
}
...
__launch_bounds__(256) __global__
    ...
    for (std::int64_t j = tid * kSiluAndMulPairsPerThread; j < n2; j += stride) {
        ...
        if (j + 3 < n2) {
            ...
            out2[j]                 = silu_mul_pair(g0, u0, limit);
            out2[j + 1]             = silu_mul_pair(g1, u1, limit);
            out2[j + 2]             = silu_mul_pair(g2, u2, limit);
            out2[j + 3]             = silu_mul_pair(g3, u3, limit);
        } else {
            out2[j] = silu_mul_pair(g0, u0, limit);
            if (j + 1 < n2) { out2[j + 1] = silu_mul_pair(gate2[j + 1], up2[j + 1], limit); }
            if (j + 2 < n2) { out2[j + 2] = silu_mul_pair(gate2[j + 2], up2[j + 2], limit); }
        }
    }
