...
#pragma unroll
            ...
            if (threadIdx.x == i && (rows_per_cuda_block == 1 || row0 + i < nrows_x)) {
                DstT& slot = dst[j * stride_col_dst + i];
                slot       = to_dst<DstT>(Accumulate ? from_dst<DstT>(slot) + tmp[j][i] : tmp[j][i]);
            }
        }
    }
}
...
void launch(const void* blocks, int n, int k, const block_q8_1* y, DstT* out, cudaStream_t stream) {
    ...
    mul_mat_vec_q<type, ncols_dst, DstT, Accumulate><<<grid, block, 0, stream>>>(
