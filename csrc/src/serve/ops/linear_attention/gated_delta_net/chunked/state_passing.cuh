...
__launch_bounds__(kernel_dims<NStrip>::THREADS, kernel_dims<NStrip>::MIN_BLOCKS) __global__
    void state_passing_kernel(const __nv_bfloat16* __restrict__ W_in,
                              ...
                              const float* __restrict__ g_cumsum, const __nv_bfloat16* state_in,
                              __nv_bfloat16* __restrict__ v_new,
                              __nv_bfloat16* __restrict__ h_chunk, __nv_bfloat16* state_out,
...
#pragma unroll
        for (int m = 0; m < M_TILES_H_PW; ++m) {
            ...
            h_frag[m][0] =
                __bfloat162float(load_ldg<__nv_bfloat16>(state_in + st_base + (int64_t)col_d0 * kStateDim + row_g0));
            h_frag[m][1] =
                __bfloat162float(load_ldg<__nv_bfloat16>(state_in + st_base + (int64_t)col_d1 * kStateDim + row_g0));
            h_frag[m][2] =
                __bfloat162float(load_ldg<__nv_bfloat16>(state_in + st_base + (int64_t)col_d0 * kStateDim + row_g1));
            h_frag[m][3] =
                __bfloat162float(load_ldg<__nv_bfloat16>(state_in + st_base + (int64_t)col_d1 * kStateDim + row_g1));
        }
    }
...
#pragma unroll
    for (int m = 0; m < M_TILES_H_PW; ++m) {
        ...
        const int d1   = d0 + 1;
        state_out[st_base + (int64_t)d0 * kStateDim + k_g0] = __float2bfloat16_rn(h_frag[m][0]);
        state_out[st_base + (int64_t)d1 * kStateDim + k_g0] = __float2bfloat16_rn(h_frag[m][1]);
        state_out[st_base + (int64_t)d0 * kStateDim + k_g1] = __float2bfloat16_rn(h_frag[m][2]);
        state_out[st_base + (int64_t)d1 * kStateDim + k_g1] = __float2bfloat16_rn(h_frag[m][3]);
    }
}
