// MoE Marlin dispatch for surogate serve, adapted from vLLM's
// csrc/libtorch_stable/moe/marlin_moe_wna16/ops.cu (Apache-2.0, Copyright (C)
// Marlin.2024 Elias Frantar; modified by Neural Magic; adapted from
// https://github.com/IST-DASLab/marlin). The torch-stable entry is replaced by
// a raw-pointer entry; the kernel template is vendored unmodified under
// vendor/, and only the instantiations this target needs are generated.
...
#include "ops/sparse_moe/marlin/marlin_moe_selector_generated.h"
...
std::int32_t marlin_moe_padded_rows(std::int32_t assignments, std::int32_t num_experts,
                                    std::int32_t block_size) noexcept {
    // Each expert's rows are padded up to a whole block, so the worst case adds one short
    // block per expert - the same bound moe_align_block_size uses.
    return assignments + num_experts * block_size;
}

std::size_t marlin_moe_b_bytes(std::int32_t n, std::int32_t k) noexcept {
    // Marlin packs 4-bit B as int32 words holding 8 values.
    return static_cast<std::size_t>(n) * static_cast<std::size_t>(k) / 8 * sizeof(std::uint32_t);
}

std::size_t marlin_moe_scale_bytes(std::int32_t n, std::int32_t k) noexcept {
...
std::size_t marlin_moe_lock_bytes() noexcept {
    return static_cast<std::size_t>(1024) * sizeof(int);
}
...
void marlin_moe_gemm_q4g64_bf16(const void* a, const void* b_tiles, const void* scales,
    ...
    // Our gather has already expanded each token into one row per routed expert, so the
    // routing table indexes those rows directly: top_k is 1 and the router weights are
    // applied by our own reduce, not here.
    MARLIN_NAMESPACE_NAME::marlin_mm(
        a, b_tiles, c, c_tmp, /*b_bias=*/nullptr, /*a_s=*/nullptr, const_cast<void*>(scales),
        /*g_s=*/nullptr, /*zp=*/nullptr, /*g_idx=*/nullptr, /*perm=*/nullptr, /*a_tmp=*/nullptr,
        const_cast<std::int32_t*>(sorted_token_ids), const_cast<std::int32_t*>(expert_ids),
        const_cast<std::int32_t*>(num_tokens_past_padded), /*topk_weights=*/nullptr,
        block_size, num_experts, /*top_k=*/1, /*mul_topk_weights=*/false, assignments, n, k,
        locks, vllm::kBFloat16, vllm::kU4B8, vllm::kBFloat16, vllm::kBFloat16,
        /*has_bias=*/false, /*has_act_order=*/false, /*is_k_full=*/true, /*has_zp=*/false,
        /*num_groups=*/k / 64, /*group_size=*/64, device, stream, /*thread_k=*/-1,
        /*thread_n=*/-1, sms, /*blocks_per_sm=*/-1, /*use_atomic_add=*/false,
        /*use_fp32_reduce=*/true, /*is_zp_float=*/false);
}

bool marlin_moe_route_enabled() {
    static const bool enabled = [] {
        const char* raw = std::getenv("SUROGATE_SERVE_MOE_MARLIN");
