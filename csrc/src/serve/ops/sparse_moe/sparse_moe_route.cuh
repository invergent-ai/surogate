...
#pragma unroll
    for (int item = 0; item < kPerLane; ++item) {
        ...
        // of 128). Each consumer indexes a table with the id -- the prefill route's counts and
        // tile bases, the decode path's expert weights -- and reached an address gigabytes past
        // the allocation: cudaErrorIllegalAddress. Ranked as -inf, the row selects valid experts
        // (the lowest ids win the tie) and its output stays NaN, as its input was. Finite logits
