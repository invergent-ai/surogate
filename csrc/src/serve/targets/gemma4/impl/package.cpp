...
namespace sinfer::targets::gemma4 {
namespace {

// Gemma 4's published presets, as `generation_config.json` states them for both dense
// sizes. Gemma 4 has no thinking mode -- its chat template renders no reasoning turn -- so
// both entries are the same preset rather than two invented ones.
constexpr ModelSamplingDefaults kGemma4Defaults{
...
ModelSamplingDefaults Package::sampling_defaults(std::string_view model) {
    if (model == model_id) { return kGemma4Defaults; }
...
family::TextGeometry Package::declared_geometry(const artifact::Reader& reader) {
    return family::TextGeometry::resolved_gemma4(
        reader.geometry(), reader.layer_types(), false, false);
}
