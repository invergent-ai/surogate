...
/// The E-series of the Gemma 4 family: `google/gemma-4-E2B-it` and
/// `google/gemma-4-E4B-it`. Compiled for the E2B; the E4B is served by laying its
/// artifact's declared geometry over these.
///
/// Two things separate it from the dense sizes, and both are per layer rather than global:
/// every block folds in a **per-token, per-layer input** after its feed-forward, and the last
/// `num_kv_shared_layers` layers project a query only and attend over an earlier layer's keys
/// and values. Neither is compiled -- the schedule and the widths come from the artifact,
/// because E2B shares 20 of 35 layers and E4B 18 of 42.
///
...
struct TextConfig {
    static constexpr int hidden       = 1536;
    static constexpr int layers       = 35;
    ...
    /// The per-layer input: one layer's slice is 256 wide and is looked up in a table of the
    /// same vocabulary as the token embedding. E2B's stacked table is [262144, 35 * 256]; the
    /// artifact stores it cut per layer, so what a layer binds is [262144, 256].
    static constexpr int per_layer_input_dim = 256;
    static constexpr int per_layer_vocab     = 262144;
    ...
    /// sqrt(1536) rounded to bf16. The E4B's is sqrt(2560); the artifact declares whichever.
    static constexpr float embedding_scale       = 3.925e1F;
...
static_assert(TextConfig::full_attention_layers() == 35);
