...
bool layer_is_windowed(const artifact::Reader& reader, const family::TextGeometry& g,
    ...
    if (width == 0) {
        throw std::runtime_error("gemma4_e: the artifact carries no " + name +
    ...
    throw std::runtime_error(
        "gemma4_e: layer " + std::to_string(layer) + " normalises a head of " +
...
float read_device_bf16(const Tensor& tensor) {
    ...
    if (status != cudaSuccess) {
        throw std::runtime_error(std::string("gemma4_e: could not read a layer scalar: ") +
...
PerLayerInputWeights load_per_layer_input(const PerLayerInputPlan& plan,
                                          const artifact::MaterializedArtifact& materialized,
                                          const family::TextGeometry& g) {
    const std::int32_t width = g.per_layer_input_dim;
    return PerLayerInputWeights{
        .embedding        = materialized_weight(materialized, plan.embedding, g.per_layer_vocab,
                                                width),
        .input_projection = materialized_weight(materialized, plan.input_projection, width,
                                                g.hidden),
        .gate             = materialized_weight(materialized, plan.gate, width, g.hidden),
        .projection       = materialized_weight(materialized, plan.projection, g.hidden, width),
        .norm             = materialized_norm(materialized, plan.norm, g.hidden),
        .projection_norm  = nullptr,
        .width            = width,
    };
}
...
DensePostMixerPayload load_mlp(const MlpPlan& plan,
                               const artifact::MaterializedArtifact& materialized,
                               const family::TextGeometry& g, bool owns_kv) {
...
void bind_text_layers(artifact::Binder& binder, WeightsProfile weights_profile, BindingPlan& out) {
    ...
    for (std::size_t layer = 0; layer < layers; ++layer) {
        ...
        // A layer that shares projects a query and nothing else. The artifact says so by
        // carrying no key for it -- the declaration gives such a layer a Q-only attention --
        // and the geometry records it so the cache is sized and indexed by owners.
        const bool owns_kv       = binder.has(prefix + "attention/key");
        target.attention.owns_kv = owns_kv;
        if (owns_kv) { g.declare_kv_owner(static_cast<std::int32_t>(layer)); }
        ...
        target.attention.value_is_key = owns_kv && !binder.has(prefix + "attention/value");
        if (owns_kv && !target.attention.value_is_key) {
            target.attention.value = bind_weight(binder, prefix + "attention/value", weights,
                                                 {static_cast<std::uint64_t>(kv_rows),
                                                  static_cast<std::uint64_t>(g.hidden)});
        } else if (owns_kv && windowed) {
            // Only the global layers may drop it, and only under `attention_k_eq_v` -- which
            // the E-series does not set, so on these checkpoints every owning layer has one.
            throw std::runtime_error(
                "gemma4_e: windowed layer " + std::to_string(layer) +
        ...
        target.attention.output = bind_weight(binder, prefix + "attention/output", weights,
                                              {static_cast<std::uint64_t>(g.hidden),
                                               static_cast<std::uint64_t>(query_rows)});
        // `use_double_wide_mlp` widens exactly the layers that share their key/value planes,
        ...
        // The per-layer input, five objects. Two of them are this layer's slice of a tensor
        // the checkpoint holds stacked; the converter cut them, so both are ordinary planes.
        const std::string pli    = prefix + "per_layer_input/";
        const auto width         = static_cast<std::uint64_t>(g.per_layer_input_dim);
        target.per_layer_input.embedding =
            bind_weight(binder, pli + "embedding", weights,
                        {static_cast<std::uint64_t>(g.per_layer_vocab), width});
        target.per_layer_input.input_projection =
            bind_weight(binder, pli + "input_projection", weights,
                        {width, static_cast<std::uint64_t>(g.hidden)});
        target.per_layer_input.gate =
            bind_weight(binder, pli + "gate", weights,
                        {width, static_cast<std::uint64_t>(g.hidden)});
        target.per_layer_input.projection =
            bind_weight(binder, pli + "projection", weights,
                        {static_cast<std::uint64_t>(g.hidden), width});
        target.per_layer_input.norm = artifact::bind_device_tensor(
            binder, pli + "norm", NumericFormat::BF16, {g.hidden});
    }
}
...
ArtifactLoadPlan bind_artifact(artifact::Binder& binder, WeightsProfile weights_profile,
    ...
    // Say that this model shares planes *before* any layer is bound: an all-owning mask and an
    // undeclared one are otherwise indistinguishable, and the difference decides whether the
    // cache is indexed by layer or by owner.
    out.geometry.declare_kv_sharing();
    ...
    if (!g.has_global_attention_geometry()) {
        ...
        throw std::runtime_error(
            "gemma4_e: this artifact declares one attention geometry; the dense Gemma 4 "
    ...
    out.final_norm = artifact::bind_device_tensor(binder, "text/final_norm", NumericFormat::BF16,
                                                  {g.hidden});
    out.per_layer_projection_norm =
        artifact::bind_device_tensor(binder, "text/per_layer_projection_norm",
                                     NumericFormat::BF16, {g.per_layer_input_dim});
...
LoadedModelData::LoadedModelData(BindingPlan plan, artifact::MaterializedArtifact materialized)
    ...
    per_layer_projection_norm =
        materialized_norm(backing, plan.per_layer_projection_norm, g.per_layer_input_dim);
    for (std::size_t layer = 0; layer < static_cast<std::size_t>(g.layers); ++layer) {
        ...
        target.projection = AttentionProjectionPayload{
            ...
            .key   = source.attention.owns_kv
                         ? materialized_weight(backing, source.attention.key, kv_rows, g.hidden)
                         : Weight{},
            ...
            .value = (source.attention.value_is_key || !source.attention.owns_kv)
                         ? Weight{}
            ...
            .owns_kv      = source.attention.owns_kv,
            .head_dim     = head_dim,
        };
        target.query_norm = materialized_norm(backing, source.attention.query_norm, head_dim);
        target.key_norm   = source.attention.owns_kv
                                ? materialized_norm(backing, source.attention.key_norm, head_dim)
                                : Tensor{};
        target.output =
        ...
        target.post_mixer = load_mlp(source.mlp, backing, g, source.attention.owns_kv);
        target.post_mixer.per_layer_input = load_per_layer_input(source.per_layer_input, backing, g);
        // One vector for the whole model, pointed at rather than copied: it lives in the
        // artifact's arena for as long as the model does.
        target.post_mixer.per_layer_input.projection_norm = &per_layer_projection_norm;
    }
