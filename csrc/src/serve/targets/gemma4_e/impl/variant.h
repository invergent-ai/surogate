...
struct Variant {
    ...
    /// Every block folds in a per-token, per-layer input after its feed-forward. That is what
    /// the "E" buys, and it is the difference between this target and the dense one.
    static constexpr bool has_per_layer_inputs = true;

    /// The block's tail: build this layer's per-layer input and fold it onto the residual.
    static void layer_epilogue(const ModelView& model, int layer, const Tensor& ids,
                               const Tensor& embedded, Tensor& residual,
                               WorkspaceArena& workspace, cudaStream_t stream);
    [[nodiscard]] static std::size_t
    layer_epilogue_workspace_capacity_bytes(const family::TextGeometry& geometry,
