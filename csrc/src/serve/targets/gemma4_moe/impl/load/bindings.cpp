...
bool layer_is_windowed(const artifact::Reader& reader, const family::TextGeometry& g,
    ...
    if (width == 0) {
        throw std::runtime_error("gemma4_moe: the artifact carries no " + name +
    ...
    throw std::runtime_error(
        "gemma4_moe: layer " + std::to_string(layer) + " normalises a head of " +
...
std::int32_t routed_gate_up_rows(const family::TextGeometry& g) {
    return kMoeGeometry.experts * 2 * g.intermediate;
}

std::int32_t routed_down_rows(const family::TextGeometry& g) {
    return kMoeGeometry.experts * g.hidden;
}
...
MixturePostMixerPayload load_mlp(const MlpPlan& plan,
    ...
    out.op.per_expert_scale = static_cast<const float*>(
        artifact::materialized_tensor(materialized, plan.per_expert_scale, NumericFormat::FP32,
                                      {static_cast<std::uint64_t>(TextConfig::experts)})
            .data);
    ...
    out.op.activation        = kMoeGeometry.activation;
...
ArtifactLoadPlan bind_artifact(artifact::Binder& binder, WeightsProfile weights_profile,
    ...
    if (!g.has_global_attention_geometry()) {
        ...
        throw std::runtime_error(
            "gemma4_moe: this artifact declares one attention geometry; every published "
            "Gemma 4 attends at two (global_head_dim, global_kv_heads)");
    }
    if (features.vision) {
