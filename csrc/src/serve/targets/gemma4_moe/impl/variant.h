...
//
// The attention half is the dense Gemma 4 target's, leaf for leaf: every layer attends, the
// output carries no gate, the value is RMS-normalised with no weight and on a global layer
// *is* the key projection's raw output (`attention_k_eq_v`), the two kinds of layer attend at
// different head widths, the block's whole output is scaled by a per-layer number, and the
// logits are squashed to +-30 before sampling.
//
// `post_mixer` is where this target differs, and it is the only place. A mixture layer runs a
// dense feed-forward and 128 routed experts over the *same* input -- the post-attention
// residual, not the dense branch's normalised copy -- norms each branch, sums them, and norms
// the sum. The router sees a third projection of that same residual: normalised with no
// weight, scaled by a learned per-channel vector and by `hidden ** -0.5`. Two projections of
// one token reaching one mixture op is why `ops::sparse_moe` takes a router input of its own.
//
...
struct Variant {
    ...
    using PostMixerWeights               = detail::MixturePostMixerPayload;
    using MtpAttentionProjectionWeights  = detail::MtpAttentionPayload;
    using MtpPostMixerWeights            = detail::MixturePostMixerPayload;
