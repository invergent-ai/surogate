...
//
// LFM2 is a hybrid whose non-attending layers convolve rather than run a delta net, and whose
// attention carries no output gate. The delta-net and MTP leaves below are declared because the
// runtime is a template over this interface, not because they can run -- each one throws,
// naming why (see variant.cpp).
struct Variant {
    ...
    using GdnProjectionWeights           = detail::ShortConvProjectionPayload;
    ...
    static constexpr family::LinearMixer linear_mixer = family::LinearMixer::ShortConv;
