...
#define SINFER_QWEN36_VARIANT    ::sinfer::targets::llama::detail::Variant
#define SINFER_QWEN36_RUNTIME_NS llama_runtime
...
void Variant::debug_probe(const char* tag, const Tensor& tensor, cudaStream_t stream) {
    ...
    // 'LLPB' -- the header layout is the family's ({magic, rows, columns,
    ...
    const std::int32_t header[4] = {0x4C4C5042, tensor.ne[0], tensor.ne[1], occurrence};
