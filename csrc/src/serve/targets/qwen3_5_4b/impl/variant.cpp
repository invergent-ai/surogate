...
#define SINFER_FAMILY_VARIANT    ::sinfer::targets::qwen3_5_4b::detail::Variant
#define SINFER_FAMILY_RUNTIME_NS qwen3_5_4b_runtime
...
std::size_t Variant::gdn_input_projection_workspace_capacity_bytes(WeightsProfile weights_profile,
    ...
    case WeightsProfile::Qwen38GroupwiseInt:
        ...
        return std::max(ops::gdn_input_proj_workspace_capacity_bytes(
                            QType::W8G32_F16S, 12288, TextConfig::hidden, ops::LinearPolicy::AllowA8,
                            first, last),
                        ops::gdn_input_proj_workspace_capacity_bytes(
                            QType::Q4_K, 12288, TextConfig::hidden, ops::LinearPolicy::A16Only,
                            first, last));
    case WeightsProfile::Qwen35Nvfp4Mixed:
    case WeightsProfile::Qwen36Nvfp4:
        return ops::gdn_input_proj_workspace_capacity_bytes(QType::NVFP4, 12288, TextConfig::hidden,
                                                            kNvfp4TextPolicy, first, last);
    case WeightsProfile::Qwen38Nvfp4:
        return ops::gdn_input_proj_workspace_capacity_bytes(
            QType::FP8_E4M3FN_ROW_BF16S, 12288, TextConfig::hidden, kFp8TextPolicy, first, last);
    }
    throw std::logic_error("invalid 27B weights profile");
}
...
std::size_t Variant::gdn_input_projection_snapshot_workspace_capacity_bytes(
    ...
    case WeightsProfile::Qwen36Nvfp4:
        return std::max(kMinimumLeafWorkspaceBytes,
                        ops::gdn_input_proj_conv_snapshot_workspace_capacity_bytes(
                            QType::NVFP4, 12288, TextConfig::hidden, kNvfp4TextPolicy, batch_size,
                            first, last));
    case WeightsProfile::Qwen38Nvfp4:
        return std::max(kMinimumLeafWorkspaceBytes,
                        ops::gdn_input_proj_conv_snapshot_workspace_capacity_bytes(
                            QType::FP8_E4M3FN_ROW_BF16S, 12288, TextConfig::hidden, kFp8TextPolicy,
                            batch_size, first, last));
    }
    throw std::logic_error("invalid 27B weights profile");
}
