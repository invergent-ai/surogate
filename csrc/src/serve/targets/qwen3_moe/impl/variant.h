...
//
// Every layer attends and the attention output carries no gate, so the difference from the
// dense target is entirely in the tail: a routed mixture with no always-on expert instead of a
// SwiGLU MLP. The linear-mixer and MTP leaves below are declared because the runtime is a
// template over this interface, not because they can run -- each one throws, naming why.
struct Variant {
    ...
    /// RMSNorm scales are stored directly, not zero-centred: the scale is `w`, not `1 + w`.
