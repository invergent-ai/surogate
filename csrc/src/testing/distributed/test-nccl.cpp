...
bool nccl_available() {
    ...
    if (!checked) {
        ...
        try {
            ...
            NCCLCommunicator::run_communicators(1, false, false, [&](NCCLCommunicator& comm) { available = true; });
        } catch (...) {
            available = false;
        }
    }
...
TEST_CASE("launch_communicators_multinode with single node", "[distributed][nccl][multinode][.]") {
    ...
    auto pack = NCCLCommunicator::launch_communicators_multinode(1,  // ngpus
                                                                 0,  // node_rank
                                                                 1,  // num_nodes
                                                                 nccl_id.data(),
...
TEST_CASE("launch_communicators_multinode with multiple local GPUs", "[distributed][nccl][multinode][multi-gpu][.]") {
    ...
    auto pack = NCCLCommunicator::launch_communicators_multinode(test_gpus,  // ngpus
                                                                 0,          // node_rank
                                                                 1,          // num_nodes
                                                                 nccl_id.data(),
                                                                 false,  // memcpy_allgather
...
TEST_CASE("launch_communicators_multinode barrier works", "[distributed][nccl][multinode][sync][.]") {
    ...
    auto pack = NCCLCommunicator::launch_communicators_multinode(test_gpus,
                                                                 0,
                                                                 1,
                                                                 nccl_id.data(),
                                                                 false,
...
TEST_CASE("launch_communicators_multinode host_all_gather works", "[distributed][nccl][multinode][allgather][.]") {
    ...
    auto pack =
        NCCLCommunicator::launch_communicators_multinode(test_gpus,
                                                         0,
                                                         1,
                                                         nccl_id.data(),
                                                         false,
...
TEST_CASE("launch_communicators_multinode validates node_rank", "[distributed][nccl][multinode][error][.]") {
    ...
    REQUIRE_THROWS_AS(NCCLCommunicator::launch_communicators_multinode(1,
                                                                       5,
                                                                       2,  // node_rank=5 but num_nodes=2
                                                                       nccl_id.data(),
                                                                       false,
                                                                       false,
                                                                       [](NCCLCommunicator&) {}),
