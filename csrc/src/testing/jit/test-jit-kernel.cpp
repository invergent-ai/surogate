...
DeviceBuffer make_device(const std::vector<float>& h, cudaStream_t stream = nullptr) {
    ...
    CUDA_CHECK(cudaMemcpyAsync(buf.ptr, h.data(), h.size() * sizeof(float), cudaMemcpyHostToDevice, stream));
    return buf;
}
...
std::vector<float> download(float* d_ptr, size_t n, cudaStream_t stream = nullptr) {
    ...
    CUDA_CHECK(cudaMemcpyAsync(h.data(), d_ptr, n * sizeof(float), cudaMemcpyDeviceToHost, stream));
...
TEST_CASE("JitKernel: load PTX from memory and launch vector_add", "[jit][gpu]") {
    ...
    auto kernel = JitKernel::load_ptx(VECTOR_ADD_PTX, std::strlen(VECTOR_ADD_PTX) + 1, meta);
...
TEST_CASE("JitKernel: launch with explicit block override", "[jit][gpu]") {
    if (!cuda_available()) SKIP("CUDA not available");
...
TEST_CASE("JitKernel: load PTX from file", "[jit][gpu]") {
    if (!cuda_available()) SKIP("CUDA not available");
...
TEST_CASE("JitKernel: load from JSON manifest", "[jit][gpu]") {
    if (!cuda_available()) SKIP("CUDA not available");
...
TEST_CASE("JitKernel: move semantics", "[jit]") {
    if (!cuda_available()) SKIP("CUDA not available");

    JitKernelMeta meta{"vector_add", 4, 0};
    auto k1 = JitKernel::load_ptx(VECTOR_ADD_PTX, std::strlen(VECTOR_ADD_PTX) + 1, meta);
    ...
    // Move assign
    JitKernel k3 = JitKernel::load_ptx(VECTOR_ADD_PTX, std::strlen(VECTOR_ADD_PTX) + 1, meta);
    k3 = std::move(k2);
...
TEST_CASE("JitKernel: launch_triton with Triton-compiled RMSNorm cubin", "[jit][triton][gpu]") {
    if (!cuda_available()) SKIP("CUDA not available");

    const std::string manifest_path = "/tmp/surogate_rmsnorm_test/rmsnorm_fwd_C768.json";
    ...
    for (int i = 0; i < N * C; ++i)
        h_x_f32[i] = std::sin(i * 0.01f);
    for (int j = 0; j < C; ++j)
        h_w_f32[j] = std::cos(j * 0.03f);

    // Convert to bf16 on host
    std::vector<__nv_bfloat16> h_x(N * C), h_w(C);
    for (int i = 0; i < N * C; ++i)
        h_x[i] = __float2bfloat16(h_x_f32[i]);
    for (int j = 0; j < C; ++j)
        h_w[j] = __float2bfloat16(h_w_f32[j]);
    ...
    CUDA_CHECK(cudaMemcpy(d_x, h_x.data(), N * C * sizeof(__nv_bfloat16), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_w, h_w.data(), C * sizeof(__nv_bfloat16), cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK(cudaMemcpy(h_out.data(), d_out, N * C * sizeof(__nv_bfloat16), cudaMemcpyDeviceToHost));
...
TEST_CASE("JitKernel: CuTe DSL RMSNorm via Library API (quack)", "[jit][cute][gpu]") {
    if (!cuda_available()) SKIP("CUDA not available");

    const std::string manifest_path = "/tmp/surogate_cute_rmsnorm_test/cute_rmsnorm_bf16_C768.json";
    if (!std::filesystem::exists(manifest_path)) {
    ...
    CUDA_CHECK(cudaMemcpy(d_x, h_x.data(), M * C * sizeof(__nv_bfloat16), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_w, h_w.data(), C * sizeof(__nv_bfloat16), cudaMemcpyHostToDevice));
    ...
    kernel.launch(dim3(grid_x), dim3(128, 1, 1), kernel.meta().shared_mem_bytes, args, nullptr);
    CUDA_CHECK(cudaDeviceSynchronize());

    // Download results
    std::vector<__nv_bfloat16> h_out(M * C);
    std::vector<float> h_rstd(M);
    CUDA_CHECK(cudaMemcpy(h_out.data(), d_out, M * C * sizeof(__nv_bfloat16), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(h_rstd.data(), d_rstd, M * sizeof(float), cudaMemcpyDeviceToHost));
