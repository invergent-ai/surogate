...
int run(const Case& c) {
    ...
    const std::int32_t kb = c.k / 128, rb = c.rows / 128;
    std::vector<std::uint8_t> codes(static_cast<std::size_t>(c.rows) * c.k);
    std::vector<float> scales(static_cast<std::size_t>(rb) * kb), x(static_cast<std::size_t>(c.k) * c.tokens);
    ...
    for (std::int32_t t = 0; t < c.tokens; ++t) {
        for (std::int32_t r = 0; r < out_rows; ++r) {
            ...
            for (std::int32_t b = 0; b < kb; ++b) {
                ...
                acc += part * scales[static_cast<std::size_t>(row / 128) * kb + b];
            }
    ...
    Weight w{};
    w.payload = d_payload; w.payload_bytes = payload.size(); w.qtype = QType::FP8_E4M3FN_BLK128_F32S;
    ...
    std::printf("  [%5d, %5d] T=%-4d %-12s rel_l2=%.2e max_abs=%.2e of max  %s\n", c.rows, c.k, c.tokens,
                c.rows_view ? "linear_rows" : "linear", rel, max_abs / max_ref, ok ? "ok" : "FAIL");
