...
int run_nvfp4_target() {
    ...
    failures += run_nvfp4_target_case(parent, 300, ops::LinearPolicy::AllowA4);
    failures += run_nvfp4_target_case(parent, 1077, ops::LinearPolicy::AllowA4);
    failures += run_nvfp4_target_case(parent, 1300, ops::LinearPolicy::AllowA4);
    return failures;
}
...
// surogate vendor patch (PATCHES.md #13): qwen3.5-0.8b fused qkgv geometry.
...
int run_w8_q08() {
    constexpr std::int32_t kHidden = 1024;
    DevicePackedWeight parent(
        quantized_weight::make_patterned_weight(QType::W8G32_F16S, 5120, kHidden, 409U));
    int failures = 0;
