...
// surogate vendor patch (PATCHES.md #16): qwen3.5-2b — same fused qkvz row
...
int run_w8_q2b() {
    constexpr std::int32_t kHidden = 2048;
    DevicePackedWeight parent(
        quantized_weight::make_patterned_weight(QType::W8G32_F16S, 8192, kHidden, 709U));
    int failures = 0;
    for (const std::int32_t tokens : {1, 2, 97}) { failures += run_w8_q2b_case(parent, tokens); }
    return failures;
}
...
int run_nvfp4() {
    ...
    failures += run_nvfp4_case(parent, 300, ops::LinearPolicy::AllowA4);
    failures += run_nvfp4_case(parent, 1077, ops::LinearPolicy::AllowA4);
    failures += run_nvfp4_case(parent, 1300, ops::LinearPolicy::AllowA4);
    return failures;
}
...
int run_fp8() {
    ...
    for (const std::int32_t tokens : {1, 2, 7, 8, 48, 65, 300, 1024, 1077}) {
