...
// valid distinct expert whatever the logits hold, and that every row selects exactly the
// reference top-k: a NaN ranks as -inf, then value descending, lower id first on a tie. Rows
// without a NaN are therefore held to the plain top-k, as before the change.
...
int run_shape(const char* name) {
    ...
    std::vector<float> alpha(static_cast<std::size_t>(count) * TopK);
    CHECK_CUDA(cudaMemcpy(ids.data(), d_ids, ids.size() * sizeof(int), cudaMemcpyDeviceToHost));
    CHECK_CUDA(cudaMemcpy(alpha.data(), d_alpha, alpha.size() * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    for (int r = 0; r < count; ++r) {
        ...
        // The reference ranking: a NaN as -inf, then value descending, the lower id first on a
        // tie.
        std::vector<float> ranked(Experts);
        for (int id = 0; id < Experts; ++id) {
            const float score = rows[r][id];
            const float value = Gating == SparseMoeGating::SigmoidBiasTopK
                                    ? host_sigmoid(score) + bias[id]
                                    : score;
            ranked[id] = std::isnan(value) ? -std::numeric_limits<float>::infinity() : value;
        }
        ...
        // The weights: an all-NaN row keeps NaN weights (its output stays NaN); a row with one
        // NaN logit, like every finite row, gets finite weights that sum to one.
        const float* row_alpha = alpha.data() + static_cast<std::size_t>(r) * TopK;
        if (r == 0) {
            for (int k = 0; k < TopK; ++k) {
                if (!std::isnan(row_alpha[k])) {
                    std::printf("FAIL %s row 0: alpha[%d] = %g, expected NaN\n", name, k, row_alpha[k]);
                    ++failures;
                    break;
                }
            }
        } else if (r == 2 || finite_row[r]) {
            double sum = 0.0;
            bool finite = true;
            for (int k = 0; k < TopK; ++k) {
                finite = finite && std::isfinite(row_alpha[k]);
                sum += row_alpha[k];
            }
            if (!finite || std::abs(sum - 1.0) > 1e-5) {
                std::printf("FAIL %s row %d: alpha sum %g%s\n", name, r, sum, finite ? "" : " (non-finite)");
                ++failures;
            }
        }
    }
...
int main() {
    ...
    failures += run_shape<64, 4, SparseMoeGating::SigmoidBiasTopK>("sigmoid+bias 4 of 64 (LFM2)");
    failures += run_shape<32, 4, SparseMoeGating::SigmoidBiasTopK>("sigmoid+bias 4 of 32 (LFM2)");
