...
from tools.reference.qwen3_5.bindings import ArtifactBinding
from tools.reference.qwen3_5.weights import WeightStore
...
def binding():
    path = Path(
        os.environ.get(
            "SINFER_QWEN3_6_27B_ARTIFACT",
            PROJECT_ROOT / "out/qwen3_6_27b.sinfer",
        )
    )
    if not path.is_file():
