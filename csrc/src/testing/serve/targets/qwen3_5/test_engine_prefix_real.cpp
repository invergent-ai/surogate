...
sinfer::PromptInput chinese_chat(bool enable_thinking) {
    sinfer::ChatMessage message;
    message.role = sinfer::ChatRole::User;
    message.parts.push_back(sinfer::MessagePart{
        .kind = sinfer::MessagePartKind::Text, .text = "你好，简单介绍一下你自己。", .media = {}});
...
int exercise_registered_frontend(const sinfer::Engine& engine) {
...
int exercise_full_prefill_chunk(sinfer::Engine& engine) {
    constexpr std::size_t kChunkTokens = 1024;
    std::vector<sinfer::TokenId> prompt(kChunkTokens, 198);
...
int exercise_zero_suffix_reuse(sinfer::Engine& engine, const std::vector<sinfer::TokenId>& prompt) {
    sinfer::RequestOptions baseline_options;
...
int exercise_prefix(sinfer::Engine& engine) {
    sinfer::RequestOptions first_options;
...
int exercise_rewrite_checkpoints(sinfer::Engine& engine) {
    auto text_message = [](sinfer::ChatRole role, std::string text) {
        sinfer::ChatMessage message;
        message.role = role;
        message.parts.push_back(sinfer::MessagePart{
    ...
    auto input_with_history = [&](int completed_responses, bool preserve_thinking) {
        ...
        if (completed_responses >= 2) {
            ...
            sinfer::ChatMessage tool = text_message(sinfer::ChatRole::Tool, "{\"value\":25}");
    ...
    const sinfer::GenerationResult second_replay =
        engine.generate(engine.prepare(input_with_history(2, true)), options(true));
    if (second_replay.generated_token_ids.size() != 4 ||
        second_replay.prefix_reuse_path != sinfer::PrefixReusePath::RestoreResponseCheckpoint ||
    ...
    const sinfer::GenerationResult mode_change =
        engine.generate(engine.prepare(input_with_history(2, false)), options(true));
    if (mode_change.generated_token_ids.size() != 4 ||
        mode_change.prefix_reuse_path != sinfer::PrefixReusePath::RestoreResponseCheckpoint ||
...
int exercise_vision(sinfer::Engine& engine) {
    ...
    auto assistant_message = [](const sinfer::GenerationResult& result) {
        sinfer::ChatMessage message;
        message.role              = sinfer::ChatRole::Assistant;
        message.reasoning_content = result.reasoning;
        message.parts.push_back(sinfer::MessagePart{
            .kind = sinfer::MessagePartKind::Text, .text = result.content, .media = {}});
        return message;
    };
    ...
    auto followup_input = [&](const std::vector<std::uint8_t>& bytes,
                              const sinfer::GenerationResult& first) {
        sinfer::PromptInput input = first_input(bytes);
        input.messages.push_back(assistant_message(first));
        sinfer::ChatMessage followup;
        followup.role = sinfer::ChatRole::User;
        followup.parts.push_back(sinfer::MessagePart{
            .kind = sinfer::MessagePartKind::Text, .text = "Give one more detail.", .media = {}});
        input.messages.push_back(std::move(followup));
        return input;
    };
    auto appended_media_input =
        [&](const std::vector<std::uint8_t>& old_bytes, const sinfer::GenerationResult& first,
            const sinfer::GenerationResult& second, const std::vector<std::uint8_t>& new_bytes) {
            sinfer::PromptInput input = followup_input(old_bytes, first);
            input.messages.push_back(assistant_message(second));
            sinfer::ChatMessage followup;
            followup.role = sinfer::ChatRole::User;
            followup.parts.push_back(image_part(new_bytes, "second.ppm"));
            followup.parts.push_back(sinfer::MessagePart{
                .kind = sinfer::MessagePartKind::Text, .text = "Compare the images.", .media = {}});
            input.messages.push_back(std::move(followup));
            return input;
        };
    ...
    sinfer::RequestOptions cross_chunk_options            = options(false);
    cross_chunk_options.execution.requested_output_tokens = 1;
    const sinfer::GenerationResult cross_chunk =
    ...
    if (!first.prompt.has_media || first.generated_token_ids.size() != 2 ||
        first.finish_reason != sinfer::FinishReason::OutputLimit) {
    ...
    const sinfer::GenerationResult appended = engine.generate(
    ...
    const sinfer::GenerationResult baseline = engine.generate(
        engine.prepare(appended_media_input(image_bytes, first, reused, second_image)),
        options(false));
    ...
    const sinfer::GenerationResult miss = engine.generate(
    ...
    sinfer::RequestOptions mtp_options            = options(false);
    mtp_options.execution.requested_output_tokens = 5;
    const sinfer::GenerationResult mtp_baseline =
    ...
    sinfer::RequestOptions stop_options = mtp_options;
    ...
    const sinfer::GenerationResult stopped_reuse =
    ...
    const std::vector<sinfer::TokenId> visual_prefix{248045, 846, 198, 248053};
    sinfer::RequestOptions source_options            = options(false);
    source_options.execution.requested_output_tokens = 1;
    const sinfer::GenerationResult bridge_source =
        engine.generate(engine.prepare_tokens(visual_prefix), source_options);
    sinfer::RequestOptions bridge_options            = options(true);
    bridge_options.execution.requested_output_tokens = 5;
    const sinfer::GenerationResult visual_bridge =
    ...
    sinfer::RequestOptions bridge_baseline_options       = bridge_options;
    bridge_baseline_options.execution.allow_prefix_reuse = false;
    const sinfer::GenerationResult visual_bridge_baseline =
...
int main() {
    const char* groupwise = std::getenv("SINFER_QWEN3_6_27B_WEIGHTS");
    const char* nvfp4     = std::getenv("SINFER_QWEN3_6_27B_NVFP4_WEIGHTS");
    if ((groupwise == nullptr || *groupwise == '\0') && (nvfp4 == nullptr || *nvfp4 == '\0')) {
        std::cout << "skip: neither SINFER_QWEN3_6_27B_WEIGHTS nor "
                     "SINFER_QWEN3_6_27B_NVFP4_WEIGHTS is set\n";
        return 77;
    }
