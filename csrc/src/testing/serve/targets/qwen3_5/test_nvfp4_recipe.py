...
from surogate.serve.convert.qwen3_5.exports import convert_nvfp4_mixed_bf16 as convert_nvfp4
from surogate.serve.convert.qwen3_5.exports import recipe_nvfp4_mixed_bf16 as recipe

inventory = recipe.EXPORT
...
def test_closed_allocation_inventory_and_site_coverage():
    ...
    assert recipe.EARLY_ATTENTION_INPUT_LAYERS == EARLY_INPUT
    assert recipe.NVFP4_ATTENTION_INPUT_LAYERS == NVFP4_INPUT
    assert recipe.BF16_ATTENTION_OUTPUT_LAYERS == BF16_OUTPUT
    assert recipe.NVFP4_ATTENTION_OUTPUT_LAYERS == NVFP4_OUTPUT
    assert inventory.GDN_LAYERS == GDN
    assert recipe.BF16_GDN_OUTPUT_LAYERS == (4,)
