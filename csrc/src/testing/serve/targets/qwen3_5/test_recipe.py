...
RECIPE_SPECS = recipe.build_recipes(GEOMETRY)
...
def test_recipe_exactly_covers_inventory() -> None:
    assert len(RECIPE_SPECS) == 1118
    assert len(RECIPES_BY_NAME) == 1118
    assert tuple(item.object_name for item in RECIPE_SPECS) == tuple(
        item.name for item in EXPORT.TENSOR_SPECS
    )
    assert all(
        recipe.expression_shape(item.expression)
        == EXPORT.TENSOR_SPECS[index].shape
        for index, item in enumerate(RECIPE_SPECS)
    )

    requirements = recipe.source_requirements(RECIPE_SPECS)
    assert len(requirements) == 1199
    assert {requirement.dtype for requirement in requirements.values()} == {"BF16"}
    assert requirements["model.embed_tokens.weight"].shape == (248320, 5120)
...
def test_fused_mtp_attention_recipe_materializes_runtime_row_order() -> None:
    ...
    fused = recipe.materialize_recipe(
        RECIPES_BY_NAME["mtp/layer/attention/query_key_gate_value"],
...
def test_fused_gdn_value_z_recipe_materializes_runtime_row_order() -> None:
    qkv_name = "model.layers.0.linear_attn.in_proj_qkv.weight"
    z_name = "model.layers.0.linear_attn.in_proj_z.weight"
    qkv = (
    ...
    fused = recipe.materialize_recipe(
        RECIPES_BY_NAME["text/layers/0/gdn/value_z"],
...
def test_representative_transforms_materialize_without_full_weights() -> None:
    ...
    convolution_name = "model.layers.0.linear_attn.conv1d.weight"
    convolution = recipe.materialize_recipe(
        RECIPES_BY_NAME["text/layers/0/gdn/convolution"],
    ...
    materialized_ids = recipe.materialize_recipe(
        RECIPES_BY_NAME["text/draft_head_token_ids"],
        TensorReader({}),
