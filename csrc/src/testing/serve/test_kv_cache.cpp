...
plan_paged_cache(std::uint32_t pages, std::uint32_t logical_pages, std::int32_t rows,
                 std::vector<sinfer::PagedKVPlaneSpec> planes,
                 sinfer::PagedKVPlaneOrder order = sinfer::PagedKVPlaneOrder::PageMajor) {
    sinfer::LayoutBuilder builder;
    auto layout = sinfer::plan_paged_kv_pool(builder, {.page_group_count      = pages,
...
int expect_device_page_ids(const sinfer::Tensor& row, std::initializer_list<std::int32_t> expected,
                           const char* label) {
...
int expect_zeroed_pages(sinfer::PagedKVPool& pool, sinfer::PagedKVPlaneOrder order,
                        std::span<const std::int32_t> pages, cudaStream_t stream,
                        const char* label) {
    const sinfer::Tensor& plane = pool.plane(0);
    ...
    for (const std::int32_t page : pages) {
        if (order == sinfer::PagedKVPlaneOrder::PageMajor) {
...
int main() {
    ...
    auto paged_plan = plan_paged_cache(10, 10, 2,
                                       {{sinfer::DType::I8, 64, 2},
                                        {sinfer::DType::I8, 64, 2},
                                        {sinfer::DType::FP16, 1, 2},
                                        {sinfer::DType::FP16, 1, 2}});
    sinfer::DeviceArena paged_arena(paged_plan.bytes);
    sinfer::PagedKVPool paged_pool({paged_arena.base(), paged_arena.capacity()}, paged_plan.layout);
    ...
    const std::size_t expected_payload =
        2 * sinfer::Tensor(nullptr, sinfer::DType::I8, {64, 64, 2, 10}).bytes() +
        2 * sinfer::Tensor(nullptr, sinfer::DType::FP16, {1, 64, 2, 10}).bytes();
    failures +=
    ...
    failures += expect_zeroed_pages(paged_pool, sinfer::PagedKVPlaneOrder::PageMajor,
                                    selected_pages, ctx.stream, "page-major selective zero");

    auto head_major_plan = plan_paged_cache(10, 10, 1, {{sinfer::DType::BF16, 128, 8}},
                                            sinfer::PagedKVPlaneOrder::HeadMajor);
    sinfer::DeviceArena head_major_arena(head_major_plan.bytes);
    sinfer::PagedKVPool head_major_pool({head_major_arena.base(), head_major_arena.capacity()},
                                        head_major_plan.layout);
    failures += check_shape(head_major_pool.plane(0), {128, 64, 10, 8}, "head-major paged plane");
    failures += expect_zeroed_pages(head_major_pool, sinfer::PagedKVPlaneOrder::HeadMajor,
    ...
    const sinfer::PagedKVResize claim[]  = {
        {.allocation = &allocation_c, .mapped_pages = 2, .page_entitlement = 5}};
    sinfer::resize_paged_kv_bundle(claim);
    allocation_c.bind_row(0);
    ...
    auto main_plan    = plan_paged_cache(4, 4, 1, {{sinfer::DType::BF16, 16, 1}});
    auto backend_plan = plan_paged_cache(2, 2, 1, {{sinfer::DType::BF16, 16, 1}});
    sinfer::DeviceArena main_arena(main_plan.bytes);
    sinfer::DeviceArena backend_arena(backend_plan.bytes);
    sinfer::PagedKVPool main_pool({main_arena.base(), main_arena.capacity()}, main_plan.layout);
    sinfer::PagedKVPool backend_pool({backend_arena.base(), backend_arena.capacity()},
                                     backend_plan.layout);
    const sinfer::PagedKVReservation impossible_bundle[] = {
        {.pool = &main_pool, .page_entitlement = 3},
        {.pool = &backend_pool, .page_entitlement = 3},
    };
    try {
        auto unused = sinfer::reserve_paged_kv_bundle(impossible_bundle);
        (void)unused;
        ++failures;
