...
int main() {
    ...
    const std::string muse = " to=weather<|message|><atem:function_calls>\n<atem:invoke name=\"weather\">\n<atem:parameter name=\"city\">Paris</atem:parameter>\n</atem:invoke>\n</atem:function_calls>";
    assert(accepts(ToolCallFormat::MuseAtem, muse));
    assert(!accepts(ToolCallFormat::MuseAtem, " to=user<|message|>Hello!"));
    assert(!accepts(ToolCallFormat::MuseAtem, muse + "<|eom|><|start|>assistant" + muse));
    assert(accepts(ToolCallFormat::MuseAtem, " to=self<|message|>Checking.<|eom|><|start|>assistant" + muse, true));
    auto wrong_city = muse;
    wrong_city.replace(wrong_city.find("Paris"), 5, "Rome");
    assert(!accepts(ToolCallFormat::MuseAtem, wrong_city));
    request.tool_choice.mode = ToolChoiceMode::Required;
    assert(accepts(ToolCallFormat::MuseAtem, muse + "<|eom|><|start|>assistant" + muse));
    ...
    request.parallel_tool_calls = false;
    assert(accepts(ToolCallFormat::MuseAtem, muse + "<|eom|><|start|>assistant" + muse));
    assert(accepts(ToolCallFormat::MuseAtem, muse + "<|eom|><|start|>assistant to=user<|message|>Done."));
    assert(accepts(ToolCallFormat::MuseAtem, " to=user<|message|>Example: <atem:invoke>not a call</atem:invoke>"));
