...
**Decide this before picking a teacher.** The transplant is a workaround with real costs, not a transparent flag: the model you train (and ship, unless you transplant back) carries the *teacher's* tokenizer and chat template — which changes your serving setup and tokens-per-word economics — and it starts training with approximated vocabulary rows that the KD run must heal before net gains appear (budget a few hundred million tokens; see the tied-embedding note below). Transplanting back afterwards is a second approximation plus a short healing SFT. With a large token budget these costs amortize (this is how production cross-family distillations are made); on a small fine-tuning budget, a weaker same-family teacher is often the better trade.
...
- **Cross-tokenizer teachers require vocabulary transplant.** The trainer itself only works when teacher and student share a vocabulary: sidecars store teacher token ids that directly index the student's LM head, with no translation layer. Same-family pairs (Qwen3-32B → Qwen3-0.6B) work out of the box. A different-tokenizer teacher is supported via [transplant](#cross-tokenizer-distillation) — but that is a project decision, not a config flag: it modifies the student, spends part of the KD budget on healing the approximated vocabulary rows, and changes the shipped tokenizer (and its chat template and tokens-per-word economics) unless you transplant back — a second lossy step plus a healing SFT. Capture validates that every shard token id is below the teacher's vocab size, but that is a necessary check, not a sufficient one — pointing capture at a different-tokenizer teacher without transplanting first produces garbage supervision.
