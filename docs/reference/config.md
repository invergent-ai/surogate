...
* **Each row's forward**: position ids restart at 0 at every row, which the engine turns into document masking (FlashAttention varlen for sliding-window layers, the mem-efficient kernel for wide global heads). Linear-attention layers (Qwen3.5-family Gated DeltaNet) mix tokens outside attention too, and restart at every document: the causal convolution reads zeros before a document's first token, and the gated delta rule runs each document as a sequence of its own from a zero recurrent state, so a row computes what it computes at the start of a padded window (bit-identical per-row losses on Qwen3.5-0.8B, 32 rows, bf16). Every other op is per token and the micro-batch shape (`per_device_train_batch_size × sequence_len`) is the padded one, so the same kernels run on the same rows: with `recipe: bf16` a row's per-token losses were bit-identical packed and padded in every measurement (Gemma 4 MoE fixtures and a real-weight truncation, RTX 5090).
* **Each row's backward, per token**: activation gradients do not depend on where the row sits in its window (the autodiff accumulation adds round to nearest, and the deterministic attention backward uses a split count fixed by the batch geometry, not by the number of documents); measured bit-identical.
...
Under `fp8_hybrid`, per-tensor quantisation scales span the whole micro-batch, so by default a row's arithmetic also depends on the other rows of its window (as it would with `per_device_train_batch_size > 1`). Set `fp8_pow2_scales: true` with row packing: scales rounded down to powers of two only shift exponents, so an element quantises to the same bits whatever the window's maximum, except elements that fall into the FP8 subnormal range because another row raised the maximum (more than ~2^14 below it for E4M3, ~2^29 for E5M2). Measured on a real-weight Gemma 4 truncation: 31 of 32 rows' losses bit-identical to padded training with the same setting, the other within 0.005 nats.

Requirements and limits: a padded shard (`sample_packing: false`), `doc_masking` on (the default), `lora_dropout: 0` (dropout masks are drawn by buffer position) and zero MoE router aux/z loss coefficients for row-for-row equivalence (they are computed over each micro-step's tokens; the config warns). Not supported with `sequence_chunks`, `parallelism: dispatch_pp`, `ep_size > 1`, `train_vision` or multi-node training. CUDA graphs are disabled because the micro-step count varies per step. At most 64 rows share a window.
...
| `doc_masking`           | bool   | `true`         | Document-level masking so packed samples cannot see each other: attention, and the convolution and recurrent state of linear-attention (Gated DeltaNet) layers. |
...
| `long_context`             | bool | `false` | Tile dense SwiGLU MLPs during forward and backward, plus GLM's routed/shared experts in resident BF16 training with `ep_size: 1`. Bounds wide intermediates by the tile size; routing and residual buffers remain proportional to context. Requires `lora_dropout: 0`; eligible surrounding ops can still use CUDA graphs. See [long context](../guides/long-context.md). |
