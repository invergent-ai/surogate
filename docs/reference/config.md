...
* **Each row's forward**: position ids restart at 0 at every row, which the engine turns into document masking (FlashAttention varlen for sliding-window layers, the mem-efficient kernel for wide global heads). Every other op is per token and the micro-batch shape (`per_device_train_batch_size × sequence_len`) is the padded one, so the same kernels run on the same rows: with `recipe: bf16` a row's per-token losses were bit-identical packed and padded in every measurement (Gemma 4 MoE fixtures and a real-weight truncation, RTX 5090).
