...
export interface Trainability {
  supported: boolean;
  known: boolean; // did we have enough config to judge? false = unknown, not "no"
  family: Family | null;
  moe: boolean;
  vision: boolean;
...
export function checkTrainable(architectures: string[] | undefined, modelType: string | undefined): Trainability {
  ...
  if (!fam) {
    // No family matched. If we never got an arch/model_type (e.g. a LoRA adapter
    // repo, or HF didn't return config), we genuinely DON'T KNOW — say so instead
    // of falsely claiming "not supported" (it may well train).
    const hasInfo = (architectures?.length ?? 0) > 0 || !!modelType;
    ...
    return {
      ...
      moe: false,
      vision: false,
      recipes: hasInfo ? [] : ALL_RECIPES,
      reason: hasInfo
        ? `${what} — not in surogate's supported architectures`
        : "couldn't read this model's config — it may still be trainable",
    };
  }
  return {
    supported: true,
    ...
    vision: !!fam.vision,
    recipes: ALL_RECIPES,
    // no "— trainable" suffix: the badge already says "✓ trainable by surogate",
    // so repeating it read as one merged line ("Qwen3 — trainable…surogate").
    reason: `${fam.label}${fam.moe ? " · MoE" : ""}${fam.vision ? " · vision" : ""} model`,
  };
}
