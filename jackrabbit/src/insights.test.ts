...
test("alerts: hot gpu flagged; nominal otherwise", () => {
  ...
  assert.ok(computeInsights(hot, Date.now()).alerts.some((i) => /hot/.test(i.text) && i.color === "red"));
  const cool = load('{"type":"gpu","gpu_id":0,"step":5,"ts":1,"temperature":55,"gpu_util":90,"mem_util":50}');
  assert.ok(computeInsights(cool, Date.now()).alerts.some((i) => /nominal/.test(i.text)));
});

test("tips: bf16 pool suggests fp8; facts include model", () => {
  const s = load('{"type":"config","ts":0,"recipe":"bf16","model":"Qwen/Qwen3-0.6B"}');
  const g = computeInsights(s, Date.now());
  assert.ok(eligibleTips(s).some((t) => /fp8/.test(t)));
  assert.ok(g.facts.some((i) => /Qwen\/Qwen3-0\.6B/.test(i.text)));
});

import { computeTips, eligibleTips, activeTags } from "./insights.ts";

test("tips: context filter + rotation", () => {
  const s = load('{"type":"config","ts":0,"recipe":"bf16","model":"Qwen/Qwen3-0.6B","lora":true,"sequence_len":2048}');
  const tags = activeTags(s);
  assert.ok(tags.has("bf16") && tags.has("lora"));
  assert.ok(!tags.has("nvfp4"));
  const pool = eligibleTips(s);
  assert.ok(pool.length > 20); // general + bf16/lora tips
  // rotation advances with time
  const a = computeTips(s, 0, 2).map((x) => x.text).join("|");
  const b = computeTips(s, 100000, 2).map((x) => x.text).join("|");
  assert.notEqual(a, b);
});

test("tips: nvfp4/moe tips only when relevant", () => {
  const sft = load('{"type":"config","ts":0,"recipe":"bf16","model":"Qwen/Qwen3-0.6B","lora":true}');
  assert.ok(!eligibleTips(sft).some((t) => /NVFP4/.test(t)));
  const moe = load('{"type":"config","ts":0,"recipe":"nvfp4","model":"Qwen/Qwen3-30B-A3B","lora":true}');
  assert.ok(eligibleTips(moe).some((t) => /expert/.test(t))); // moe tip surfaces
});
