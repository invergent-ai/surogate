...
OUT=output/fugu_ultra_frontier_paper; LOG=$OUT/process_logs; mkdir -p "$LOG"; : > "$LOG/launch.log"
rm -rf $OUT/run_default $OUT/step_* $OUT/checkpoints $OUT/rollouts $OUT/broadcasts 2>/dev/null
.venv/bin/surogate grpo-infer "$D/infer_frontier_paper.yaml" > "$LOG/infer.log" 2>&1 &
...
.venv/bin/surogate grpo-orch "$D/orch_frontier_paper.yaml" > "$LOG/orch.log" 2>&1 &
echo "orch pid $!" >> "$LOG/launch.log"
CUDA_VISIBLE_DEVICES=1 .venv/bin/surogate grpo-train "$D/train_frontier_paper.yaml" > "$LOG/train.log" 2>&1 &
echo "train pid $!" >> "$LOG/launch.log"
echo "ORCH_TRAIN_LAUNCHED" >> "$LOG/launch.log"; wait
