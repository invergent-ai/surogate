...
OUT=output/fugu_ultra_singleturn; LOG=$OUT/process_logs; mkdir -p "$LOG"; : > "$LOG/launch.log"
...
.venv/bin/surogate grpo-orch "$D/orch_singleturn.yaml" > "$LOG/orch.log" 2>&1 &
echo "orch pid $!" >> "$LOG/launch.log"
CUDA_VISIBLE_DEVICES=1 .venv/bin/surogate grpo-train "$D/train_singleturn.yaml" > "$LOG/train.log" 2>&1 &
