...
CKP=output/fugu_ultra_paper/step_00000110
TRAIN_PID=2906828
while [ ! -d "$CKP" ]; do
  kill -0 "$TRAIN_PID" 2>/dev/null || { echo "ALERT: trainer pid $TRAIN_PID DIED before step 110 ($(date +%H:%M:%S))"; exit 1; }
  sleep 180
done
...
.venv/bin/python scratch_eval_live_throttled.py --label step110_fullmix --conc 3 --n 60
echo "==== trend tail ===="
