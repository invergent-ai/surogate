...
class RowPreprocessor:
    standard_keys = (
        ...
        + list(
            chain.from_iterable(
                [f"{prefix}_{k}" for k in _pair_keys] for prefix in ["rejected", "positive", "negative"]
            )
        )
        + [
            "rejected_response",
    ...
    def __init__(
        ...
        *,
        ...
        random_state: np.random.RandomState | int | None = RAND_SEED,
    ...
    def batched_preprocess(
        self, batched_row: dict[str, Any], *, strict: bool, ignore_max_length_error: bool
    ) -> dict[str, Any]:
    ...
    def _patch_arrow_writer():
        ...
        def _new_init(self, schema=None, features=None, *args, **kwargs):
            if features is not None:
                messages_feature = Sequence(
                    feature={
                        "role": Value(dtype="string"),
                        "content": Value(dtype="string"),
                    }
                )
                messages_feature_with_loss = Sequence(
                    feature={
                        "role": Value(dtype="string"),
                        "content": Value(dtype="string"),
    ...
    def _check_rejected_response(row: dict[str, Any]) -> None:
        if "rejected_response" in row:
            ...
            if (
                ...
                or isinstance(rejected_response, str)
                and rejected_response == messages[-1]["content"]
            ):
