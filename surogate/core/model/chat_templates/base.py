...
from surogate.core.model.utils import Prompt, Word

CHAT_TEMPLATE_MAPPING: Dict[str, 'ChatTemplate'] = {}
...
class ChatTemplate:
    ...
    def to_generate_chat_template(self) -> 'ChatTemplate':
        copied = deepcopy(self)
        return ChatTemplate(
            copied.template_type,
            prefix=[],
            prompt=['{{QUERY}}'],
            chat_sep=None,
            template_processor_cls=copied.template_processor_cls,
            auto_add_bos=True,
            stop_words=copied.stop_words,
        )
...
def register_chat_template(chat_template: 'ChatTemplate', *, exist_ok: bool = False) -> None:
    template_type = chat_template.template_type
    if not exist_ok and template_type in CHAT_TEMPLATE_MAPPING:
        raise ValueError(f'The `{template_type}` has already been registered in the TEMPLATE_MAPPING.')
    CHAT_TEMPLATE_MAPPING[template_type] = chat_template