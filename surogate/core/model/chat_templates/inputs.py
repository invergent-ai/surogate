...
from surogate.core.model.chat_templates.utils import Messages, Tool, messages_to_history
...
class StdChatTemplateInputs:
    ...
    def from_dict(cls, inputs: Dict[str, Any]) -> 'StdTemplateInputs':
        ...
        media_kwargs = StdChatTemplateInputs.remove_messages_media(messages)
        ...
        all_keys = set(f.name for f in fields(StdChatTemplateInputs))
...
@dataclass
class ChatTemplateInputs:
    chosen: StdChatTemplateInputs  # or Dict[str, Any]
    rejected: Optional[StdChatTemplateInputs] = None
    positive: List[StdChatTemplateInputs] = field(default_factory=list)  # or Dict[str, Any]
    negative: List[StdChatTemplateInputs] = field(default_factory=list)
    ...
    def __post_init__(self):
        ...
        for key in ['chosen', 'rejected', 'positive', 'negative']:
            ...
            if key in {'chosen', 'rejected'}:
                setattr(self, key, StdChatTemplateInputs.from_dict(value_dict))
            else:
                res = []
                for i in range(len(value_dict['messages'])):
                    ...
                    for k in all_keys:
                        ...
                        if val is None:
                            continue
                        kwargs[k] = val[i]
                    res.append(StdChatTemplateInputs.from_dict(kwargs))
    ...
    @classmethod
    def from_dict(cls, inputs: Dict[str, Any]) -> 'ChatTemplateInputs':
        inputs = deepcopy(inputs)
