...
class CustomAoFloat8Linear(Float8Linear):
...
def custom_convert_to_float8_training(
    ...
    from_float = lambda m: CustomAoFloat8Linear.from_float(
        m,
        config=config,
    )

    def filter_lora_layers(module, fqn: str) -> bool:
        layers_to_ignore = ["lora_A", "lora_B", "lm_head"]
...
def w8a8_block_fp8_matmul_triton(
    ...
    if should_use_persistent:
        ...
        _w8a8_block_fp8_matmul_persistent[grid](
            ...
            M, N, K,
            block_n, block_k,
            A.stride(-2), A.stride(-1), B.stride(1), B.stride(0), C.stride(-2), C.stride(-1), As.stride(-2),
            As.stride(-1), Bs.stride(1), Bs.stride(0),
...
def _w8a8_block_fp8_matmul_persistent(
        ...
        stride_am, stride_ak, stride_bk, stride_bn, stride_cm, stride_cn, stride_As_m, stride_As_k, stride_Bs_k,
        stride_Bs_n,
        # Meta-parameters
...
try:
    ...
    try:
        ...
        if "cutlass cannot initialize" in e.lower():
            logger.warning_once(
                f"FBGEMM on the current GPU cannot load - will switch to Triton kernels"
            )
        else:
            logger.warning_once(
                f"FBGEMM on the current GPU cannot load with error = {e} - will switch to Triton kernels"
            )
        has_fbgemm = False
    del block_scale, xq
...
try:
    if (
        ...
        import fbgemm_gpu.experimental
    ...
    logger.warning(
        "Could not find fbgemm_gpu.experimental.gemm.triton_gemm.fp8_gemm.triton_quantize_fp8_block"
    )
