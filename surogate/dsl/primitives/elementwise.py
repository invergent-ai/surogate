...
def mask_scatter(x: Tensor[B, T, C], mask: Tensor[B, T, int32], src: Tensor[B * T, C]) -> Tensor[B, T, C]:
    """Replace rows in x at masked positions with src."""
    ...


@primitive(impl="kernels.deepstack_inject")
def deepstack_inject(x: Tensor[B, T, C], mask: Tensor[B, T, int32], src: Tensor[B * T, C]) -> Tensor[B, T, C]:
