...
def mamba_conv1d(
    ...
    *,
    ...
    Args:
        x: Input tensor [B, T, D_conv] where D_conv = intermediate_size + 2 * n_groups * ssm_state_size
...
def mamba_conv1d_backward(
    d_out: Tensor["B", "T", "D_conv"],
    x: Tensor["B", "T", "D_conv"],
    weight: Tensor["D_conv", "K"],
) -> tuple[Tensor["B", "T", "D_conv"], Tensor["D_conv", "K"], Tensor["D_conv"] | None]:
...
def mamba_ssm_scan(
    ...
    *,
    ...
    chunk_size: int = 256,
) -> tuple[Tensor["B", "T", "H", "D"], Tensor["B", "H", "D", "N"]]:
    ...
    Args:
        hidden_states: Input hidden states [B, T, num_heads, head_dim]
        dt: Time step / delta [B, T, num_heads]
        A: State decay (typically negative log of learned parameter) [num_heads]
        B: Input-to-state projection [B, T, n_groups, ssm_state_size]
        C: State-to-output projection [B, T, n_groups, ssm_state_size]
    ...
    Returns:
        (output, final_state):
            output: SSM output [B, T, num_heads, head_dim]
...
def mamba_ssm_scan_backward(
    ...
    Tensor["B", "T", "H", "D"],  # d_hidden_states
    Tensor["B", "T", "H"],       # d_dt
    Tensor["H"],                  # d_A
    Tensor["B", "T", "G", "N"],  # d_B
    Tensor["B", "T", "G", "N"],  # d_C
...
def mamba_split_proj(
    ...
    *,
    ...
    Tensor["B", "T", "I"],      # gate
    Tensor["B", "T", "D_conv"], # hidden_states_B_C (for conv)
    Tensor["B", "T", "H"],      # dt
]:
    ...
    - hidden_states_B_C: input to conv + SSM
    - dt: time step deltas
    ...
    Args:
        ...
        num_heads: Number of heads (size of dt)
...
def mamba_split_conv_out(
    conv_out: Tensor["B", "T", "D_conv"],
    *,
    intermediate_size: int,
    groups_state_size: int,
) -> tuple[
    Tensor["B", "T", "I"],  # hidden_states
    Tensor["B", "T", "GN"], # B (input-to-state)
    Tensor["B", "T", "GN"], # C (state-to-output)
]:
    """Split convolution output into hidden states, B, and C.

    Args:
        conv_out: Convolution output [B, T, conv_dim]
