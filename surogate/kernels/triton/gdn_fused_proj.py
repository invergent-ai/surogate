...
#
# Adapted from SGLang Triton JIT kernels:
# study/sglang/python/sglang/jit_kernel/triton/gdn_fused_proj.py
...
def compile_gdn_fused_proj(
    ...
    """Compile GDN fused projection kernels (contiguous variant for Qwen3.5).

    Produces a single Triton kernel that replaces the post-matmul
    split/reshape/cat sequence in the linear-attention block.

    Args:
        num_heads_qk: Number of QK head groups.
        num_heads_v: Number of V heads.
        head_qk: Per-head QK dimension.
        head_v: Per-head V dimension.
        output_dir: Directory for cubin + manifest output.
        sm: Target SM version (auto-detected if None).

    Returns:
        Dict mapping kernel names to manifest JSON paths.
    """
    from surogate.kernels.compiler import compile_triton_kernel

    manifests: dict[str, str] = {}

    # The contiguous variant is what Surogate uses — Qwen3.5 stores qkv and z
    # as separate weights, so after matmul the layout is contiguous [q|k|v|z].
    manifests["gdn_fused_proj_contiguous"] = compile_triton_kernel(
        fn=fused_qkvzba_split_reshape_cat_contiguous_kernel,
        signature={
            "mixed_qkv": "*bf16",
            "z": "*bf16",
            "b": "*bf16",
        ...
        kernel_name="gdn_fused_proj_contiguous",
        num_warps=1,
        num_stages=3,
        sm=sm,
    )
