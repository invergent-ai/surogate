...
def parse_directory(
    ...
    if (not isinstance(value, dict) or not _ROOT_MEMBERS <= frozenset(value)
            or not frozenset(value) <= _ROOT_MEMBERS | _ROOT_OPTIONAL):
        raise ArtifactError(
            "directory root must contain exactly identity and objects, plus at most external "
            "and geometry"
        )
