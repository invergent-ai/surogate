"""Build an E-series Gemma 4 serving artifact from a HuggingFace checkpoint.
...
def validate_config(config: Mapping[str, object]) -> inventory.Geometry:
    ...
    # What separates this target from the dense one. A checkpoint with neither is a dense
    # Gemma 4 and belongs to `convert/gemma4/`; converting it here would produce an artifact
    # whose per-layer input objects all resolved away and whose layer schedule named no
    # shared block -- a dense model under an E-series identity, which the engine would load.
    if geometry.per_layer_input_dim <= 0 and geometry.kv_shared_layers <= 0:
        raise ValueError(
            "this checkpoint has neither per-layer input embeddings nor shared key/value "
            "layers; it is a dense Gemma 4 and converts with the gemma4 target"
        )
    return geometry
    ...
    *,
    ...
    report = family_conversion.build_conversion_report(
        ...
        config_summary={
            ...
            "per_layer_input_dim": geometry.per_layer_input_dim,
            "kv_shared_layers": geometry.kv_shared_layers,
            "first_shared_layer": geometry.first_shared_layer,
            "shared_kv_intermediate": geometry.shared_kv_intermediate,
        },
...
def _geometry_block(geometry: inventory.Geometry) -> dict[str, float]:
    ...
    return {
        ...
        "per_layer_input_dim": float(geometry.per_layer_input_dim),
        "per_layer_vocab": float(geometry.per_layer_vocab),
        "kv_shared_layers": float(geometry.kv_shared_layers),
        "shared_kv_intermediate": float(geometry.shared_kv_intermediate),
