"""Build a Gemma 4 mixture serving artifact from a HuggingFace checkpoint.

Safetensors in, artifact out -- or a GGUF read where it lies, as on the dense target.
`--gguf-repack` names the map the bridge wrote beside the checkpoint it synthesised,
and the objects that map covers are served from the GGUF's own bytes.
    ...
    *,
    ...
    report = family_conversion.build_conversion_report(
        ...
        config_summary={
            ...
            "expert_intermediate": geometry.expert_intermediate,
...
def load_resources(model_dir: Path) -> tuple[ResourcePayload, ...]:
    ...
    The dense target's note holds here too: Gemma 4 states its chat template in a different
    place at each size and the base checkpoints publish none, which the artifact carries as an
    absence rather than an invented template.
    """
    root = Path(model_dir)
