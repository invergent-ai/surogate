...
def preflight_conversion(model_dir: str | Path) -> ConversionPreflight:
    ...
    export = family_inventory.export_inventory(family_inventory.FP8_BLOCK, geometry)
    with _reader_factory(source)() as reader:
        root = recipe.source_root_of(reader.names)
    recipes = recipe.build(geometry, root)
    expected = {spec.name for spec in export.TEXT_CORE_TENSOR_SPECS
                if spec.format == family_inventory.FP8_BLOCK_FORMAT}
...
def materialize_fp8_block_weight(entry: recipe.Nvfp4WeightRecipe, reader: ShardReader) -> tuple[torch.Tensor, torch.Tensor]:
    """The fused object's codes [rows, k] and scale grid [rows/128, k/128], stacked from its
    components' `weight` / `weight_scale_inv` in the order the parts name."""
    ...
    for part in entry.parts:
        ...
        for rows in part.rows:
            if rows.begin % BLOCK or rows.end % BLOCK:
                raise ValueError(f"{entry.object_name}: row range {rows.begin}:{rows.end} is not whole 128-blocks")
            codes_parts.append(words[0][rows.begin:rows.end])
            scale_parts.append(words[1][rows.begin // BLOCK:rows.end // BLOCK])
    ...
    if tuple(codes.shape) != tuple(entry.shape) or tuple(scales.shape) != (entry.shape[0] // BLOCK, entry.shape[1] // BLOCK):
        ...
        with ArtifactWriter(
            ...
            for index, spec in enumerate(export.OBJECT_SPECS, start=1):
                ...
                if isinstance(spec, export.ResourceSpec):
                    ...
                    codes, scales = materialize_fp8_block_weight(fp8_by_name[spec.name], reader)
                    payload = encode_fp8_block_scaled(codes, scales, spec.shape)
                elif spec.name in direct_by_name:
                    entry = direct_by_name[spec.name]
                    # the GDN control projections ship dense here (the NVFP4 export quantised them)
                    tensor = reader.get(entry.source_name + ".weight" if entry.dequantize else entry.source_name)
