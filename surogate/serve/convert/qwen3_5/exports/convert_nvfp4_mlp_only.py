...
Canonical invocation::
    ...
    python -m surogate.serve.convert.qwen3_5.convert --profile nvfp4-mlp-only \
...
from . import recipe_nvfp4_mlp_only as recipe

from .. import inventory as family_inventory
from .. import recipe as dense_recipe
from .. import convert as family_convert
from .. import draft_head


#: This export exists only for Qwen3.8-27B.
GEOMETRY = family_inventory.GEOMETRY_QWEN3_8
EXPORT = family_inventory.export_inventory(
    family_inventory.NVFP4_MLP_ONLY, GEOMETRY)
...
def _validate_quantized_config(
    ...
    summary = family_convert.validate_config(config)
...
def build_object_plan(
    ...
    preflight_inventory()
    return family_conversion.build_object_plan(EXPORT.OBJECT_SPECS, resources)
...
def preflight_conversion(
    ...
    official_summary = family_convert.validate_config(official_config)
        ...
        with ArtifactWriter(
            ...
            for index, spec in enumerate(EXPORT.OBJECT_SPECS, start=1):
                ...
                if isinstance(spec, EXPORT.ResourceSpec):
                    ...
                    payload = encode_direct(scalar, EXPORT.FP32)
                ...
                del payload
                print(
                    f"[{index}/{len(EXPORT.OBJECT_SPECS)}] {spec.name}",
                    flush=True,
                )
