"""Persistent-object contract for the complete Qwen3.5-2B artifact.
...
def _build_text_core_specs() -> tuple[TensorSpec, ...]:
    ...
    for layer in range(24):
        ...
        specs.append(_tensor(prefix + "input_norm", (2048,), BF16))

        if layer in FULL_ATTENTION_LAYERS:
            specs.extend(
                (
                    _tensor(prefix + "attention/query_key_gate_value", (5120, 2048), W8),
                    _tensor(prefix + "attention/query_norm", (256,), BF16),
                    _tensor(prefix + "attention/key_norm", (256,), BF16),
                    _tensor(prefix + "attention/output", (2048, 2048), W8),
                )
            )
        else:
            specs.extend(
                (
                    ...
                    _tensor(prefix + "gdn/a_projection", (16, 2048), BF16),
                    _tensor(prefix + "gdn/b_projection", (16, 2048), BF16),
                    _tensor(prefix + "gdn/query_key_value_z", (8192, 2048), W8),
                    _tensor(prefix + "gdn/norm", (128,), BF16),
                    _tensor(prefix + "gdn/output", (2048, 2048), W8),
                )
            )
    ...
    specs.extend(
        (
            ...
            _tensor("text/output_head", (248320, 2048), W8),
        )
    )
    return tuple(specs)
...
def _build_draft_head_specs() -> tuple[TensorSpec, ...]:
    return (
        _tensor("text/draft_head", (131072, 2048), W8),
...
def _build_mtp_specs() -> tuple[TensorSpec, ...]:
    return (
        ...
        _tensor("mtp/layer/attention/query_key_gate_value", (5120, 2048), W8),
        _tensor("mtp/layer/attention/query_norm", (256,), BF16),
        _tensor("mtp/layer/attention/key_norm", (256,), BF16),
        _tensor("mtp/layer/attention/output", (2048, 2048), W8),
        _tensor("mtp/layer/post_attention_norm", (2048,), BF16),
        _tensor("mtp/layer/mlp/gate_up", (12288, 2048), W8),
        _tensor("mtp/layer/mlp/down", (2048, 6144), W8),
        _tensor("mtp/final_norm", (2048,), BF16),
    )


def _build_vision_specs() -> tuple[TensorSpec, ...]:
    # This checkpoint's own tower, which the target's binder now consumes.
