# `qwen3_5` Vision parity tool

This tool compares the independent `.sinfer` Python reference with the source BF16 Vision tower at
matching semantic boundaries.

Compare quantized artifact Vision activations with the source BF16 tower:

```bash
python -m tools.parity.qwen3_5.vision \
  --weights out/qwen3_6_27b.sinfer \
  --model-dir /path/to/Qwen3.6-27B/base-hf-bf16 \
  --messages messages.json
```

It captures the tower's first, middle and last block, whatever its depth, and requires an
artifact that ships one.
