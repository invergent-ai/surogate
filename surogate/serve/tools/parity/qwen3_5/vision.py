...
from surogate.serve.tools.reference.common.multimodal import load_messages
from surogate.serve.tools.reference.qwen3_5 import RefModel



def capture_layers(depth: int) -> tuple[int, ...]:
    """The blocks the reference encoder taps: the tower's first, middle and last."""
...
def main() -> None:
    ...
    with RefModel(args.weights, device=device, compile_codec=True) as model, torch.inference_mode():
        if model.vision_config is None:
            parser.error("this artifact has no vision tower to compare")
        captured = capture_layers(model.vision_config.depth)
    ...
    comparisons = {
        ...
        for name in tuple(f"block_{layer:02d}" for layer in captured) + ("merger",)
    }
    report = {
