...
from surogate.serve.artifact.container import Artifact, tensor_payload_bytes
...
def decode(obj, spec):
    ...
    if obj.layout == "ggml-blocks-v1":
        data = run_rows(obj, 1).reshape(-1)
        decoded, offset = [], 0
        for fmt, count in obj.segments or ((obj.format, obj.shape[0]),):
            kind = GGMLQuantizationType[fmt]
            block, width = GGML_QUANT_SIZES[kind]
            row_bytes = obj.shape[1] // block * width
            end = offset + count * row_bytes
            decoded.append(np.asarray(dequantize(data[offset:end].reshape(count, row_bytes), kind),
                                      dtype=np.float32).reshape(count, obj.shape[1]))
            offset = end
        if offset != len(data):
            raise ValueError(f"{obj.name}: segments do not cover source bytes")
        return np.concatenate(decoded)
    payload = bytes(art.payload(obj))
    if obj.format == inv.W8:
    ...
    return decode_direct(payload, obj.format, obj.shape).float().cpu().numpy()
...
def check(name, reference=None, rows=None, tol=None):
    ...
    ok = obj.shape == spec.shape and obj.bytes == tensor_payload_bytes(
        obj.layout, obj.format, obj.shape, obj.segments)
...
for layer in g.full_attention_layers:
    ...
    qr, _, gr, _ = cv.attention_rows_q_k_gate_v(g)
...
for layer in g.gdn_layers:
    ...
    check(f"text/layers/{layer}/gdn/output", cv._untile_v(g, src.float32(b + "ssm_out.weight"), 1), tol=1e-6)
    check(f"text/layers/{layer}/gdn/a_log", np.log(-cv._untile_v_heads(g, src.float32(b + "ssm_a").reshape(g.gdn_value_heads), 0)), tol=1e-6)
    check(f"text/layers/{layer}/gdn/dt_bias", cv._untile_v_heads(g, src.float32(b + "ssm_dt.bias").reshape(g.gdn_value_heads), 0), tol=1e-6)
    check(f"text/layers/{layer}/gdn/norm", src.float32(b + "ssm_norm.weight"), tol=1e-2)
    alpha = cv._untile_v_heads(g, src.float32(b + "ssm_alpha.weight"), 0)
    beta = cv._untile_v_heads(g, src.float32(b + "ssm_beta.weight"), 0)
    check(f"text/layers/{layer}/gdn/a_b_projection", np.concatenate([alpha, beta]), tol=1e-2)
    conv = src.float32(b + "ssm_conv1d.weight")  # channel-major, as ggml holds it
    check(f"text/layers/{layer}/gdn/convolution", np.ascontiguousarray(np.concatenate([conv[:2 * g.key_dim], cv._untile_v(g, conv[2 * g.key_dim:], 0)], axis=0).T), tol=1e-2)
    ...
    fused = np.concatenate([gate, up], axis=1).reshape(-1, g.hidden); del gate, up
    check(f"text/layers/{layer}/mlp/routed_gate_up", fused, rows=[(0, min(64, g.intermediate)), ((g.experts - 1) * 2 * g.intermediate, min(64, g.intermediate))], tol=1e-6)
    del fused
    down = src.float32(b + "ffn_down_exps.weight").reshape(-1, g.intermediate)
    check(f"text/layers/{layer}/mlp/routed_down", down, rows=[(0, min(64, g.hidden)), ((g.experts - 1) * g.hidden, min(64, g.hidden))], tol=1e-6)
    del down
if g.ple_ngram:
    b = f"blk.{g.ple_layer}."
    check(f"text/layers/{g.ple_layer}/ple/key", src.float32(b + "ple_key.weight"), tol=1e-2)
    check(f"text/layers/{g.ple_layer}/ple/value", src.float32(b + "ple_value.weight"), tol=1e-2)
    check(f"text/layers/{g.ple_layer}/ple/norm_key", src.float32(b + "ple_norm_key.weight"), tol=1e-6)
    check(f"text/layers/{g.ple_layer}/ple/norm_conv", src.float32(b + "ple_norm_conv.weight"), tol=1e-6)
    check(f"text/layers/{g.ple_layer}/ple/convolution", src.float32(b + "ple_conv1d.weight").T, tol=1e-2)
...
check("text/output_head", src.float32("token_embd.weight" if g.tied_embeddings else "output.weight"), rows=[(0, min(256, g.vocab)), (g.vocab - min(256, g.vocab), min(256, g.vocab))], tol=1e-6)
check("text/token_embedding", src.float32("token_embd.weight"), rows=[(0, min(256, g.vocab)), (g.vocab - min(256, g.vocab), min(256, g.vocab))], tol=1e-6)
if g.ple_ngram:
