...
def layer_norm(
    x: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor, *, eps: float = VISION_NORM_EPS
) -> torch.Tensor:
    return F.layer_norm(
        x,
        (x.shape[-1],),
...
def vision_position_ids(grid_thw: torch.Tensor, *, merge: int = VISION_SPATIAL_MERGE) -> torch.Tensor:
...
def bilinear_indices_and_weights(grid_thw: torch.Tensor, *, side: int, merge: int) -> tuple[torch.Tensor, torch.Tensor]:
    device = grid_thw.device
...
def interpolate_position_embedding(
    table: torch.Tensor, grid_thw: torch.Tensor, *, merge: int = VISION_SPATIAL_MERGE
) -> torch.Tensor:
    side = isqrt(table.shape[0])
    if side * side != table.shape[0]:
        raise ValueError("vision position table must contain a square grid")
    indices, weights = bilinear_indices_and_weights(grid_thw, side=side, merge=merge)
...
def _vision_inv_freq(device_type: str, device_index: int | None, head_dim: int, theta: float) -> torch.Tensor:
    device = torch.device(device_type, device_index)
    dim = head_dim // 2
    values = torch.arange(0, dim, 2, device=device, dtype=torch.float32)
    return 1.0 / (theta ** (values / dim))
...
def apply_vision_rope(
    q: torch.Tensor, k: torch.Tensor, position_ids: torch.Tensor, *, theta: float = VISION_ROPE_THETA
) -> tuple[torch.Tensor, torch.Tensor]:
    inv = _vision_inv_freq(q.device.type, q.device.index, q.shape[-1], theta)
...
def vision_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, cu_seqlens: torch.Tensor) -> torch.Tensor:
    ...
    while segment + 1 < len(boundaries):
        ...
        run_end = segment + 1
        while run_end + 1 < len(boundaries) and boundaries[run_end + 1] - boundaries[run_end] == length:
            run_end += 1
        begin, end = boundaries[segment], boundaries[run_end]
        batch = run_end - segment

        def heads(x: torch.Tensor) -> torch.Tensor:
            return x[begin:end].reshape(batch, length, q.shape[1], q.shape[2]).transpose(1, 2)

        attended = _sdpa(heads(q), heads(k), heads(v))
        out[begin:end] = attended.transpose(1, 2).reshape(end - begin, q.shape[1], q.shape[2])
        segment = run_end
    return bf16(out)
