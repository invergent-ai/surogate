...
LINEAR_FORMATS = frozenset((BF16, Q4, Q5, Q6, W8))
...
class Layout:
    ...
    def for_layer(self, index: int) -> "Layout":
        return self.layers[index] if self.layers else self
...
def _layout_of(names: frozenset[str], cfg: ModelConfig) -> Layout:
    layers = tuple(
        Layout(
            split_attention=f"text/layers/{i}/attention/query_key" in names,
            gdn_split_qk_vz=f"text/layers/{i}/gdn/query_key" in names,
            gdn_split_qkv_z=f"text/layers/{i}/gdn/query_key_value" in names,
            vision=False,
        )
        for i in range(cfg.layers)
    )
    return Layout(False, False, False, "vision/patch_embedding" in names, layers)


def _geometry(shapes, declared, layer_types) -> ModelConfig:
    """Resolve metadata first; the binding contract separately validates stored shapes."""
    return model_config_from_declared(declared, layer_types=layer_types)


def _vision_geometry(shapes, declared, cfg) -> VisionConfig:
    return vision_config_for(cfg, vision_config_from_declared(declared))


def _text_contract(cfg: ModelConfig, layout: Layout, draft_vocab: int) -> tuple[_ExpectedTensor, ...]:
    tensors: list[_ExpectedTensor] = [_linear("text/token_embedding", (cfg.vocab, cfg.hidden))]
    for layer in range(cfg.layers):
        local = layout.for_layer(layer)
        ...
        if cfg.is_full(layer):
            ...
            tensors.extend(
                (
                    ...
                    _tensor(prefix + "gdn/a_projection", (cfg.gdn_v_heads, cfg.hidden), BF16),
                    _tensor(prefix + "gdn/b_projection", (cfg.gdn_v_heads, cfg.hidden), BF16),
                )
            )
            if local.gdn_split_qk_vz:
                tensors.extend(
                    (
            ...
            elif local.gdn_split_qkv_z:
                tensors.extend(
                    (
                        _linear(prefix + "gdn/query_key_value", (cfg.conv_dim, cfg.hidden)),
...
def _vision_contract(vision: VisionConfig | None) -> tuple[_ExpectedTensor, ...]:
    ...
    tensors: list[_ExpectedTensor] = [
        ...
        _tensor("vision/position_embedding", (vision.position_embeddings, hidden), BF16),
    ]
...
def expected_row_views(cfg: ModelConfig, layout: Layout) -> int:
    ...
    count = 6 if cfg.mtp_layers else 0
    for i in range(cfg.layers):
        local = layout.for_layer(i)
        count += 2
        count += (4 if local.split_attention else 6) if cfg.is_full(i) else (6 if local.gdn_fused else 4)
    return count
...
def _validate_inventory(artifact: Artifact, contract: tuple[_ExpectedObject, ...]) -> None:
    if artifact.identity.architecture != "qwen3_5":
        raise BindingError("artifact architecture is not qwen3_5")
    if len(artifact.objects) != len(contract):
        raise BindingError(f"artifact has {len(artifact.objects)} objects; expected {len(contract)}")
    ...
    for actual in artifact.objects:
        ...
        if isinstance(expected, _ExpectedTensor):
            ...
            if actual.runs:
                raise BindingError(f"{actual.name}: this reference requires inline BF16/groupwise weights")
            layout_name = CONTIGUOUS if format_name == BF16 else expected.layout
            signature = (expected.name, expected.shape, format_name, layout_name)
            if (
                actual.name,
                actual.shape,
                actual.format,
                ...
                raise BindingError(f"object {actual.name!r} does not match tensor signature {signature!r}")
        else:
            ...
            if (
                ...
                raise BindingError(f"object {actual.name!r} does not match resource signature {signature!r}")
...
def _row_view(
    ...
    if block.layout not in (ROW_SPLIT, CONTIGUOUS) or len(block.shape) != 2:
...
class ArtifactBinding:
    ...
    def __init__(self, artifact: Artifact, *, owns_artifact: bool = False):
        shapes = {obj.name: obj.shape for obj in artifact.objects if isinstance(obj, TensorObject)}
        names = frozenset(shapes)
        declared = getattr(artifact, "geometry", None)
        self.config = _geometry(shapes, declared, artifact.layer_types)
        cfg = self.config
        ...
        self.draft_vocab = cfg.draft_vocab
        self.vision_config = (
            _vision_geometry(shapes, getattr(artifact, "vision_geometry", None), cfg)
            if artifact.vision_geometry
            else None
        )
        self.contract = object_contract(cfg, self.layout, self.draft_vocab, self.vision_config)
        ...
        if not required <= present:
            raise BindingError(f"missing frontend resources: {sorted(required - present)}")
        self.contract = tuple(
            spec
            for spec in self.contract
            if not (isinstance(spec, _ExpectedResource) or spec.name == "text/output_head") or spec.name in present
        )
        ...
        for layer in range(cfg.layers):
            prefix = f"text/layers/{layer}/"
            local = self.layout.for_layer(layer)
            ...
            if cfg.is_full(layer):
                if local.split_attention:
                    ...
                    gate_value = _row_view(parent, pair, 2 * pair, (pair, hidden), row_views)
                ...
                if local.gdn_split_qk_vz:
                    ...
                    qk_view = _row_view(qkv_parent, 0, 2 * key_dim, (2 * key_dim, hidden), row_views)
                    vz_view = None
                    ...
                    qk_view = _row_view(parent, 0, 2 * key_dim, (2 * key_dim, hidden), row_views)
                    vz_view = _row_view(
                        parent,
                        2 * key_dim,
        ...
        self.vision = self._bind_vision(blocks, self.vision_config) if self.vision_config is not None else None
        self.row_views = tuple(row_views)
        self.axis_views = tuple(axis_views)
        expected_tensors = sum(isinstance(spec, _ExpectedTensor) for spec in self.contract)
        if len(self.tensors) != expected_tensors:
            raise RuntimeError(f"incomplete typed binding: {len(self.tensors)} of {expected_tensors} tensors")
        expected_views = expected_row_views(cfg, self.layout)
        if len(self.row_views) != expected_views:
            raise RuntimeError(f"incomplete typed binding: {len(self.row_views)} of {expected_views} row views")
    ...
    @staticmethod
    def _bind_vision(blocks: dict[str, PhysicalBlock], vision: VisionConfig) -> VisionBinding:
    ...
    def _validate_aliases(self) -> None:
        ...
        for layer in self.text.layers:
            ...
            if view.block is not layer.gdn.convolution_storage or view.axes != (1, 0) or view.shape != expected:
    ...
    def _validate_draft_ids(self) -> None:
        ...
        if token_ids.dtype != torch.int32 or tuple(token_ids.shape) != (self.draft_vocab,):
