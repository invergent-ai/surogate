"""Command-line reference inference over a native SInfer artifact."""
...
from surogate.serve.tools.reference.common.multimodal import MultimodalBatch, load_messages
from surogate.serve.tools.reference.common.sampling import Sampler, SamplingConfig
from surogate.serve.tools.reference.common.tap import FileTap
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    with RefModel(
        ...
        if args.decode > 0:
            try:
                sampler = Sampler(
                    _sampling_config(frontend, args),
                    model.config.token_domain,
                    model.device,
                )
            except ValueError as exc:
                parser.error(str(exc))
