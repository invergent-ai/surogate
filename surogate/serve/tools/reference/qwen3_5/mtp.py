"""Explicit one-layer MTP draft schedule."""
...
def forward(
    ...
    *,
    ...
    emb = rmsnorm(embeddings, model.weight(mtp_weights.embedding_norm), eps=cfg.rms_eps)
    previous = rmsnorm(hidden, model.weight(mtp_weights.hidden_norm), eps=cfg.rms_eps)
    x = linear(
    ...
    out = rmsnorm(x, model.weight(mtp_weights.final_norm), eps=cfg.rms_eps)
    token = (
        int(
            torch.argmax(
