...
class ModelState:
    ...
    def __post_init__(self) -> None:
        cfg = self.config
        head = dict(kv_heads=cfg.kv_heads, head_dim=cfg.head_dim)
        self.kv = KVCache(cfg.full_layers, self.capacity, self.device, self.kv_dtype, **head)
        self.mtp_kv = KVCache(1, self.capacity, self.device, self.kv_dtype, **head)
        self.conv = [
