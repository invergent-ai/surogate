"""Vision-tower schedule over a native SInfer artifact, for the checkpoints that ship one."""
...
from surogate.serve.tools.reference.common.vision_ops import (
    add_bias,
    apply_vision_rope,
    gelu,
...
class VisionEncoder:
    ...
    def __init__(
        ...
        *,
        ...
        if binding.vision_config is None:
            raise ValueError("this artifact has no vision tower")
        self.binding = binding
        self.config = binding.vision_config
        ...
        *,
        ...
        if combined_pixels.shape != (
            ...
            raise ValueError(
                ...
                f"[{int(combined_grid.prod(-1).sum())},{cfg.patch_dim}]"
            )
        ...
        tap_layers = {0, cfg.depth // 2, cfg.depth - 1}
        ...
        for layer in vision.layers:
            ...
            del weight
            x = residual_add(x, h)
            if tap and layer.index in tap_layers:
