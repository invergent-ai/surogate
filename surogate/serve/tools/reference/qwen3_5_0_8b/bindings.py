...
def _text_contract() -> tuple[_ExpectedTensor, ...]:
    tensors: list[_ExpectedTensor] = [_tensor("text/token_embedding", (248320, 1024), W8)]
    for layer in range(24):
        prefix = f"text/layers/{layer}/"
        tensors.append(_tensor(prefix + "input_norm", (1024,), BF16))
...
def _validate_inventory(artifact: Artifact) -> None:
    ...
    if len(_OBJECT_CONTRACT) != 287:
        raise RuntimeError("Qwen3.5-0.8B reference contract must contain 287 objects")
    if len(artifact.objects) != len(_OBJECT_CONTRACT):
        raise BindingError(
            f"artifact has {len(artifact.objects)} objects; expected 287"
        )
...
class ArtifactBinding:
    ...
    def __init__(self, artifact: Artifact, *, owns_artifact: bool = False):
        ...
        for layer in range(24):
            ...
            if layer in FULL_ATTENTION_LAYERS:
                qkgv = blocks[prefix + "attention/query_key_gate_value"]
                attention = FullAttentionBinding(
                    query_key=_row_view(qkgv, 0, 2560, (2560, 1024), row_views),
                    query=_row_view(qkgv, 0, 2048, (2048, 1024), row_views),
                    key=_row_view(qkgv, 2048, 2560, (512, 1024), row_views),
                    gate_value=_row_view(qkgv, 2560, 5120, (2560, 1024), row_views),
                    output_gate=_row_view(qkgv, 2560, 4608, (2048, 1024), row_views),
                    value=_row_view(qkgv, 4608, 5120, (512, 1024), row_views),
                ...
                convolution = AxisView(
                    convolution_storage, (1, 0), (6144, 4)
                )
                ...
                gdn = GdnBinding(
                    ...
                    query_key=_row_view(qkvz, 0, 4096, (4096, 1024), row_views),
                    query=_row_view(qkvz, 0, 2048, (2048, 1024), row_views),
                    key=_row_view(qkvz, 2048, 4096, (2048, 1024), row_views),
                    value_z=_row_view(qkvz, 4096, 8192, (4096, 1024), row_views),
                    value=_row_view(qkvz, 4096, 6144, (2048, 1024), row_views),
                    norm=blocks[prefix + "gdn/norm"],
                    z=_row_view(qkvz, 6144, 8192, (2048, 1024), row_views),
            ...
            layers.append(
                TextLayerBinding(
                    ...
                    mlp=MlpBinding(
                        gate_up=gate_up,
                        gate=_row_view(
                            gate_up, 0, 3584, (3584, 1024), row_views
                        ),
                        up=_row_view(
                            gate_up, 3584, 7168, (3584, 1024), row_views
                        ),
        ...
        self.mtp = MtpBinding(
            ...
            layer=MtpLayerBinding(
                ...
                attention=MtpAttentionBinding(
                    ...
                    query=_row_view(
                        mtp_qkgv, 0, 2048, (2048, 1024), row_views
                    ),
                    key=_row_view(
                        mtp_qkgv, 2048, 2560, (512, 1024), row_views
                    ),
                    output_gate=_row_view(
                        mtp_qkgv, 2560, 4608, (2048, 1024), row_views
                    ),
                    value=_row_view(
                        mtp_qkgv, 4608, 5120, (512, 1024), row_views
                    ),
                ...
                mlp=MlpBinding(
                    gate_up=mtp_gate_up,
                    gate=_row_view(
                        mtp_gate_up, 0, 3584, (3584, 1024), row_views
                    ),
                    up=_row_view(
                        mtp_gate_up, 3584, 7168, (3584, 1024), row_views
                    ),
        ...
        self.vision = None  # text-only target
        self.row_views = tuple(row_views)
        self.axis_views = tuple(axis_views)
        if len(self.tensors) != 281:
            raise RuntimeError(
                f"incomplete Qwen3.5-0.8B typed binding: {len(self.tensors)} tensors"
            )
        if len(self.axis_views) != 18:
            raise RuntimeError("incomplete Qwen3.5-0.8B GDN axis binding")
