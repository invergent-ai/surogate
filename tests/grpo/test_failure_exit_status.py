...
# `split` reaches the compiled extension through their config
...
def test_a_planned_teardown_is_not_reported_as_a_crash():
    """Teardown sets `shutdown_event` and then kills the server subprocesses, so
    ...
    with (
        ...
        split._watch_components([(proc, "rollout server")], threading.Event(), shutdown, abort)
...
def test_a_late_trainer_crash_is_not_discarded_by_teardown():
    ...
    Teardown kills the server subprocesses, so their sentinels fire exactly like a
