...
def test_flatten_descriptor_summary_totals_fusion_candidates():
    summary = {
        "forward": {
            ...
            "lora_schema_target_slices": 3,
            "forward_hook_points": 4,
            "forward_hook_schema_slot_points": 4,
            "forward_hook_schema_target_points": 4,
            "name": "forward",
        },
        "backward": {
            "num_ops": 12,
            ...
            "grouped_lora_schema_target_slices": 1,
            "name": "backward",
        },
    }
    ...
    assert flattened["lora_schema_target_slices"] == 5
    assert flattened["grouped_lora_schema_slot_slices"] == 1
    assert flattened["grouped_lora_schema_target_slices"] == 1
    assert flattened["forward_hook_points"] == 4
    assert flattened["forward_hook_schema_slot_points"] == 4
    assert flattened["forward_hook_schema_target_points"] == 4
...
def test_flatten_arena_summary_keeps_top_level_graph_and_region_counts():
    summary = {
        ...
        "arena_compiled_fwd_stack_bytes": 768,
        "arena_schema_total_activation_arena_bytes": 1536,
        "arena_schema_frame_arena_extra_bytes": 256,
    ...
    assert flattened["arena_compiled_fwd_stack_bytes"] == 768
    assert flattened["arena_schema_total_activation_arena_bytes"] == 1536
    assert flattened["arena_schema_frame_arena_extra_bytes"] == 256
...
"""
    ...
    assert summary["hook_after_communication_targets"] == 1
